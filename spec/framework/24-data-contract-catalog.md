...
| `DATA-OPS-016` | record | production promotion record | authoritative-release-control-plane | GitHub Actions tooling | R2 + workflow artifact | `promotion_id` | append-only per promote run | `DATA-OPS-014`,`DATA-OPS-015` | 记录当前 `promote-prod` 或 `operational-prod-refresh` workflow producer run identity、promotion authority（`reviewed_candidate` 或 `operational_unblock`）、promotion mode（gradual / deploy_with_migration）、previous/current deployment identity、gateway rollout steps、readiness snapshots 与 typed rollback handle。`reviewed_candidate` 路径必须记录被消费的 reviewed candidate 引用；`operational_unblock` 路径必须记录触发该例外路径的 blocker 列表与理由。producer 必须先验证 baseline/current Cloudflare identity 一致，`deploy_with_migration` 路径还必须记录 `jobs -> ops -> gateway` 三次切换后的 readiness snapshots。 |
