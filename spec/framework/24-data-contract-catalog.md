...
| `DATA-OPS-016` | record | production promotion record | authoritative-release-control-plane | GitHub Actions tooling | R2 + workflow artifact | `promotion_id` | append-only per promote run | `DATA-OPS-014`,`DATA-OPS-015` | 记录当前 `promote-prod` workflow producer run identity、被消费的 reviewed candidate 引用、promotion mode（gradual / deploy_with_migration）、previous/current deployment identity、gateway rollout steps、readiness snapshots 与 typed rollback handle。producer 必须先验证 baseline/current Cloudflare identity 一致，`deploy_with_migration` 路径还必须记录 `jobs -> ops -> gateway` 三次切换后的 readiness snapshots。 |
