...
| `DATA-OPS-012` | report | rollout skew probe snapshot | attested-release-artifact | `ops-worker` + pre-release harness | `EnvironmentRunReport` + raw run bundle | `{run_timestamp,probe_run_id}` | per probe serial | none | 记录 `TEST-OPS-001` 的 dual-version probe：至少包含 baseline/candidate gateway version IDs、对应 official version tags、dual-version deployment ID、explicit override strategy、probe-owned authority identities，以及 `new Worker -> old DO` / `old Worker -> new DO` 的 observed pair set。`observed_gateway_version_id` 若 runtime 未提供官方 version ID 则必须为 `null`；此时必须改由 official `observed_gateway_version_tag` 完成 override 观测，禁止把 repo-local `WORKER_VERSION_ID` fallback 伪装成官方 version ID。 |
| `DATA-OPS-013` | report | pre-release cost observation snapshot | attested-release-artifact | `ops-worker` + GitHub Actions tooling | `EnvironmentRunReport` + raw run bundle | `{run_timestamp,observation_id}` | per observation serial | none | 记录 `TEST-COST-001` pre-release half 的 bounded workload window、official Cloudflare HTTPS query locators（`cloudflare.com` 或其子域）、normalized `cost_surfaces` 与 `model_comparison`；它只能证明 pre-release 实际指标与模型比较，不替代 `ProdCostSnapshotAttestation`。 |
| `DATA-OPS-014` | manifest | production release candidate manifest | authoritative-release-control-plane | GitHub Actions tooling | R2 + workflow artifact | `candidate_id` | immutable per reviewed candidate | `EnvironmentRunAttestation` | 记录 `ci-integration` / `staging` / `pre-release` attestation、目标 release ref / commit、reviewed source run identity、当前 `release-candidate` workflow producer run identity 与是否包含 DO migration；consumer 还必须把 `source_repository` / `origin_repository` 绑定到当前仓库，production promote 只允许消费该 manifest，而不得重新脑补“当前 HEAD 即候选”。 |
| `DATA-OPS-015` | record | production install record | authoritative-release-control-plane | GitHub Actions tooling | R2 + workflow artifact | `install_id` | append-only per install run | Cloudflare account state | 记录当前 `prod-install` workflow producer run identity、prod bootstrap 所 ensure 的固定命名资源、Access application metadata（含 `application_domain`）、首次 deployment identity 与 workers.dev topology；它是后续 promote / rollback / prod-cost 的 topology baseline。该 record 只允许在 prod 仍未存在 active deployment identity 时被创建；一旦 active prod topology 已存在，`prod-install` 必须 fail-closed。 |
| `DATA-OPS-016` | record | production promotion record | authoritative-release-control-plane | GitHub Actions tooling | R2 + workflow artifact | `promotion_id` | append-only per promote run | `DATA-OPS-014`,`DATA-OPS-015`,`DATA-OPS-018` | 记录当前 `promote-prod` 或 `operational-prod-refresh` workflow producer run identity、promotion authority（`reviewed_candidate` 或 `operational_unblock`）、promotion mode（gradual / deploy_with_migration）、previous/current deployment identity、gateway rollout steps、readiness snapshots 与 typed rollback handle。`reviewed_candidate` 路径必须记录被消费的 reviewed candidate 引用；`operational_unblock` 路径必须记录触发该例外路径的 blocker 列表与理由。producer 必须先验证 baseline/current Cloudflare identity 一致；当 success baseline 已因 earlier failed prod automation 漂移而不可再用时，允许在再次 live revalidate 通过后消费 `DATA-OPS-018` typed `ProdCurrentStateSnapshot` 重建 live prod baseline，而不是伪造新的 success record。`deploy_with_migration` 路径还必须记录 `jobs -> ops -> gateway` 三次切换后的 readiness snapshots。 |
| `DATA-OPS-017` | record | production rollback record | authoritative-release-control-plane | GitHub Actions tooling | R2 + workflow artifact | `rollback_id` | append-only per rollback run | `DATA-OPS-016` | 记录当前 `rollback-prod` workflow producer run identity、source promotion、实际请求的 rollback handle、restored deployment identity、逐 worker restore 结果与 rollback 后 readiness snapshot。producer 必须先验证当前 Cloudflare prod identity 仍等于 source promotion 的 `current_deployment_identity`；若不匹配则不得 replay rollback handle。 |
| `DATA-OPS-018` | snapshot | production current-state blocker snapshot | raw-production-recovery-signal | GitHub Actions tooling | workflow raw-state artifact | `{origin_run_id,observed_at}` | append-only per failed prod recovery observation | `DATA-OPS-015`,`DATA-OPS-016`,`DATA-OPS-018 (typed recovery reuse only after live revalidation)`,`DATA-OPS-019` | 记录某次 fail-closed `promote-prod` / `operational-prod-refresh` 在停止前实测到的 live prod identity、workers.dev / protected-ops topology 元数据、baseline_match，以及每个固定 prod Worker 的 raw Cloudflare deployments/versions backread。它不是 success `ProdInstallRecord` / `ProdPromotionRecord`，只能作为后续恢复的 typed blocker snapshot；consumer 必须先验证 `origin_repository` 绑定当前仓库，并再次 live revalidate 当前 prod identity 仍完全等于该 snapshot 的 `current_deployment_identity`，否则不得继续部署。legacy raw blocker snapshot 必须先归一化成该 schema，不能直接作为 runtime baseline 输入；若 later recovery 再次以 typed blocker snapshot 为 baseline，则新的 snapshot 也必须显式保留这条回链。若 post-mutation refresh 又失败或不能继续保留 truthful canonical snapshot，则同次 raw blocker artifact 还必须伴随 `DATA-OPS-019` 明确记录 canonical `current-production-state.json` 被 refreshed、quarantined、removed 或仍处于 refresh_failed。 |
| `DATA-OPS-019` | report | production current-state disposition sidecar | raw-production-recovery-signal | GitHub Actions tooling | workflow raw-state artifact | `{observed_at,current_state_path}` | append-only per stale current-state handling event | `DATA-OPS-018` | 记录某次 prod recovery path 对 canonical `current-production-state.json` 的处置结果：已 refreshed、已 quarantine 到 `pre-failure-current-production-state.json`、已 remove canonical path，或 refresh 仍失败。最小字段至少包含当前 canonical basename、`disposition`、`pre_failure_snapshot_path`（若有）与 `refresh_error`（若有）。它不是 baseline，也不能替代 `DATA-OPS-018`；唯一用途是证明 stale canonical snapshot 没有继续被保留成看似可用的 live current state。 |
