...
| `DATA-OPS-012` | report | rollout skew probe snapshot | attested-release-artifact | `ops-worker` + pre-release harness | `EnvironmentRunReport` + raw run bundle | `{run_timestamp,probe_run_id}` | per probe serial | none | 记录 `TEST-OPS-001` 的 dual-version probe：至少包含 baseline/candidate gateway version IDs、dual-version deployment ID、explicit override strategy、probe-owned authority identities，以及 `new Worker -> old DO` / `old Worker -> new DO` 的 observed pair set。 |
| `DATA-OPS-013` | report | pre-release cost observation snapshot | attested-release-artifact | `ops-worker` + GitHub Actions tooling | `EnvironmentRunReport` + raw run bundle | `{run_timestamp,observation_id}` | per observation serial | none | 记录 `TEST-COST-001` pre-release half 的 bounded workload window、official Cloudflare query sources、normalized `cost_surfaces` 与 `model_comparison`；它只能证明 pre-release 实际指标与模型比较，不替代 `ProdCostSnapshotAttestation`。 |
