...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `CF-WKR-001` | Workers | lifecycle | workers-limits | Paid | 入站 HTTP 请求在客户端保持连接期间没有固定 wall-time cap。Cloudflare 只对 runtime update 明确记录了 in-flight request 的 `30s` grace period；deployment 切换或其他平台中断会影响 in-flight requests，但文档未承诺同样的 grace。 | `/_matrix/client/v3/sync` 可以由 Worker 持有长轮询，但必须把 runtime update、deployment 切换与其他平台中断都视为正常早返回原因；实现不得把 `30s` grace 泛化成“所有中断都保证有 30 秒”。 | `21`,`30`,`42` |
...
| `CF-WKR-003` | Workers | limits | workers-limits, workers-pricing | Paid | 在 Workers `Standard` usage model 下，HTTP 请求 CPU time 默认 `30s`，可提升至 `5 min`。Cloudflare 定价文档同时说明，迁移自 legacy Bundled 的既有 Worker 可能仍保留自动 `50 ms` CPU limit，直到显式修改。Queue consumer 与 DO alarm 的 wall time 上限为 `15 min`。 | 权威热路径必须远低于显式配置的 CPU 上限；发布门禁必须验证并记录每个 Worker 的实际 CPU limit，不得假设“默认就是 30 秒”；重建、缩略图、导出必须异步化。 | `13`,`21`,`30`,`31`,`33`,`42` |
...
| `CF-WKR-012` | Worker deployments | deployment | workers-versions-deployments | Paid | Worker 版本与部署是分离概念；新版本可先上传后再部署；单个 deployment 可同时承载 `1` 或 `2` 个 Worker versions。`wrangler versions upload` 不支持携带 Durable Object migrations；涉及 DO migration 时必须改用 `wrangler deploy`。Cloudflare 还明确指出，引用某个指定 version 的 deployment 可能尚未传播完成，因此“控制面已显示该 deployment/version”不等于所有请求路径已立即稳定命中新代码。 | 生产发布必须使用 versions/deployments，而不是隐式“每改即发”；任何跨请求签名 token 或兼容契约都必须容忍 rollout 期间的双版本并存；带 DO migration 的发布流程不能假设“先 upload 再 deploy”。对 release-gate non-local harness，仅凭 fresh deployment/version IDs 仍不足以宣告 ready，deploy 后还必须经由真实请求路径的 readiness probe 再进入 suite。 | `21`,`30`,`40`,`42` |
...
| `CF-WKR-015` | Workers Logs | billing/retention | workers-pricing, workers-logs | Paid | Workers Logs 含 `20M` log events/月，保留 `7` 天；单条 Workers Logs event 最大 `256 KB`，超限会被截断，并由平台把 `$cloudflare.truncated` 设为 `true`。 | 生产必须设计日志采样、摘要化与截断识别策略；应用与证据管道不得把被截断事件当作完整记录。 | `41` |
...
| `CF-WKR-017` | OpenTelemetry / Traces | billing | workers-opentelemetry, workers-traces | Paid | Cloudflare observability 文档当前对 tracing quota/pricing 存在冲突：OTel export 页面写明自 `2026-03-01` 起 traces 与 logs 各含 `10M` events/月、超额 `$0.05 / million`；Traces 页面则写 traces spans 与 Workers Logs 共享 event quota/pricing/retention。当前本地基线不能把 tracing 的精确 included quota、overage price 或 retention 视为已澄清事实。 | 启用 traces / OTel 时，成本模型与门禁必须把 trace span volume 单独参数化，并回链 `OQ-0002`；在 Cloudflare 口径收敛前，禁止把某一组 tracing 计费数字硬编码为确定真相。 | `15`,`41` |
...
| `CF-WKR-029` | Billing Usage API | billing | billing-usage-paygo-api | PayGo / select accounts | `GET /accounts/{account_id}/billing/usage/paygo` 是官方 PayGo billable-usage API；无 query 参数时返回 current billing period；该 endpoint 当前仍是 beta，且访问只对 select accounts 开放。 | production monthly cost automation 必须先把 target account 对该 API 的真实可用性当成前置条件；若 endpoint 不可用或返回非成功结果，workflow 必须 fail-closed 并保留 raw blocker artifacts。 | `15`,`41`,`42` |
| `CF-WKR-030` | Billing cycle | billing | billing-usage-based-billing, billing-profile-api, account-subscriptions-api | Account billing | Cloudflare usage-based billing 以账户 billing period 为单位，不保证与 calendar month 对齐；官方示例明确说明 billing date 会决定发票覆盖窗口。官方 billing profile API 在文档中暴露 `next_bill_date`，而 account subscriptions API 暴露 `current_period_end`（即下次 billing due）；两者都可作为 official closed-period anchor，但若当前账号只返回多组不一致或缺失的锚点，则不得脑补。 | `prod-cost-monthly` 不得再假设“上一个自然月”就是真实 closed billing window；workflow/tooling 必须先根据官方 billing period 事实解析最新 closed window，再抓取 usage records，并在任何无法证明窗口 truthful 的情况下 fail-closed。 | `15`,`41`,`42` |
...
| `CF-WKR-023` | Workers RPC / Service Bindings RPC / Durable Object RPC | limits | workers-rpc, do-rpc-stubs | Paid | 普通 serialized RPC message 的 hard ceiling 为 `32 MiB`；更大传输必须改用 stream-based transfer，而不是单条序列化消息。Durable Object method-call RPC 需按 DO stubs 指南回链到 Workers RPC 文档理解该 transport 约束，且接收端类必须继承 Workers runtime 的 `DurableObject` 基类，否则 stub method call 会直接失败。 | 所有内部 Worker/DO RPC 契约都必须分页、分段或改为 stream / R2 locator 设计；不得把大房间投影、导出段或审计结果一次性塞进单个 RPC 返回值。任何通过 `stub.method()` 暴露的 DO authority class 都必须继承 runtime `DurableObject` 基类，不能只靠 Node 本地测试 shim。 | `21`,`23`,`26`,`30`,`31`,`32`,`42` |
...
| `CF-WKR-028` | Workers global fetch | routing | workers-limits, workers-compatibility-flags | Paid | Cloudflare 明确说明：使用全局 `fetch()` 在同一 zone 内调用另一个 Worker、且未使用 service binding 时会失败；若启用 `global_fetch_strictly_public` compatibility flag，请求才会按“来自公共 Internet”的方式重新走 Cloudflare front door。未启用该 flag 时，同 zone 请求会被路由到 zone origin，并绕过映射到该 URL 的 Worker 与 Cloudflare security settings。 | 任何从一个 Worker 调用同 zone public Worker hostname 的实现，都必须在 service binding / custom domain / `global_fetch_strictly_public` 三者之间做显式选择，不能默认把 `https://<worker>.<subdomain>.workers.dev` 当成普通外部 URL。对于当前 `ops-worker -> MATRIX_PUBLIC_BASE_URL` 的 rollout-skew probe，若继续使用 `workers.dev` public ingress 而不是 service binding，就必须显式钉住 `global_fetch_strictly_public`；否则 Cloudflare 生成的 same-zone failure（例如 error `1042`）必须被视为平台约束命中，而不是产品语义证据。 | `21`,`42` |
...
| `CF-DO-005` | Durable Objects | deployment | do-known-issues, do-lifecycle | Paid | Worker 与 DO 代码更新是全局最终一致发布，短时间内会出现新 Worker 调旧 DO 版本，窗口通常为数秒到数分钟。 | Worker/DO 接口必须前后兼容；迁移必须与普通发布解耦。对刚部署完成的 non-local environment，不得把“Cloudflare API 已回读到最新 deployment/version IDs”直接视为关键请求路径已稳定，suite 前还必须等待代表性 Worker<->DO 路径 readiness 成功。 | `21`,`23`,`42` |
...
| `CF-DO-020` | Durable Objects SQLite | transaction semantics | do-sqlite-storage-api | Paid | `ctx.storage.sql.exec()` 不允许执行 `BEGIN TRANSACTION`、`SAVEPOINT` 等事务控制语句；显式事务必须使用 `ctx.storage.transaction()` 或 `ctx.storage.transactionSync()`。其中 `transactionSync()` 仅接受同步回调，回调抛错时事务回滚。 | 所有基于 SQLite-backed DO 的 persistence/authority 事务封装都必须经由 `ctx.storage.transactionSync()`（或适配的 `transaction()`），不能直接拼装原生 SQL 事务语句。Node 本地 SQLite shim 若允许 `BEGIN/COMMIT`，也不能单独证明 Cloudflare runtime 兼容性。 | `21`,`22`,`23`,`42` |
| `CF-DO-021` | Durable Objects gradual deployments | deployment | workers-gradual-deployments | Paid | 在 gradual deployment 下，每个 Durable Object 实例会按当前 deployment 的 version percentages 被分配到某个 Worker version；对同一 deployment，该实例后续请求始终落到同一已分配 version，直到创建新的 deployment。`Cloudflare-Workers-Version-Key` / `Cloudflare-Workers-Version-Overrides` 可以控制请求命中哪个 Worker version，但不会改写该 deployment 已决定的 Durable Object version assignment。 | rollout-skew proof 不能再依赖 `baseline=100%`、`candidate=0%` 再配合 version override 来“制造” candidate-side authority；若要同时观察 `new Worker -> old DO` 与 `old Worker -> new DO`，dual-version deployment 必须给两侧非零份额，并对 probe-owned authority identities 做 bounded sampling，直到真实拿到 baseline/candidate 两类 DO assignment，否则 fail-closed。 | `42` |
...
| `CF-NET-007` | Cloudflare Access | application hostname ownership | access-applications, workers-dev | Zone | Access self-hosted public application 的 hostname/domain 必须属于当前账户中的 active zone；但 Workers 当前官方文档另外允许对 account-owned `workers.dev` 与 Preview URL 直接启用 Cloudflare Access。换言之，active-zone custom hostname 不是唯一合法 Access ingress；不过占位 team domain、未启用 Access 的裸 `workers.dev` URL、或无可验证 AUD / service-auth contract 的配置仍不能视为受保护管理面。 | non-local `ops-worker` 的 positive coverage 可以走 active-zone management hostname，或走已实际启用 Access 的 account-owned `workers.dev` ingress；无论哪条路径，workflow 与 Spec 都必须记录真实 team domain、AUD、service-auth/JWT contract。仅有 `workers.dev` URL 与 `.cloudflareaccess.invalid` placeholder 仍不足以形成可审计 Access ingress。 | `21`,`40`,`42` |
...
* 任何从 Worker 内发起、目标仍位于同 zone public hostname 的 HTTP 调用，都必须显式选用 service binding / custom domain / `global_fetch_strictly_public` 其一；不得把默认 global `fetch()` 路径当成可稳定命中目标 Worker 的普通外部请求。引用：`CF-WKR-028`。
