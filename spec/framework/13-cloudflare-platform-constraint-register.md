...
| `CF-WKR-030` | Billing cycle | billing | billing-usage-based-billing, billing-profile-api | Account billing | Cloudflare usage-based billing 以账户 billing period 为单位，不保证与 calendar month 对齐；官方示例明确说明 billing date 会决定发票覆盖窗口，而官方 billing profile API 暴露 `next_bill_date` 可用于解析当前账期锚点。 | `prod-cost-monthly` 不得再假设“上一个自然月”就是真实 closed billing window；workflow/tooling 必须先根据官方 billing period 事实解析最新 closed window，再抓取 usage records，并在任何无法证明窗口 truthful 的情况下 fail-closed。 | `15`,`41`,`42` |
