...
| Cloudflare Workers runtime and pricing surfaces | local snapshots keyed by `workers-limits`,`workers-pricing`,`workers-logs`,`workers-opentelemetry`,`workers-rpc`,`service-bindings`,`workers-placement`,`workers-gradual-deployments`,`workers-versions-deployments`,`workers-secrets` | latest observed docs match pinned semantics used by current `CF-WKR-*` rows | `2026-03-26` | `none` | `no-op` | platform | pre-release + monthly |
| Cloudflare storage and queue surfaces | local snapshots keyed by `do-limits`,`do-lifecycle`,`do-alarms`,`do-known-issues`,`do-rpc-stubs`,`do-migrations`,`do-sqlite-storage-api`,`do-websockets`,`do-pricing`,`d1-limits`,`d1-read-replication`,`d1-pricing`,`kv-how-it-works`,`r2-consistency`,`r2-limits`,`r2-pricing`,`queues-pricing` | latest observed docs match pinned semantics used by current `CF-DO-*`,`CF-D1-*`,`CF-KV-*`,`CF-R2-*`,`CF-QUE-*` rows | `2026-03-26` | `none` | `no-op` | platform | pre-release + monthly |
| Cloudflare network and access control surfaces | local snapshots keyed by `network-ports`,`access-application-paths`,`access-validate-jwt`,`access-service-tokens` | latest observed docs match pinned semantics used by current `CF-NET-*` rows | `2026-03-26` | `none` | `no-op` | security + platform | pre-release + monthly |
