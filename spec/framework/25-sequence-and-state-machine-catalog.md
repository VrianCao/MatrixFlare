...
| `FLOW-OPS-PROD-COST-SNAPSHOT` | production monthly cost snapshot | `41`,`42`,`44` | GitHub Actions, official Cloudflare billing API, `DATA-OPS-015`, `ProdCostSnapshotAttestation` | first-of-month schedule or explicit prod-cost request | resolve the closed calendar-month billing window, capture raw billing/resource/deployment observations for the fixed prod topology, upload the raw bundle to immutable R2, then write provenance and `ProdCostSnapshotAttestation` | missing prod topology, missing official billing access, raw bundle upload failure, unresolved pricing semantics, or attestation/provenance failure must fail-closed; raw blocker artifacts must still be preserved |
