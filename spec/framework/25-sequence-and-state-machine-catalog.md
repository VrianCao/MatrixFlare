...
| `FLOW-OPS-PROD-PROMOTE` | production release promotion | `42` | GitHub Actions, `DATA-OPS-014`, `DATA-OPS-015`, Cloudflare versions/deployments, Workers | explicit prod promote request | validate reviewed candidate manifest against the current repository and checked-out commit, validate current prod state against baseline record, promote `jobs-worker` / `ops-worker`, roll `gateway-worker` through bounded gradual steps when no DO migration, or run the migration-safe `jobs -> ops -> gateway full` deploy path when DO migration is required, record readiness at each gate, and persist rollback handle in `DATA-OPS-016` | missing candidate attestation, repository/head mismatch, Cloudflare state drift, version upload failure, readiness failure, or restore-handle generation failure must fail-closed and stop promotion |
