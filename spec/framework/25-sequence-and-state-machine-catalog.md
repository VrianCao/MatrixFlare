...
| --- | --- | --- | --- | --- | --- | --- |
| `FLOW-CS-DISCOVERY` | client discovery | `30` | client, `gateway-worker` | `GET /.well-known/matrix/client`, `GET /_matrix/client/versions`, `GET /_matrix/client/*/capabilities`, or `GET /_matrix/client/{r0,v1,v3}/login` | 返回 homeserver 元数据、当前真实支持的 `/versions` compatibility ladder、capabilities 与当前真实支持的 login flow advertisement；其中 `/versions` ladder 只解决 browser compatibility discovery，不自动放宽 `DEC-0001` 的 stub-only surfaces，见 `DEC-0007` | 静态错误、discoverability truth 漂移或缓存回退 |
...
| `FLOW-CS-REGISTER` | registration | `30` | client, `gateway-worker`, `UserDO` | `GET /_matrix/client/{r0,v1,v3}/register`, `GET /_matrix/client/{r0,v1,v3}/register/available`, `GET /_matrix/client/v1/register/m.login.registration_token/validity`, or `POST /_matrix/client/{r0,v1,v3}/register` | registration discovery / availability surface 返回当前真实支持的 UIA stage、用户名可用性与 token validity；最终注册提交创建用户、初始设备与初始 session | policy 禁止、UIA 未完成、用户名占用或 token 无效时返回稳定错误；不得出现半创建，也不得让 alias 间 challenge 或错误模型漂移 |
| `FLOW-CS-LOGIN` | login | `30` | client, `gateway-worker`, `UserDO` | `GET /_matrix/client/{r0,v1,v3}/login`, `POST /_matrix/client/{r0,v1,v3}/login`, or `GET /_matrix/client/*/account/whoami` | login discovery 返回当前真实支持的 flow；登录提交认证、创建设备 session 并返回 token；`whoami` 返回当前 access token 绑定主体 | 失败不创建任何 session；discoverability、成功写入、错误模型与 `whoami` 读取都不得因 compatibility alias 或 session 漂移而失真 |
...
| `FLOW-CS-REFRESH` | refresh token and logout | `30` | client, `gateway-worker`, `UserDO` | `POST /_matrix/client/*/refresh`, `POST /_matrix/client/*/logout`, or `POST /_matrix/client/*/logout/all` | 校验 refresh token 并轮换 session，或按路由撤销当前/全部 session | refresh 重放必须失败或返回已轮换结果；logout / logout-all 不得留下仍可继续通过 access token 的已撤销 session |
...
| `FLOW-OPS-PROD-CANDIDATE` | reviewed production release candidate issuance | `42`,`44` | GitHub Actions, `EnvironmentRunAttestation`, `DATA-OPS-014` | explicit release-candidate request | validate the reviewed `ci-integration` / `staging` / `pre-release` attestation trio, bind them to the current repository and shared source run identity, and mint immutable `ReleaseCandidateManifest` | missing attestation, repository mismatch, run-timestamp mismatch, or stale source run identity must fail-closed |
| `FLOW-OPS-PROD-INSTALL` | production topology install | `42` | GitHub Actions, Cloudflare account API, Zero Trust Access, Workers, D1, KV, R2, Queues | explicit prod install request | ensure fixed-name prod resources, ensure workers.dev Access application for `ops-worker`, verify prod topology is still unbootstrapped, bootstrap deploy in `gateway bootstrap -> jobs -> ops -> gateway full` order, record install artifact | missing credentials, partial resource state, stale identity, existing active prod deployment, or failed bootstrap deploy must fail-closed; install may be retried only while prod still lacks active deployment identity and must never silently accept placeholder topology |
| `FLOW-OPS-PROD-PROMOTE` | production release promotion | `42` | GitHub Actions, `DATA-OPS-014`, `DATA-OPS-015`, `DATA-OPS-018`, `DATA-OPS-019`, Cloudflare versions/deployments, Workers | explicit prod promote request | validate reviewed candidate manifest against the current repository and checked-out commit, validate current prod state against baseline record or a prior typed `ProdCurrentStateSnapshot`, promote `jobs-worker` / `ops-worker`, roll `gateway-worker` through bounded gradual steps when no DO migration, or run the migration-safe `jobs -> ops -> gateway full` deploy path when DO migration is required, record readiness at each gate, persist rollback handle in `DATA-OPS-016`, and when post-mutation refresh cannot safely leave a truthful canonical current-state snapshot, emit a `DATA-OPS-019` disposition sidecar that records whether the canonical path was refreshed, quarantined, or removed | missing candidate attestation, repository/head mismatch, Cloudflare state drift, stale or non-normalizable current-state snapshot, missing required current-state disposition sidecar, version upload failure, readiness failure, or restore-handle generation failure must fail-closed and stop promotion |
| `FLOW-OPS-PROD-ROLLBACK` | production rollback | `42` | GitHub Actions, `DATA-OPS-016`, Cloudflare versions/deployments, Workers | explicit rollback request | validate rollback handle, verify current Cloudflare prod state still matches the source promotion record, restore previous active worker versions, run bounded post-rollback readiness probe, persist `DATA-OPS-017` | missing previous version identity, DO-migration-only rollback request, prod-state drift, or readiness failure must fail-closed and leave an auditable rollback record |
| `FLOW-OPS-PROD-COST-SNAPSHOT` | production monthly cost snapshot | `41`,`42`,`44` | GitHub Actions, official Cloudflare billing profile + billing usage API, `DATA-OPS-015`, `ProdCostSnapshotAttestation` | first-of-month schedule or explicit prod-cost request | resolve the latest closed Cloudflare billing-period window from official billing profile `next_bill_date`, or from the unique account subscriptions `current_period_end` when `next_bill_date` is absent, download the current prod baseline `ProdInstallRecord`, prove the resolved window starts strictly after `installed_at`, capture raw billing/resource/deployment observations for the fixed prod topology, upload the raw bundle to immutable R2, then write provenance and `ProdCostSnapshotAttestation` | missing prod topology, missing official billing access, missing trustworthy billing-period anchor, conflicting subscriptions anchors, missing prod install baseline, a window that still overlaps pre-install spend, raw bundle upload failure, unresolved pricing semantics, or attestation/provenance failure must fail-closed; raw blocker artifacts must still be preserved |
...
| `STATE-PROD-ROLLOUT` | production rollout lifecycle | `42` | prod promote / rollback run | candidate_validated, versions_uploaded, jobs_promoted, ops_promoted, gateway_canary, gateway_stable, rollback_ready, rollback_requested, rolled_back, failed | bounded blast radius, rollback availability, and readiness at every transition |
| `STATE-PROD-COST-SNAPSHOT` | production cost snapshot lifecycle | `41`,`42`,`44` | monthly prod-cost run | billing_window_resolved, raw_snapshot_captured, raw_bundle_uploaded, provenance_written, attested, failed | raw blocker retention, immutable provenance, and closed-month truthfulness |
