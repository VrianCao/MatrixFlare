...
| --- | --- | --- | --- |
| Legacy initial sync and deprecated event stream routes | `Deferred` | `MX-CS-029`,`DEC-0001` | 当前产品边界已固定为 stub-only；若要启用，必须先补 dedicated compatibility contracts，并以新的或替代 `DEC-0001` 明示放开。 |
...
| Joined rooms / room alias directory / room summary | `Deferred` | `MX-CS-021`,`DEC-0001` | 当前产品边界固定为 stub-only；未来若启用必须先补 dedicated contracts，并以新的或替代 `DEC-0001` 明示放开。 |
| Notifications listing endpoint | `Deferred` | `MX-CS-022`,`DEC-0001` | `/sync` 计数不等于 `/notifications` 列表语义；当前产品边界固定为 stub-only；只有新的或替代 `DEC-0001` 才可放开。 |
