...
| `MX-CS-012` | Client-Server | `v1.17` | URL preview | `Required-Conditional` | `33`,`40` | isolated preview worker | `none` | `none` | `none` | `TEST-SEC-002` | `EVID-SEC-001` | `L3` | 默认关闭；启用前必须先补齐隔离抓取器与 SSRF 子规范。 |
...
| `MX-CS-016` | Client-Server | `v1.17` | pushers and external push gateway integration | `Required-Conditional` | `30`,`40` | `gateway-worker`,`UserDO` | `none` | `none` | `none` | `TEST-SEC-002` | `EVID-SEC-001` | `L3` | 本系统不内建 Push Gateway；启用前必须先补齐外部推送网关子规范。 |
...
| `MX-CS-018` | Client-Server | `v1.17` | reporting APIs and abuse-report submission | `Deferred` | `40` | `gateway-worker`,`ops-worker` | `none` | `none` | `none` | `TEST-SEC-002` | `EVID-SEC-001` | `L3` | 需补齐运维处置流程与 abuse workflow 子规范后才能 GA。 |
...
| `MX-RV-013` | Room Version | `v1.17` | older stable room versions | `Deferred` | `31` | `RoomDO` | `IF-INT-ROOM-001` | `DATA-ROOM-001`-`008` | `FLOW-ROOM-EVENT-ADMISSION` | `TEST-ROOM-002` | `EVID-ROOM-002` | `L2-L3` | 只有新增独立 strategy 并通过对应测试门禁后才可支持。 |
