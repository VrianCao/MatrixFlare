...
* 当前实现基线：Matrix `v1.17`。
* 当前观察到的 `latest`：`2026-03-26` 时为 `v1.17`。
...
| `MX-CS-011` | Client-Server | `v1.17` | media config, upload, download, thumbnail | `Required-Core` | `33` | `gateway-worker`,`UserDO`,R2 | `IF-CS-050`,`IF-CS-051`,`IF-QUE-002` | `DATA-USER-015`,`DATA-R2-001`,`DATA-R2-002`,`DATA-R2-003`,`DATA-D1-004` | `FLOW-CS-MEDIA-UPLOAD`,`FLOW-CS-MEDIA-DOWNLOAD`,`FLOW-CS-REMOTE-MEDIA-FETCH` | `TEST-MEDIA-001` | `EVID-MEDIA-001` | `L1-L3` | `/_matrix/client/v1/media/{config,download,thumbnail}` 是 `v1.17` current authenticated media surface，当前 profile 下这些路由必须要求 access token；`/_matrix/media/*` 对应家族仅作为兼容/替代路径保留，且不得弱化鉴权或改变参数语义。上传仍受 Cloudflare request body limit 约束。 |
| `MX-CS-012` | Client-Server | `v1.17` | URL preview | `Deferred` | `33`,`40` | isolated preview worker | `IF-CS-058` | `none` | `FLOW-CS-DISABLED-ROUTE` | `TEST-CS-004` | `EVID-CS-004` | `L1-L3` | 包括 `GET /_matrix/client/v1/media/preview_url` 与已弃用的 `GET /_matrix/media/v3/preview_url`；在 dedicated IF/DATA/FLOW 与 SSRF 子规范完整落地且由新的或替代 [DEC-0001](/root/Matrix/spec/decisions/DEC-0001.md) 明示放开前，不允许宣称支持。 |
...
| `MX-CS-016` | Client-Server | `v1.17` | pushers and external push gateway integration | `Required-Conditional` | `30`,`40` | `gateway-worker`,`UserDO` | `IF-CS-055` | `none` | `FLOW-CS-DISABLED-ROUTE` | `TEST-CS-004`,`TEST-SEC-002` | `EVID-CS-004`,`EVID-SEC-001` | `L1-L3` | 包括 `GET /_matrix/client/*/pushers` 与 `POST /_matrix/client/*/pushers/set`；`L1-L3` 都必须维持 deterministic disabled truth；仅在 dedicated contracts、外部推送网关子规范完整落地且由新的或替代 [DEC-0001](/root/Matrix/spec/decisions/DEC-0001.md) 明示放开后，才允许启用完整功能。 |
...
| `MX-CS-025` | Client-Server | `v1.17` | registration/password-reset email and msisdn `requestToken` bootstrap surfaces | `Required-Conditional` | `30`,`40` | `gateway-worker` | `IF-CS-007` | `none` | `FLOW-CS-DISABLED-ROUTE` | `TEST-CS-004`,`TEST-SEC-002` | `EVID-CS-004`,`EVID-SEC-001` | `L1-L3` | 包括 `/_matrix/client/*/register/{email,msisdn}/requestToken` 与 `/_matrix/client/*/account/password/{email,msisdn}/requestToken`；`L1-L3` 都必须维持 deterministic stub truth；仅在 verification transport、provider trust 与 abuse budget 子规范完整落地且由新的或替代 [DEC-0001](/root/Matrix/spec/decisions/DEC-0001.md) 明示放开后，才允许启用完整功能。 |
...
| `MX-CS-029` | Client-Server | `v1.17` | legacy stream routes and room-preview initial snapshot compatibility surface | `Deferred` | `30`,`31` | `gateway-worker` | `IF-CS-064` | `none` | `FLOW-CS-DISABLED-ROUTE` | `TEST-CS-004` | `EVID-CS-004` | `L1-L3` | 包括 `GET /_matrix/client/*/events`、`GET /_matrix/client/*/events/{eventId}`、`GET /_matrix/client/*/initialSync` 与 `GET /_matrix/client/*/rooms/{roomId}/initialSync`；其中前 3 个属于 legacy/deprecated compatibility surface，`GET /rooms/{roomId}/initialSync` 本身不是 deprecated，但在当前项目中仍作为 room-preview compatibility surface 保持 `Deferred`；在 dedicated compatibility contracts 完整落地且由新的或替代 [DEC-0001](/root/Matrix/spec/decisions/DEC-0001.md) 明示放开前，不得宣称支持。 |
...
| --- | --- | --- | --- |
| Legacy stream routes and room-preview initial snapshot compatibility surface | `Deferred` | `MX-CS-029`,`DEC-0001` | 当前产品边界已固定为 stub-only；其中 `GET /rooms/{roomId}/initialSync` 仍是 Matrix `v1.17` current surface，但在本项目中与 legacy stream compatibility 一并保持 `Deferred`；若要启用，必须先补 dedicated compatibility contracts，并以新的或替代 `DEC-0001` 明示放开。 |
| In-band identity service features | `Required-Conditional` | `MX-CS-003`,`MX-CS-005`,`DEC-0001` | 依赖外部 identity integration，不属于本系统内建；当前产品边界固定为 stub-only，只有 dedicated contracts 与新的或替代 `DEC-0001` 后才允许启用。 |
| Push gateway delivery | `Required-Conditional` | `MX-CS-016`,`DEC-0001` | 依赖外部 push 基础设施；当前产品边界固定为 stub-only；只有 dedicated contracts 与新的或替代 `DEC-0001` 后才允许启用。 |
| URL preview | `Deferred` | `MX-CS-012`,`DEC-0001` | 默认关闭；在 dedicated contracts、隔离抓取器子规范与新的或替代 `DEC-0001` 落地前不得宣称支持。 |
| Reporting APIs | `Deferred` | `MX-CS-018`,`DEC-0001` | 当前产品边界固定为 stub-only；未来若启用必须先补齐 abuse workflow 子规范，并以新的或替代 `DEC-0001` 明示放开。 |
| Email/MSISDN requestToken bootstrap | `Required-Conditional` | `MX-CS-025`,`DEC-0001` | 依赖可验证的 email / SMS transport、provider trust 与滥用预算；当前产品边界固定为 stub-only；只有新的或替代 `DEC-0001` 才可放开。 |
...
| VoIP TURN credentials | `Required-Conditional` | `MX-CS-028`,`DEC-0001` | 依赖外部 TURN 基础设施与凭据轮换；当前产品边界固定为 stub-only；只有 dedicated contracts 与新的或替代 `DEC-0001` 后才允许启用。 |
...
| Joined rooms / room alias directory / room summary | `Deferred` | `MX-CS-021`,`DEC-0001` | 这些仍是 Matrix `v1.17` current surfaces；当前产品边界固定为 stub-only，而不是声称它们已被上游废弃；未来若启用必须先补 dedicated contracts，并以新的或替代 `DEC-0001` 明示放开。 |
| Notifications listing endpoint | `Deferred` | `MX-CS-022`,`DEC-0001` | `/sync` 计数不等于 `/notifications` 列表语义；该端点仍是 Matrix `v1.17` current surface，当前产品边界只是把它固定为 stub-only；只有新的或替代 `DEC-0001` 才可放开。 |
...
| Room upgrade | `Deferred` | `MX-CS-023`,`DEC-0001` | replacement room 与迁移语义尚未钉死；当前产品边界固定为 stub-only；未来若启用必须先补 dedicated contracts，并以新的或替代 `DEC-0001` 明示放开。 |
