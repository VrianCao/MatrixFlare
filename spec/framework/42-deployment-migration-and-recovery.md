...
* 当 `promote-prod` 或 `operational-prod-refresh` 因 baseline/current Cloudflare production identity mismatch、active deployment identity 不可解析、或当前 active worker version set 不满足 rollout 前提而 fail-closed 时，workflow raw blocker artifacts 不能只保留一句报错；至少还必须保留 typed `ProdCurrentStateSnapshot`，其中包含当前实测 prod worker state、每个固定 prod Worker 的 script name、latest active deployment ID、active worker version IDs、workers.dev / protected ops topology 元数据，以及官方 deployments/versions API 回读结果（若调用成功），从而让后续 repair 能诚实重建当前 prod baseline，而不是继续猜测 Cloudflare 实际状态。若 post-mutation refresh 自身也失败，则 canonical `current-production-state.json` 不得继续指向 pre-failure snapshot；workflow 必须把该 snapshot quarantine 到 non-canonical path，或直接删除 canonical 文件名后再 fail-closed，并同时留下 `ProdCurrentStateDisposition` 说明 canonical path 最终是 refreshed、quarantined、removed 还是仍处于 refresh_failed。
* 若某次 earlier `promote-prod` / `operational-prod-refresh` 在部分 worker 已切换后失败，导致先前 success `ProdInstallRecord` / `ProdPromotionRecord` 不再匹配 live prod state，则后续恢复 run 允许把 prior raw blocker 中的 `ProdCurrentStateSnapshot` 作为 baseline 输入，而不要求先伪造新的 success promotion record；但 consumer 仍必须先验证 `origin_repository` 绑定当前仓库，并再次回读 live Cloudflare prod identity，证明它仍与该 snapshot 记录的 `current_deployment_identity` 完全一致。任何 stale snapshot、缺失 `current_deployment_identity` 的 snapshot、或 live prod 已再次漂移的情况都必须继续 fail-closed。
...
* production automation 的可审计基线固定为：`ReleaseCandidateManifest`、`ProdInstallRecord`、`ProdPromotionRecord`、`ProdCurrentStateSnapshot`、`ProdCurrentStateDisposition`、`ProdRollbackRecord` 与 `ProdCostSnapshotAttestation`。其中 `ProdCurrentStateSnapshot` 只允许作为 raw blocker 驱动的 recovery baseline 使用，并且 consumer 仍必须重新 live revalidate `current_deployment_identity`；`ProdCurrentStateDisposition` 只允许作为 stale canonical snapshot 处置 sidecar 使用，不能单独替代 snapshot truth。以上工件都必须回链 GitHub run identity 与 Cloudflare deployment identity。
* `release-candidate`、`prod-install`、`promote-prod`、`operational-prod-refresh`、`rollback-prod`、`prod-cost-monthly` 即使最终 fail-closed，也必须尽可能先把 raw state / blocker artifact 上传为 workflow artifact；`prod-cost-monthly` 还必须先把 raw cost bundle 上传到 immutable R2，再允许后续 provenance / attestation 阶段失败。
