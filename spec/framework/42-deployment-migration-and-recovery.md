...
* `prod-cost-monthly` 在查询 `billing/usage/paygo` 前，必须先通过官方 billing profile `next_bill_date` 解析 latest closed billing period，并下载当前 prod baseline `ProdInstallRecord`；不得再把“上一个自然月”写死为 production snapshot window，也不得在 `billing_period.start` 仍早于或等于 install date 时生成 `ProdCostSnapshotAttestation`。若当前账号对 Billing Usage API 仍不可用、billing-cycle 解析结果不足以证明窗口 truthful、或找不到有效的 prod install baseline，则 workflow 必须 fail-closed 并保留 raw blocker artifacts。
