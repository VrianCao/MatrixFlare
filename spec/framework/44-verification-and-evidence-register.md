...
| `EVID-CS-001` | `MX-CS-001`,`MX-CS-002`,`MX-CS-003`,`MX-CS-005`,`MX-CS-006`,`MX-CS-019`,`MX-CS-024`,`MX-CS-026`,`CF-WKR-027`,`DEC-0007` | integration/protocol | client-core CI + staging attestation bundle plus required local runtime-control regression coverage for shared public-entry wiring | local + CI + staging | per release candidate | discovery/capabilities/session/profile/password-change/deactivate 核心用例与 profile propagation 基线全通过，且 `/.well-known/matrix/client`、`/_matrix/client/versions`、`GET /login`、`GET /register`、`GET /register/available`、`GET /capabilities` 对真实 route truth、disabled login-token、3PID surfaces 与 `m.profile_fields` 的宣告一致；其中 `/versions` 还必须真实证明返回 pinned `v1.17` 与当前 browser-client 兼容所需的 cumulative stable version ladder，至少包括 `r0.6.1` 与 `v1.1` 到 `v1.17`。该证据口径受 `DEC-0007` 约束：它证明的是 browser compatibility discovery，不是对 stub-only surfaces 的隐式 enablement。对 `r0` / `v1` / `v3` compatibility aliases，attestation 还必须逐别名证明 `GET /login` / `GET /register` / `GET /register/available`、`POST /register {}` 初始 `401` UIA challenge、成功 registration、成功 password login、unsupported login type / wrong-password 错误模型、以及 session issuance / `device_id` 副作用 parity，不得因 alias 不同而漂移；同一 bundle 还必须保留 alias discovery surfaces 的 `Cache-Control` 真值证明，其中 `GET /login` 为 `public, max-age=60`，`GET /register` / `GET /register/available` 为 `no-store`，并证明 anonymous public-entry limiter 生效时，针对 `GET /login` / `GET /register` / `GET /register/available` 的 `r0` / `v1` / `v3` alias matrix 进行的 bounded non-local probe 至少观察到一次 live `429 M_LIMIT_EXCEEDED`，且该窗口内 alias discovery GET surfaces 只返回 `200` 或 `429`；依据 `CF-WKR-027`，不得再把“每个 alias individually 都命中 `429`”写成 attestation pass criteria。与此同时，同一 evidence gate 还必须消费 local runtime-control regression coverage，fail-closed 证明 shared public-entry policy wiring 仍被锁在同一 gateway policy family，而不能被这份 non-local attestation 单独声称为已证明。browser-style `Origin`/`OPTIONS` CORS-preflight 行为也必须通过。此外 password-change 至少覆盖带 token 与 tokenless password-UIA 两个分支，profile 至少覆盖 full-read、`keyName` GET/PUT/DELETE、`m.tz`/custom-field 与 propagation 行为；refresh 还必须真实证明 old-refresh retry window 与 post-use invalidation，且同一 evidence gate 继续消费 local runtime-control regression，fail-closed 证明 expired access-token response 在 refresh 仍可恢复时会附带 `soft_logout: true`。`GET /capabilities` 还必须真实证明 `m.room_versions.default = 12` 与 `available.{11,12} = stable`，并保留 browser-origin `GET/OPTIONS` 下 `200/401` 都不会丢失 CORS truth 的证据，否则当前 browser client 会把 create-room 退回到 legacy room-version 默认值或在鉴权失败时被浏览器层拦截。 | `evidence/L1/EVID-CS-001/` | release lifetime | client protocol |
| `EVID-CS-002` | `MX-CS-004`,`MX-CS-006`,`MX-CS-007`,`MX-CS-010`,`MX-CS-015` | protocol | `/sync` conformance attested staging report plus required local runtime-control snapshot-boundary regression | staging + local | per release candidate | filter lifecycle、`include_leave`、lazy-load members、initial/incremental/limited/`full_state`/`use_state_after`、push rules 与 notification counts 全通过，且同一 `since` token 在 inline JSON filter 与 stored `filter_id` 之间切换时不会错误返回 token/filter mismatch；其中还必须证明真实 Web client 形态的“initial inline filter 带 `room.timeline.limit` + `lazy_load_members`，后续 stored filter 只保留 `lazy_load_members`”切换路径可继续增量 `/sync`，并证明同一用户新设备 / 新登录会话的 initial `/sync` 会直接返回已加入房间的 recent visible timeline 与可回翻的 `prev_batch`；`include_leave = true` 的 relogin/new-device leave 房间也必须有对等证明并真实消费 returned `prev_batch` 做 `/messages` backfill。与此同时，companion local runtime-control regression 还必须证明：若在 `collectSince()` 与 `RoomDO.projectForSync()` 之间出现 post-snapshot room event，initial `/sync` 不会把它偷带进同一响应，而下一次 incremental `/sync` 会正常看到它。同一 attested report 还必须保留 browser-origin `/sync` 的 `OPTIONS` preflight、带 `Origin` 的 `401 M_MISSING_TOKEN`、以及 `timeout > 0` held long-poll 被用户流更新唤醒后的 CORS truth，避免只在 `timeout=0` 或非浏览器路径上看起来正常。 | `evidence/L1/EVID-CS-002/` | release lifetime | client protocol |
| `EVID-CS-003` | `MX-CS-013`,`MX-CS-014` | protocol | devices/E2EE transport attested staging report | staging | per release candidate | key claim at-most-once、device CRUD/delete UIA、cross-signing upload/signature handling、`/sync` 的 `device_lists` / `device_one_time_keys_count` / `device_unused_fallback_key_types` 真值、`GET /_matrix/client/*/keys/changes` 对同设备 `from`/`to` sync token 区间的 `changed` / `left` truth（包括共享加密房间 peer change、最后一个共享加密房间离开后的 `left` truth、以及非加密共享房间离开不会产生 `left`），以及 backup metadata/object handling 通过 | `evidence/L1/EVID-CS-003/` | release lifetime | client protocol |
...
| `EVID-ROOM-001` | `MX-CS-008`,`MX-CS-009`,`MX-CS-010` | property/integration | room-core attested CI + staging report | CI + staging | per release candidate | 本地房间行为和 fanout 全通过，且空 `state_key` 的 room-state `/state/{eventType}` 与 `/state/{eventType}/` 变体都被 attested 证明收敛到同一 truth | `evidence/L1/EVID-ROOM-001/` | release lifetime | room core |
...
| `EVID-MEDIA-001` | `MX-CS-011`,`MX-FED-008` | integration/load | media pipeline attested staging + pre-release report | staging + pre-release | per release candidate | `L1` 至少覆盖本地 upload/download/thumbnail、authenticated current media surface，以及 deprecated `/_matrix/media/*/download` / `thumbnail` 的 legacy unauthenticated + freeze 行为；同一 evidence 还必须证明 browser-style `Origin`/`OPTIONS` 对 `/_matrix/client/v1/media/config` 与 deprecated `/_matrix/media/*/config` 的 CORS/preflight truth不会阻断 authenticated config fetch，并锁定关键 CORS 响应头。并且 animated thumbnail 变体不会污染 non-animated cache key；`L2-L3` 另外覆盖 federation media serve 与 remote cache 行为，包括 deprecated unauthenticated routes 在 freeze 之后对 cache miss 不会触发新的远端抓取；所有适用子集都必须通过 | `evidence/L1/EVID-MEDIA-001/` | release lifetime | media |
...
| `EVID-E2E-001` | `MX-CS-001`,`MX-CS-002`,`MX-CS-006`,`MX-CS-007`,`MX-CS-008`,`MX-CS-009`,`MX-CS-010`,`MX-CS-011`,`MX-CS-013`,`MX-CS-014`,`MX-CS-017` | browser/e2e | pinned Element Web + Playwright Chromium attested staging report plus local browser-harness contract regression | staging + local | per release candidate | 同一 attested staging report 必须包含有效 `browser_journey_coverage` sidecar，并真实证明：使用 pinned official Element Web release bundle、测试时生成的 `config.json`、真实 Chromium storage/session 与已部署 staging homeserver 时，canonical journey matrix 的 `coverage_ratio >= 0.80`，且全部 `P0` journeys 为 `pass`。这些 `P0` journeys 至少包括 custom homeserver configure + register/login、session restore / relogin / new-session history、private room/DM 核心协作、文件上传下载、profile propagation、encrypted room send/receive、Secure Backup recovery key 生成、以及新 session device verification + earlier encrypted history decryption。raw bundle 还必须保留 browser journey coverage、Element Web tarball source/digest、以及关键失败或安全旅程的 trace / screenshot / download artifact reference。与此同时，companion local regression 还必须锁定 journey matrix 完整性、coverage threshold 计算与 browser harness bundle-verification contract；若 staging attestation 缺失 sidecar、coverage 低于阈值、或任何 `P0` journey 不是 `pass`，本证据必须 fail-closed。 | `evidence/L1/EVID-E2E-001/` | release lifetime | client protocol + browser usability |
| `EVID-AS-001` | `MX-AS-001`,`MX-AS-002`,`MX-AS-003`,`MX-CS-017` | integration | appservice and derived-data report | staging | per release candidate when enabled | namespace、txn delivery、directory side-effects 通过；当前不把 client `/login` `/register` 上 `m.login.application_service` legacy auth 子集计入本证据闭环，该部分仍受 [`OQ-0008`](/root/Matrix/spec/open-questions/OQ-0008.md) 约束 | `evidence/L3/EVID-AS-001/` | release lifetime | integrations |
...
| `EVID-OPS-003` | `REQ-OPS-003`,`REQ-OPS-010`,`REQ-OPS-011`,`REQ-OPS-012`,`DATA-OPS-014`,`DATA-OPS-015`,`DATA-OPS-016`,`DATA-OPS-017`,`DATA-OPS-018`,`DATA-OPS-019`,`DEC-0005`,`DEC-0006` | production automation | local contract report + same-head prod workflow artifacts | CI + prod | per prod automation change and per same-head proof | `TEST-OPS-003` 对应 local contract tests 全通过，且 same-head workflow 真实证明 reviewed candidate / prod install / prod promote / operational prod refresh / prod rollback / prod-cost raw blocker retention contract 与当前 head 一致；若 prod automation 需要从 earlier failed run 的 mixed-state blocker 继续推进，则 same-head proof 还必须真实显示 typed `ProdCurrentStateSnapshot` 的保留与恢复路径都按 contract fail-closed 运行，而不是口头声称“已可继续”；若 post-mutation refresh 自身也失败，则 same-head proof 还必须显示 canonical `current-production-state.json` 已被 quarantine 或移除，并保留 `ProdCurrentStateDisposition` 明确记录 refreshed / quarantined / removed / refresh_failed 处置结果，而不是继续把 stale snapshot 留在 canonical 路径上。若 remote proof 尚缺，则 evidence 必须把 blocker 诚实写成 fail，而不是口头跳过 | `evidence/common/EVID-OPS-003/` | release lifetime | platform |
...
| --- | --- | --- |
| `L1` | `Local-Core` | `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-E2E-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001` |
