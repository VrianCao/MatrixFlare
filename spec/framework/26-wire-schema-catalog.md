...
| `billing_period` | object | 必须包含 `start`,`end` RFC 3339 UTC，且 `start <= end`；对 production monthly snapshot，它必须对应官方 Cloudflare latest closed billing period，而不是仓库自定的 calendar-month proxy |
