...
| `TEST-CS-001` | discovery, capabilities, registration availability, registration, login discovery/exchange, password change, account deactivation, refresh, logout, profile surfaces and profile propagation basics | CI + staging | `L1-L3` | 必须覆盖 `/.well-known/matrix/client`、`/_matrix/client/versions`、`GET /login` flow advertisement、`GET /register` compatibility flow advertisement、以及 `GET /register/available` truth；其中 `/versions` 不仅要返回 pinned `v1.17`，还必须保留当前 browser-client 兼容所需的 cumulative stable version ladder，至少包括 `r0.6.1` 与 `v1.1` 到 `v1.17`，不得只返回最新 stable version。该断言受 `DEC-0007` 约束：它证明的是 browser compatibility discovery，而不是对 `DEC-0001` stub-only surfaces 的隐式放开。对 `r0` / `v1` / `v3` compatibility aliases，必须逐别名证明 `GET /login` / `GET /register` / `GET /register/available`、`POST /register {}` 的初始 `401` UIA challenge、成功 registration、成功 password login、以及 unsupported login type / wrong-password 错误模型都与同一 password / dummy truth 对齐，不得因 alias 不同而在 challenge 顺序、错误模型、成功写入、session issuance 或 `device_id` 副作用上漂移。alias discovery surfaces 还必须显式断言 `GET /login` 的 `Cache-Control: public, max-age=60`，以及 `GET /register` / `GET /register/available` 的 `Cache-Control: no-store`；并且必须在 anonymous public-entry rate limit 生效时，对 `GET /login` / `GET /register` / `GET /register/available` 的 `r0` / `v1` / `v3` alias matrix 做真实 non-local bounded-window probe：同一 bounded retry window 内至少要观察到一次 live `429 M_LIMIT_EXCEEDED`，同时 alias discovery GET surfaces 在该窗口内只允许返回 `200` 或 `429`，不得漂移成其他状态。依据 `CF-WKR-027`，Workers `ratelimits` 是 local-to-location、eventually consistent、permissive，因此这条 non-local 证明不得再把“每个 alias individually 都命中 `429`”写成 release gate；shared public-entry policy wiring 必须继续由 companion local runtime-control coverage fail-closed 锁定，而不是由这条 non-local `429` window 单独推断。除此之外，还必须覆盖 `/capabilities` 中 `m.change_password` / `m.3pid_changes` / `m.get_login_token` / `m.profile_fields` / `m.room_versions` truth、password change / deactivate 的 UIA 基线，并显式覆盖带 access token 与不带 access token 的 password-UIA 分支，以及 profile full-read、`keyName` GET/PUT/DELETE、`m.tz`/custom-field 语义与 profile change propagation；其中 refresh 还必须显式证明“上一代 refresh token 在新 access/new refresh 尚未被使用前仍可 retry，并收敛到同一 rotated token pair；一旦当前 access 或当前 refresh 已被使用，上一代 refresh token 就立即失效”。若 non-local harness 暂不方便直接注入 access-token expiry，companion local runtime-control coverage 仍必须 fail-closed 证明“access token 已过期且 refresh 仍可恢复”时 authenticated route 返回 `401 M_UNKNOWN_TOKEN + soft_logout: true`。其中 `m.room_versions` 至少必须证明 `default = 12` 与 `available.{11,12} = stable`，否则当前 browser client 会把 create-room 回退到 legacy room-version 默认值。browser-style `Origin` 请求下 discovery / login / register discovery surface，以及 authenticated `/capabilities` 的 `200/401` 响应，都必须保留可读的 CORS headers 与 `OPTIONS` preflight truth。在 [`OQ-0008`](/root/Matrix/spec/open-questions/OQ-0008.md) 关闭前，本项只要求验证 discovery 不误宣告 `m.login.application_service`；其 dedicated legacy-auth semantics 不得被隐式当作已进入当前 gate。 |
| `TEST-CS-002` | filter lifecycle, `/sync` initial/incremental/limited/`full_state`/`use_state_after`, `include_leave`, lazy-load members, account data, push rules, and notification counts | staging | `L1-L3` | 重点验证 token 单调性、filter determinism、leave-room visibility、member lazy-load 与通知计数语义；同一 `since` token 还必须能在 inline JSON filter 与 stored `filter_id` 之间切换而不被错误判成 token mismatch，并显式覆盖真实 Web client 会做的“initial inline filter 带 `room.timeline.limit` + `lazy_load_members`，后续 stored filter 只保留 `lazy_load_members`”切换路径。除此之外，还必须显式验证“同一用户新设备 / 新登录会话的 initial `/sync`”会直接返回已加入房间的 recent visible timeline 与可回翻的 `prev_batch`，而不是只返回 `state/state_after` 快照；对 `include_leave = true` 的 leave 房间也必须有对等的 relogin/new-device proof，并显式消费 returned `prev_batch` 继续 `/messages` backfill。staging attestation 之外，companion local runtime-control coverage 还必须故意在 `collectSince()` 与 `RoomDO.projectForSync()` 之间注入 post-snapshot room event，证明 initial `/sync` 不会泄漏该事件，而下一次 incremental `/sync` 会看到它。对于 browser-style `Origin` 的 authenticated `/sync`，还必须显式覆盖 `OPTIONS` preflight、带 `Origin` 的 `401 M_MISSING_TOKEN`、以及 `timeout > 0` 时 held long-poll 被后续用户流变更唤醒后的 CORS truth，避免 `/sync` 只在 `timeout=0` 或非浏览器路径上看起来正常。 |
| `TEST-CS-003` | devices, to-device, key upload/query/claim/changes, cross-signing, backup metadata | staging | `L1-L3` | 必须验证 one-time key at-most-once、device delete 的 UIA 基线、cross-signing upload/signature handling，以及 `/sync` 中的 `device_lists` / `device_one_time_keys_count` / `device_unused_fallback_key_types` 真值；同时必须验证 `GET /_matrix/client/*/keys/changes` 会对同一设备的 `from`/`to` sync token 区间返回稳定的 `changed` / `left` 集合，覆盖共享加密房间另一用户的 device-list 变化、最后一个共享加密房间离开后的 `left` truth、以及非加密共享房间离开不会产生 `left`，并对缺失、跨设备、倒置或超前当前用户流的 token fail-closed。 |
...
| `TEST-ROOM-001` | room creation, membership, event send, redaction, receipts, typing | CI + staging | `L1-L3` | 房间核心行为；其中空 `state_key` 的 room-state 写入/读取必须同时覆盖 `/state/{eventType}` 与保留尾斜杠的 `/state/{eventType}/`，并证明二者收敛到同一 event/dedupe/query truth。 |
...
| `TEST-OPS-003` | production install/promote/operational-refresh/rollback/cost automation contract and failure-artifact retention | CI + prod | support prerequisite | 必须同时覆盖 reviewed candidate manifest 验证、producer/current-repo 绑定、prod install active-topology guard、migration vs non-migration promote path 选择、`operational-prod-refresh` 受控例外边界、rollback 前 current-state guard，以及 `release-candidate` / `prod-install` / `promote-prod` / `operational-prod-refresh` / `rollback-prod` / `prod-cost-monthly` 在 fail-closed 时仍保留 raw blocker artifacts 的 workflow contract。它不是 `TEST-OPS-001` 或 `TEST-COST-001` 的替代，而是 production automation prerequisite。 |
...
* `TEST-OPS-003` 必须覆盖 production automation contract：reviewed candidate manifest 验证、producer/current-repo 绑定、prod install record 生成、migration vs non-migration promote path 选择、`operational-prod-refresh` 的 `operational_unblock` authority 边界、rollback handle 序列化、上游 workflow/CLI contract 对齐、active-topology / current-state drift 的 fail-closed 行为，以及 `release-candidate` / `prod-install` / `promote-prod` / `operational-prod-refresh` / `rollback-prod` / `prod-cost-monthly` 的 workflow 层 raw blocker artifact 保留。若 earlier failed prod automation 已把 live prod 留在 mixed deployment state，本测试还必须覆盖 typed `ProdCurrentStateSnapshot` 的 capture 与 recovery path：后续 promote/refresh 只能在再次 live revalidate 通过时消费该 snapshot，不能把它当作 drift 豁免；若 post-mutation refresh 自身也失败，则 canonical `current-production-state.json` 也必须被 quarantine 或移除，不能继续留着 stale snapshot，并且必须生成 `ProdCurrentStateDisposition` 明确记录该 canonical path 的最终处置。它属于 production automation prerequisite，不得被误写成 `TEST-OPS-001`、`TEST-COST-001` 或其他 non-local gate 的替代。
