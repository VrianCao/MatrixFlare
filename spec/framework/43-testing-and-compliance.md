...
| `TEST-COST-001` | quota accounting and budget guardrail validation | monthly + pre-release | `L1-L3` | pre-release half 必须对 bounded workload 执行 official Cloudflare metrics/billing query 并产出 attested `PreReleaseCostObservation`；monthly half 继续要求基于官方 billing profile `next_bill_date` 推导 latest closed billing period 的 production `ProdCostSnapshotAttestation`，并证明 `billing_period.start` 严格晚于当前 prod baseline `ProdInstallRecord.installed_at` 对应 UTC 日期；不得把 previous calendar month 当作默认 truthful window。 |
