...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  当前状态:
  `tests/integration/l1-mandatory.test.mjs`、`tests/staging/l1-mandatory.test.mjs`、`tests/pre-release/l1-mandatory.test.mjs` 已改成 dedicated remote harness，直接驱动部署后的 HTTP surface，并通过 `packages/testing/src/evidence.mjs` 的 parser 复核为 environment-backed；`packages/testing/src/nonlocal.mjs` 与 `.github/workflows/nonlocal-phase08.yml` 也已补上 Cloudflare 资源 ensure、环境 deploy、R2 artifact upload 与 GitHub Actions non-local 入口，因此 `08.04` 不再停留在“薄 local shim”阶段。这里最多只能称为“结构就绪”，不代表已经完成成功的 attested non-local run、完整 L1 语义覆盖，或 `08.04` 已闭环。现在 parser 还会对 template-literal dynamic import 等隐藏依赖 fail-closed，non-local deploy/run/upload/attest primitives 也会要求 GitHub-signed Actions OIDC job identity，并按 dedicated environment 全局串行 workflow / fresh deployment identity / 当前 account workers.dev subdomain / 当前 active deployment-version identity 回读复核，避免仅靠本地伪造 `GITHUB_ACTIONS=true`、跨 ref 争用环境、或篡改 deployment JSON 就假装产出 non-local gate 结果。  
  但本项仍未完成：`2026-04-01` 的真实 run `23860170272` 已证明仓库曾经达到 `ci-integration`、`staging`、`pre-release` 三环境成功部署、通过 bounded readiness probe、执行 environment-backed suite、上传 raw bundle 到 R2，并生成有效 attestation；旧的“等待 staging 成功 attestation” blocker 已失效，相关完成声明不得再沿用。可是在同日针对代码状态 `654952d91e24c9699054aa2b4da4c7a97e1d7c38` 触发的最新验证 run `23862913809` 中，`pre-release` 又在 readiness 已通过后失败：`tests/pre-release/l1-mandatory.test.mjs` 的 smoke path `PUT /_matrix/client/v3/rooms/{roomId}/send/m.room.message/pre-release-smoke` 实际返回远端 `500`，导致 `pre_release_run_report.status=fail`、attestation 被 fail-closed 拒绝生成，因而该代码状态连“三环境 attested smoke”都不能诚实视为稳定复现。除此之外，当前 `08.04` 的长期剩余问题仍收敛为两类：其一，`tests/integration/l1-mandatory.test.mjs`、`tests/staging/l1-mandatory.test.mjs`、`tests/pre-release/l1-mandatory.test.mjs` 仍只覆盖 smoke 级 HTTP path，还没有把 `TEST-CS-001`~`TEST-COST-001` 的完整 L1 语义迁入 dedicated non-local suites；其二，`packages/testing/src/evidence.mjs` 对 non-local canonical `TEST-ID -> implementation files` 继续 fail-closed，且现在不仅拒绝 `l1-mandatory.test.mjs`、`bootstrap.test.mjs` 与 `tests/shared/*` 这类 generic/shared proof 路径、拒绝 basename 未以对应 `TEST-ID` 小写形式起始的同目录改名 shim，还要求 canonical file 必须真实存在于 repo、且其 repo-owned transitive dependency closure 也留在同一环境目录内，因此当前仍不能诚实补上映射或宣称全部 mandatory non-local gate 项都已有 dedicated environment-backed proof。换言之，non-local harness 已从“薄 shim”推进到“曾经拿到过三环境 attested smoke、但 latest verified code state 仍存在 pre-release smoke 不稳定 + 长期缺 dedicated TEST-ID suites”的状态，距离 `08.04` 完成仍有明显差距。
