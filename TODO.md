...
- [ ] 通过 `DEC-0005` / `DEC-0006` 把 production automation contract 固化到 Spec/TODO，再落 GitHub Actions + CLI/tooling 的 `prod-install` / `release-candidate` / `promote-prod` / `operational-prod-refresh` / `rollback-prod` / `prod-cost-monthly` 基础设施；same-head remote 验证必须证明执行入口真实可用，且 fail-closed 时仍保留 raw blocker artifact。
  ...
  针对 pre-mutation blocker 的 failure-artifact validator repair 现已随 PR `#16` 合并到 `master` head `825222f41775a006bf06103a0776ff96f855e795`：若 raw blocker 仍保留 canonical `current-production-state.json`、不存在 `pre-failure-current-production-state.json`、且该 canonical snapshot 自身通过 typed `ProdCurrentStateSnapshot` 校验，则 `prod-current-state-failure-validate` 允许把它视为诚实的 pre-mutation blocker；一旦出现 quarantine evidence、canonical snapshot 缺失、或 snapshot 自身无效，则依旧必须显式保留 `ProdCurrentStateDisposition` 并继续 fail-closed。`tests/local/runtime-foundations/nonlocal-tooling.test.mjs` 也已补上对应 regression，避免把 stale/malformed snapshot 偷放行。但修完这条 validator 以后，same-head rerun 并不是“尚未发生”：current merged head `825222f41775a006bf06103a0776ff96f855e795` 已真实拿到 `Operational Prod Refresh` run `24046187190` 与 `Promote Prod` run `24046187161`（均 created `2026-04-06T19:01:26Z`）；两者都在 `Resolve production baseline input` 处 fail-closed，step log 明确报出 `TypeError: legacy prod current state snapshot access must be an object`。也就是说，current merged-head blocker 已从 “缺 repaired-head rerun” 前移成了更精确的 runtime defect：legacy `current-production-state.json` baseline 虽然已被 raw artifact 保留下来，但 current resolver 还不能在它缺少顶层 `access` 时，从 embedded `prod-promotion-record.json` 的 prod topology 元数据诚实恢复 `protected_ops_url`。当前 worktree 已本地修补该点，并新增 install-baseline / promotion-baseline derivation 与 underivable fail-closed regression；但在新的 same-head remote rerun 出来之前，`08.05A` 仍必须保持 reopen。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  同期 standalone `prod-cost-monthly` runs `24003512022`,`24005242689`,`24005804747` 也把 `OQ-0002` 下的 production blocker 进一步前移成了更精确的 then-current `master` truth：revised contract 已真实执行到 `Resolve closed production billing window` 与 prod baseline artifact 下载路径；其中 `24005242689` 证明即使 target account 已拿到新增 billing-view scope，`/billing/profile` 的成功响应仍可能不带 `next_bill_date`，`24005804747` 则进一步证明当时的 `master` head 已能在 same-head remote truth 下退回到唯一的 official subscriptions `current_period_end`，把 `billing-window.json` 解析成 `from_date = 2026-03-01`、`to_date = 2026-03-29`，并在 raw artifact + immutable R2 bundle 保留后，于 `Capture production cost snapshot` 步骤按 fail-closed 语义明确报出 `production cost snapshot requires a closed billing window that starts after prod install date 2026-04-05`。因此 production-side blocker 现阶段已经可以诚实改写成“historical remote billing-cycle closure 已打通，但 current account 仍未到 fixed prod topology 的第一个 truthful post-install closed billing period”；不再需要把它表述成“prod monthly snapshot 仍缺 same-head remote closure”。但 `08.05` 现在依然不能被概括成“只剩 cost blocker”，因为 `08.05A` 仍处于 reopened 状态：current `master` head `825222f41775a006bf06103a0776ff96f855e795` 的 same-head `Operational Prod Refresh` run `24046187190` 与 `Promote Prod` run `24046187161` 虽已真实存在，却都在 `Resolve production baseline input` 处按 fail-closed 语义卡在同一 legacy baseline normalization defect；后者还额外保留了 `Resolve candidate commit sha` 对 `.../downloaded/candidate/prod-release-candidate.json` 的 `ENOENT` raw blocker，但当前 first-order blocker 已先前移到 baseline resolver。本轮 local repair 正是在消除此 merged-head runtime defect，并且只有在新的 same-head remote rerun 真实证明 `release-candidate` / `promote-prod` / `operational-prod-refresh` 已按修补后的 contract 收口之后，`08.05A` 才能诚实转回完成。与此同时，authoritative `L1` release-gate blocker 仍然是 `OQ-0006` + `OQ-0002` 共同挡住的 `EVID-COST-001`，以及 shared pre-release attestation 缺失连带导致的 `EVID-MEDIA-001` / `EVID-DER-001` / `EVID-SEC-001` / `EVID-OPS-001` fail-closed；`TEST-OPS-003` / `EVID-OPS-003` 属于 production automation support prerequisite，不应被误写成 `L1 Local-Core` mandatory gate 本身。也就是说，当前不能诚实声称完成的是 `08.05` / Phase 08 overall，而 `L1` 侧的 blocker set 仍以 cost evidence path 为准。
