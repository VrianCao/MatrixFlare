...
- [ ] 通过 `DEC-0005` / `DEC-0006` 把 production automation contract 固化到 Spec/TODO，再落 GitHub Actions + CLI/tooling 的 `prod-install` / `release-candidate` / `promote-prod` / `operational-prod-refresh` / `rollback-prod` / `prod-cost-monthly` 基础设施；same-head remote 验证必须证明执行入口真实可用，且 fail-closed 时仍保留 raw blocker artifact。
  ...
  `2026-04-06` current `master` 的 production automation truth 还需要同步前推到 head `73cb166f911ea6d192f7d7b8ad5ab3e64fc78775`。same-head `Operational Prod Refresh` run `24042461600` 与 `Promote Prod` run `24042505083` 现都已经真实成功走过 `Resolve production baseline input`；其中 `24042461600` 甚至继续成功完成了 `Refresh production topology operationally`、raw state artifact staging 与 upload，`24042505083` 也继续成功完成了 `Resolve candidate commit sha`、raw state artifact staging 与 upload。换言之，`08.05A` 的 active blocker 已不能再诚实表述成“same-head master 尚未证明 baseline normalization / provenance binding 可闭环”。
  更精确的 current truth 是：`24042461600` 在 raw artifact 保留之后，最终 fail-closed 报出 `Operational prod refresh failed without retaining a usable current-production-state snapshot or disposition artifact after baseline resolution.`；`24042505083` 则在 raw artifact 保留之后，最终 fail-closed 报出 `Resolving candidate commit sha failed after raw state preservation.`。因此，本项当前真正未闭环的部分已经后移为两条更晚的 workflow contract：一是 `operational-prod-refresh` 的 current-state snapshot / disposition retention 语义仍需 same-head 修复，二是 `promote-prod` 在 candidate commit sha 经 raw-artifact 保留后的 fail-closed enforcement path 仍需 same-head 修复；这轮 verified remote truth 还没有证明更后面的 candidate checkout / manifest-match gate。current worktree 现又进一步把 prod current-state snapshot writer 收紧为“runtime 必须显式写入 truthful prod ops ingress，不能再从缺 `access` 的 promotion baseline 产出自相矛盾的 typed snapshot”，同时也把 legacy raw blocker 的 baseline-lineage normalization 收紧为“既要匹配 snapshot 自身的 baseline lineage，也必须由 legacy snapshot 自身真实保留可用的 `protected_ops_url`，selected baseline record 不得回填缺失 ingress 元数据”；但在这组 repair 合并到 `master` 并通过新的 same-head `promote-prod` / `operational-prod-refresh` rerun 之前，`08.05A` 仍必须保持 reopen。
