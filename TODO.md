...
- [x] 通过 `DEC-0005` 把 production automation contract 固化到 Spec/TODO，再落 GitHub Actions + CLI/tooling 的 `prod-install` / `release-candidate` / `promote-prod` / `rollback-prod` / `prod-cost-monthly` 基础设施；same-head remote 验证必须证明执行入口真实可用，且 fail-closed 时仍保留 raw blocker artifact。
  ...
  latest reviewed pushed head `700b8600264b78eacc07733a7fd09248d9c68e8a` 已对 `prod-cost-monthly` / phase08 embedded `prod-cost` contract 做了 same-head rerun 验证：standalone `prod-cost-monthly` run `24003512022` 与 embedded `prod-cost` job in `24003512030` 都先执行到 revised entry contract 中的 `Resolve closed production billing window` 与 `Download prod install baseline record` 路径；其中 billing-window step 在缺失可信 `next_bill_date` 时按 fail-closed 语义写出 blocker，而 baseline download 继续成功。standalone workflow 继续在 fail-closed 前保留并上传 raw blocker bundle，embedded `prod-cost` job 则还额外证明 workflow 能在 gate 前进入 `Write prod-cost provenance` step。随后 same-head rerun `24005242689`（head `949a6c72ff042fbc2698b52432882e6aa1de2ea1`）又进一步证明：即使用户已补齐 billing-view scope，当前 target account 的 `/billing/profile` 响应仍可能成功但不带 `next_bill_date`，所以“只差 token scope”并不成立。current worktree 因而已把 prod-cost billing-window contract 从“只认 profile `next_bill_date`”推进成“优先使用 profile `next_bill_date`，若目标账号缺该字段则退回到唯一的 account subscriptions `current_period_end`”；这条修复当前已完成本地 Spec/tooling closure，但尚未拿到 same-head remote rerun proof。换言之，`08.05A` 当前仍可诚实维持完成，因为 production automation prerequisite 的关键真相已经是：workflow/tooling entry contract 与 raw blocker retention 已 same-head 验证存在，且当前 active gap 已转移到 `08.05` 的成本证据闭环，而不是 automation entry contract 本身。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  `2026-04-05` latest reviewed pushed-head run with substantive same-head evidence artifacts 现已前推到 `24003512030`（head `700b8600264b78eacc07733a7fd09248d9c68e8a`）；同一 head 上更晚的 rerun `24003585072` 在任何 job / artifact 产出前即被取消，不构成新的 evidence truth。这轮 same-head evidence 真相继续维持同一 cost-only blocker surface：`phase08-evidence-l1-20260405T142859Z-24003512030-1` artifact 真实给出 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002` 为 `pass`；`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001` 为 `fail`。其中前四项已不再是 dedicated coverage、Access topology、rollout-skew proof 或 source-id drift 缺口，而只是继续被 shared pre-release attestation 缺失连带 fail-closed；same-head `EVID-COST-001` 输出里的 `source-ids.json` 现已真实包含 `CF-WKR-029` / `CF-WKR-030`，因此 source-id drift 也不再是 active blocker。
  同期 standalone `prod-cost-monthly` run `24003512022` 与 embedded `prod-cost` job in `24003512030` 也把 `OQ-0002` 下的 production blocker 进一步前移成了比 earlier 404 更精确的 current-head truth：revised contract 已真实执行到 `Resolve closed production billing window` 与当前 prod baseline artifact 下载路径；其中 billing-window step 在缺失可信 `next_bill_date` 时按 fail-closed 语义写出 blocker，standalone workflow 继续保留且上传 raw blocker bundle，embedded `prod-cost` job 还额外证明 workflow 能在 gate 前进入 `Write prod-cost provenance` step。后续 same-head rerun `24005242689`（head `949a6c72ff042fbc2698b52432882e6aa1de2ea1`）又进一步证明：即使 target account 已拿到新增 billing-view scope，`/billing/profile` 的成功响应仍可能不带 `next_bill_date`；因此 current worktree 已把 production-side contract 再向前推进成“先尝试官方 billing profile `next_bill_date`，若缺失则退回到唯一的 official subscriptions `current_period_end`，再下载并消费当前 prod install baseline，最后才允许查询 usage API 并生成 self-describing snapshot payload”。这条 fallback 目前只完成了本地 Spec/tooling closure，尚未拿到 same-head remote rerun proof，所以 production blocker 现阶段仍不能改写成“已确认只剩 post-install closed month”；当前更诚实的表述仍是：`OQ-0002` 下 monthly snapshot 继续缺失 same-head remote closure，而即使 fallback 在下一轮 rerun 中成功，也仍必须继续等待 fixed prod topology 的第一个 truthful post-install closed billing period。与此同时，同轮 `pre-release` raw artifact 仍明确记录 `TEST-DER-001`,`TEST-MEDIA-001`,`TEST-OPS-001`,`TEST-SEC-001` 全部 `pass`，但 `TEST-COST-001` 继续按 `OQ-0006` fail-closed，`pre_release_cost_observation` 仍为 `null`，pre-release attestation 被整体跳过。换言之，`08.05` 当前剩余 blocker 仍只剩两条而且都与成本相关：一是 `OQ-0006` 下 official billing-grade pre-release cost proof 仍不存在，继续挡住 shared `pre_release_run_report` attestation；二是 `OQ-0002` 下 production monthly snapshot 仍缺 same-head remote closure，而且最终还必须落到 first truthful post-install closed billing period。只要这两类 blocker 中任一未收口，`EVID-COST-001` 仍会继续失败，而 `EVID-MEDIA-001` / `EVID-DER-001` / `EVID-SEC-001` / `EVID-OPS-001` 也都会继续因为 shared pre-release attestation 缺失而 fail-closed，因此仍不能诚实声称达到 `L1 Local-Core`。
