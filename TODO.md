...
- [ ] 为 `staging` / `pre-release` 建立可审计的 Access-protected management ingress 与 automation contract（account-owned `workers.dev`/Preview URL 或 active-zone custom hostname），作为 `TEST-DER-001` rebuild consistency 和 `TEST-OPS-001` positive non-local gate 的前置条件。
  ...
  当前状态:
  `packages/testing/src/nonlocal.mjs` 当前仍把 non-local `ops-worker` 暴露为 `*.workers.dev` URL，并把 `ACCESS_TEAM_DOMAIN` 设成 `<environment>.cloudflareaccess.invalid` 占位值；GitHub Actions workflow 也还没有 Access application / service token automation。结合 `2026-04-02` 复核的 Cloudflare 官方 Access 文档与新增 pinned snapshots `research/sources/cloudflare-access-applications.md`、`research/sources/cloudflare-workers-dev.md`，仓库 truth 已被回退到更准确状态：self-hosted public Access applications 仍要求 active-zone domain，但 current Workers docs 也明确允许对 account-owned `workers.dev`/Preview URL 直接启用 Access，因此 “必须 active-zone custom hostname 才有合法 Access ingress” 已不再是可审计真相。当前真正未收敛的 blocker 是：仓库仍没有真实启用的 Access ingress、真实 team domain/AUD、或 GitHub Actions 可用的 service-auth/JWT automation contract；因此仓库仍不能诚实声称具备 positive non-local `/_ops` path。在该前置条件收敛前，`TEST-DER-001` rebuild consistency 与 `TEST-OPS-001` 只能继续 fail-closed。
...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  较新的 GitHub Actions 尝试 run `23887828305`（created `2026-04-02T06:45:44Z`，updated `2026-04-02T06:51:20Z`，code state `4add65895612e6ba58d7f20bbaefc68954f30dc1`）进一步证明当前 pushed head 仍未闭环：`ci-integration` 成功完成 deploy、suite、R2 upload、attestation generation，并上传了 `phase08-ci-integration-attestation-20260402T064549Z-23887828305-1` artifact；`staging` 与 `pre-release` 虽也成功执行到 deploy、environment-backed suite、raw bundle upload、R2 upload、attestation generation，但都在 `Enforce environment gate` 失败，并跳过 attestation artifact upload。`gh api repos/VrianCao/MatrixFlare/actions/runs/23887828305/artifacts` 只返回 `ci-integration` attestation artifact，加上 `staging` / `pre-release` raw bundle 与 `phase08-evidence-l1-20260402T064549Z-23887828305-1` evidence artifact，因此该 run 不能被写成“当前 code state 已具备 staging/pre-release attested pass”；它只能说明当前 head 已继续把 non-local suite 跑到 attestation 生成前后，但 `08.04` / `08.05` 仍未收敛。
  ...
  随后的 non-local 尝试已把 `TEST-SEC-001` 的真实远程 blocker 暴露出来：`staging` / `pre-release` 在 run `23882982493` 的 raw bundle 中都失败于 `tests/*/test-sec-001.test.mjs:280` 对 `GET /_matrix/client/v3/publicRooms?limit=1` 的 baseline abuse-guard 断言，其中 `staging` 实际返回 `200`、`pre-release` 实际返回 `503`，而不是期望的 `429`。该结果证明此前 `gateway-worker` 的 coarse guard 仍依赖 isolate-local `Map`，在 Cloudflare non-local 环境下不能稳定形成同一环境内的真实护栏。当前 dirty worktree 已据此把该路径推进为更贴合平台事实的实现：新增官方快照 `research/sources/cloudflare-workers-rate-limit.md`，把 `CF-WKR-027` 回写到 `13` / `15` / `40`，将 `packages/runtime-core/src/abuse-guard.mjs` 改为优先走 Workers `ratelimits` binding；仅在本地/非 release-gate 场景且 binding 缺失时才回退本地 `Map`，而 `ci-integration`、`staging`、`pre-release` 以及其他非本地环境名一旦缺 binding 就直接 fail-closed。`packages/testing/src/nonlocal.mjs` 也会为这三个环境重写独立 rate-limit `namespace_id`，并在 readiness 前与 suite 前通过 Cloudflare 当前脚本 settings 回读复核实际绑定的 `ratelimit_namespaces`；校验现已要求 `gateway-worker` 自身持有完整 namespace 集合，且非 gateway worker 不得冒带这些 binding，从而避免跨环境或跨 worker 污染被 union 掩盖。对应本地 targeted gates `node --test tests/local/runtime-foundations/phase-08-runtime-controls.test.mjs`、`node --test tests/local/runtime-foundations/nonlocal-tooling.test.mjs`、`node --test tests/local/runtime-foundations/testing-harness.test.mjs`、`npm run test:all` 与 `npm run governance:check` 现已重新通过；但在当前代码状态拿到新的 GitHub Actions attested rerun 之前，这一修复仍只能被记为“已本地验证、待 non-local attestation 复核”，不能把 `TEST-SEC-001` / `EVID-SEC-001` 写成已通过。
  但本项仍未完成，且当前 blocker 已收敛为五类：其一，`TEST-CS-002` 的本地/public-path 语义、dedicated staging canonical suite 与 strict mapping 现已真实落位：`packages/runtime-core/src/durable-objects.mjs` 已把 `/sync` timeline limit、`limited/prev_batch`、fanout notification event 与 push-rule notification/highlight truth 贯通到 `RoomDO.projectForSync()` / `UserDO.appendRoomFanout()`，`tests/local/client-identity/phase-05.test.mjs` 已补上对应 targeted coverage，`tests/staging/test-cs-002.test.mjs` 也已通过 `packages/testing/src/evidence.mjs` 的 strict parser 与 `node --test tests/local/runtime-foundations/testing-harness.test.mjs` 复核。最新本地 fail-closed `npm run evidence:l1 -- --timestamp 20260402T103957Z` 中，`EVID-CS-002` 的 `mapping_error` 已继续保持清零并转为 `Missing valid staging attestation for EVID-CS-002`，说明当前 blocker 已不再是 canonical suite/mapping 缺失，而是当前 dirty worktree 还没有同轮 GitHub Actions staging attestation。其二，`TEST-DER-001` 不仅仍缺 staging/pre-release dedicated canonical suites，也继续受新的前置条件 `08.04B` / `OQ-0004` 约束：Cloudflare 官方现在允许对 account-owned `workers.dev`/Preview URL 启用 Access，但当前仓库仍没有真实启用的 Access ingress、team domain/AUD 或 service-auth automation contract，因此 rebuild consistency 的 positive `/_ops` path 还无法 honest remote 化。其三，`TEST-OPS-001` 除缺 pre-release dedicated skew orchestration 外，也同样先被 `08.04B` / `OQ-0004` 卡住；现有 workflow 只有单 deployment smoke + provenance，且没有真实 Access ingress，因此不能替代 new Worker -> old DO / old Worker -> new DO compatibility gate。其四，`TEST-COST-001` 仍缺 pre-release cost/guardrail suite，并继续被真实 production monthly snapshot 缺失卡住，受 `spec/open-questions/OQ-0002.md` 约束。其五，即使 `TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-SEC-001` 当前都已具备 dedicated suites 与 strict mapping，当前 dirty worktree 也还没有对应的 GitHub Actions attested rerun；因此这些项仍不能被写成“当前代码状态已有 attested pass”。换言之，`08.04` 已从“attested smoke”推进到“更大范围的 dedicated canonical non-local coverage + strict mapping”，并新增了对 ops Access topology 这一前置条件的诚实登记，但距离完成仍有明确差距。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  较新的 GitHub Actions 尝试 run `23887828305`（head `4add65895612e6ba58d7f20bbaefc68954f30dc1`）说明 pushed head 的 non-local path 还没有闭环：`ci-integration` 拿到了 attestation artifact，但 `staging` / `pre-release` 都只产出了 raw artifact，并在 `Enforce environment gate` 后终止；因此 evidence job 虽执行并上传了 `phase08-evidence-l1-20260402T064549Z-23887828305-1`，却依旧只能诚实失败。也就是说，`23880866545` 仍只是“三环境 attestation 全拿齐并进入 evidence-l1”的基线，而不是当前 pushed head 的通过证明。
  在补齐 `research/sources/cloudflare-workers-dev.md`、修正 `CF-NET-007` 与 `spec/open-questions/OQ-0004.md` 里的 active-zone-only 过时前提之后，本轮重新执行了 `npm run governance:check`、`node --test tests/local/client-identity/phase-05.test.mjs`、`node --test tests/local/runtime-foundations/testing-harness.test.mjs`、`npm run test:all`、`date -u +%Y%m%dT%H%M%SZ`（得到 `20260402T103957Z`）以及 fail-closed `npm run evidence:l1 -- --timestamp 20260402T103957Z`。结果再次证明 evidence gate 仍保持诚实：治理检查保持 `PASS`，`TEST-CS-002` 相关 targeted gate 12/12 通过，`testing-harness` 183/183 通过，本地 regression matrix 364/364 通过，而 `evidence:l1` 仍按预期只让 `EVID-GOV-001` 为 `pass`。
  在这轮最新本地 evidence 中，`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-SEC-001` 的 non-local coverage 都已表现为“canonical file 已存在，但缺同轮 attestation”；其中 `EVID-CS-002` 已不再带 `mapping_error`，而是诚实转为 `Missing valid staging attestation for EVID-CS-002`，`manual-artifacts.json` 也同步记录 `environment_harness_ready: true`。这说明当前 `/sync limited`、push-rule notification 与 unread counter 真值已经落到当前代码与 staging canonical suite/mapping 上，但当前 dirty worktree 仍缺同轮 GitHub Actions attested rerun。`EVID-DER-001` 仍明确暴露 `TEST-DER-001` 在 `staging` / `pre-release` 的 `mapping_error`；`EVID-OPS-001` 与 `EVID-COST-001` 也仍明确暴露 `pre-release` canonical mapping 缺失，其中 `EVID-COST-001` 还额外缺 `prod_cost_snapshot`。与此同时，`EVID-DER-001` 与 `EVID-OPS-001` 继续同时受两层真实 blocker 约束：一是 dedicated canonical coverage 尚未落地，二是 `08.04B` / `OQ-0004` 已改写为更准确的前置条件语义后，仓库依旧缺真实启用的 Access ingress、team domain/AUD 与 service-auth automation contract；Cloudflare 官方虽然允许对 account-owned `workers.dev`/Preview URL 启用 Access，但当前 repo 仍不能把公开 `workers.dev` URL + placeholder team domain 写成 positive `/_ops` proof。`EVID-COST-001` 仍同时受两层真实 blocker 约束：一是缺 `TEST-COST-001` 的 pre-release canonical non-local coverage，二是缺真实 `prod-cost-attestation`；后一项继续受 `spec/open-questions/OQ-0002.md` 约束。因此，`08.05` 依旧保持未完成，且当前代码状态仍不能诚实声称达到 `L1 Local-Core`。
