...
- [ ] 通过 `DEC-0005` / `DEC-0006` 把 production automation contract 固化到 Spec/TODO，再落 GitHub Actions + CLI/tooling 的 `prod-install` / `release-candidate` / `promote-prod` / `operational-prod-refresh` / `rollback-prod` / `prod-cost-monthly` 基础设施；same-head remote 验证必须证明执行入口真实可用，且 fail-closed 时仍保留 raw blocker artifact。
  ...
  `2026-04-06` 另一个必须并存记录的现实是：`operational-prod-refresh` 虽然已经作为同一 prerequisite 的一部分被 earlier same-head runs 真实证明存在并可执行，但 latest live prod/browser truth 仍显示当前 prod 停在旧 gateway head，而 current `master` 上两次 rerun `24023572886`（head `b29ab1f3381d17cbff12997f4ab5b04e9a9414c9`）与 `24024832790`（head `2c5101b4296bec9cadcffabc54298fcd0ac2523b`）都在 baseline/current production identity drift 上 fail-closed。当前 hotfix worktree 正在修的是这些 later drift failures 的 raw blocker retention 精度；在新的 same-head rerun 重新证明 tightened blocker-artifact contract 之前，`08.05A` 必须诚实保持 reopen，而不能继续保留 `[x]` 或口头冒充“latest master 已重新闭环”。当前重新关闭本项所需的最小真实条件是：current `master` 上的 `operational-prod-refresh` rerun 至少一次 fail-closed 或成功地保留/产出 tightened contract 所要求的 raw blocker artifact，其中必须能真实看到 `current-production-state.json`（若继续 drift fail）或新的 production record（若刷新成功）。
