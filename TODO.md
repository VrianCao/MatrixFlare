...
- [ ] 通过 `DEC-0005` / `DEC-0006` 把 production automation contract 固化到 Spec/TODO，再落 GitHub Actions + CLI/tooling 的 `prod-install` / `release-candidate` / `promote-prod` / `operational-prod-refresh` / `rollback-prod` / `prod-cost-monthly` 基础设施；same-head remote 验证必须证明执行入口真实可用，且 fail-closed 时仍保留 raw blocker artifact。
  ...
  `2026-04-06` 另一个必须并存记录的现实是：`operational-prod-refresh` 虽然已经作为同一 prerequisite 的一部分被 earlier same-head runs 真实证明存在并可执行，但 latest live prod/browser truth 仍显示当前 prod 停在旧 gateway head，而 current `master` 上 reruns `24023572886`（head `b29ab1f3381d17cbff12997f4ab5b04e9a9414c9`）与 `24024832790`（head `2c5101b4296bec9cadcffabc54298fcd0ac2523b`）都在 baseline/current production identity drift 上 fail-closed。更近一轮 `master` run `24033551396`（head `3eacdba6c0e7b2ec107c479911e55c96751764c7`）已经把旧版 raw-blocker retention contract 的最低要求推进成 remote truth：job `operational-prod-refresh` 在 fail-closed 结束前真实上传了 raw artifact `prod-operational-refresh-raw-state-20260406T132521Z-24033551396-1`，其中确实包含 `state/operational-refresh/current-production-state.json` 与 `downloaded/baseline/prod-promotion-record.json`。因此，本项 current blocker 已不能再表述成“master 还没有 same-head rerun 证明会保留 current-production-state.json”；更精确的 truth 是：live prod drift 仍未修复，而当前 hotfix worktree 还在继续同时收紧 `promote-prod` 与 `operational-prod-refresh` 的 blocker-artifact honesty，避免 post-mutation refresh 二次失败时把 pre-failure snapshot 误当成 live current state，也避免 record baseline 只按 artifact 名被误认成选中的 upstream run。基于这个更严格的 contract，`08.05A` 仍必须保持 reopen，直到 current branch 合并后的 `master` 再次以 same-head remote run 真实证明：对当前改动覆盖到的 `promote-prod` / `operational-prod-refresh` 两条路径，要么 drift 继续 fail-closed 且 raw artifact 真实保留 refreshed / quarantined / explicitly-removed stale current-state 处置证据，要么新的 production record 被成功产出。
