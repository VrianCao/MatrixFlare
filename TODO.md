...
- [ ] 通过 `DEC-0005` / `DEC-0006` 把 production automation contract 固化到 Spec/TODO，再落 GitHub Actions + CLI/tooling 的 `prod-install` / `release-candidate` / `promote-prod` / `operational-prod-refresh` / `rollback-prod` / `prod-cost-monthly` 基础设施；same-head remote 验证必须证明执行入口真实可用，且 fail-closed 时仍保留 raw blocker artifact。
  ...
  latest reviewed pushed head 现已前推到 `a706c04127a5cd7978b850061c81b3c646220a4e`：same-head standalone `prod-cost-monthly` run `24005804747` 已真实证明当时 `master` 上新增的 subscriptions fallback 不只是本地 closure，而是 historical remote truth，而不是当前分支已拿到的 same-head proof。该 run 中 `Resolve closed production billing window` 已从 billing profile 缺字段的情况退回到唯一的 account subscriptions `current_period_end`，并把 `billing-window.json` 真实写成 `resolution_method = cloudflare-account-subscriptions-current-period-end`、`next_bill_date = 2026-04-29T00:00:00Z`、`from_date = 2026-03-01`、`to_date = 2026-03-29`；同时 `Resolve latest successful prod-install baseline run` / `Download prod install baseline record` 继续成功，raw artifact 与 immutable R2 bundle 也都已保留。随后 `Capture production cost snapshot` 在同一 run 中继续按 fail-closed 语义停止，明确报出 `production cost snapshot requires a closed billing window that starts after prod install date 2026-04-05`。这说明 production-side blocker 已不再是“缺可信 billing-cycle anchor”，而是“当前账号还没到 fixed prod topology 的第一个 truthful post-install closed billing period”；但它不再足以单独支撑 `08.05A` 继续维持完成态，因为本项当前还被新的 prod blocker-artifact repair reopen。
  `2026-04-06` current `master` 的 production automation truth 还需要同步前推到 head `73cb166f911ea6d192f7d7b8ad5ab3e64fc78775`。same-head `Operational Prod Refresh` run `24042461600` 与 `Promote Prod` run `24042505083` 现都已经真实成功走过 `Resolve production baseline input`，而 raw state artifact staging / upload 也都已发生。这说明 `08.05A` 的 active blocker 已不能再诚实表述成“same-head master 尚未证明 baseline normalization / provenance binding 可闭环”。
  但必须回退一个过度表述：这两轮 run 里带 `continue-on-error` 的内部步骤并不等于已经“成功完成”更晚 contract。`24042461600` 的 final guard 真实仍读到 `steps.operational_refresh.outcome = failure`，并在 raw artifact 保留后 fail-closed 报出 `Operational prod refresh failed without retaining a usable current-production-state snapshot or disposition artifact after baseline resolution.`；`24042505083` 的 final guard 也真实仍读到 `steps.candidate.outcome = failure`，并在 raw artifact 保留后 fail-closed 报出 `Resolving candidate commit sha failed after raw state preservation.`。因此，current `73cb166f911ea6d192f7d7b8ad5ab3e64fc78775` 的 same-head remote truth 只能证明 baseline-input closure 与 raw-artifact retention boundary 已前推到更晚阶段，不能把 `Refresh production topology operationally` 或 `Resolve candidate commit sha` 写成已诚实收口。
  这组 repair 合并后的 `master` head `4b7ac076f53f6e2a1ec16a610453b627d0da6d82` 又把其中一条 blocker 往前推进了一步，但还没有完全关闭：same-head `Operational Prod Refresh` run `24045788582` 已经真实保留了 typed `state/operational-refresh/current-production-state.json`，而且本地下载该 raw artifact 后可见 `ProdCurrentStateSnapshot.access.protected_ops_url = https://matrix-ops-worker-prod.yevdndjyebksug.workers.dev`，说明 earlier “连 usable current-state snapshot 都没留下”的 blocker 已被 merged-head remote truth 纠正；然而同一 raw artifact 仍未携带 `current-production-state-disposition.json`，现行 `prod-current-state-failure-validate` 也因此在 workflow final guard 中继续 fail-closed。复核 [42-deployment-migration-and-recovery.md](/root/Matrix/spec/framework/42-deployment-migration-and-recovery.md) 当前条文可再确认：`baseline/current Cloudflare production identity mismatch` 这类 pre-mutation blocker 至少必须留下 typed `ProdCurrentStateSnapshot`，而 `ProdCurrentStateDisposition` 是 stale canonical snapshot 处置 sidecar，重点约束的是 post-mutation refresh / quarantine / removal path，而不是所有 pre-mutation blocker 都必须无条件附带。因此，`24045788582` 暴露出的 current mismatch 已不再是 snapshot ingress 缺失，而是 failure-artifact validator 对 pre-mutation baseline-drift blocker 过度要求 disposition sidecar。
  current worktree 现已据此把这条 contract 修正为更贴合 Spec 的 fail-closed语义：若 raw blocker 仍保留 canonical `current-production-state.json`、不存在 `pre-failure-current-production-state.json`、且该 canonical snapshot 自身通过 typed `ProdCurrentStateSnapshot` 校验，则 `prod-current-state-failure-validate` 允许把它视为诚实的 pre-mutation blocker；一旦出现 quarantine evidence、canonical snapshot 缺失、或 snapshot 自身无效，则依旧必须显式保留 `ProdCurrentStateDisposition` 并继续 fail-closed。`tests/local/runtime-foundations/nonlocal-tooling.test.mjs` 也已补上对应 regression，避免把 stale/malformed snapshot 偷放行。但在这组 validator repair 合并到 `master` 并通过新的 same-head `Operational Prod Refresh` rerun 之前，`08.05A` 仍必须保持 reopen；而且 `promote-prod` 的 same-head rerun 也还未完成，candidate checkout / manifest-match gate 仍未被 repaired head 真实覆盖。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  同期 standalone `prod-cost-monthly` runs `24003512022`,`24005242689`,`24005804747` 也把 `OQ-0002` 下的 production blocker 进一步前移成了更精确的 then-current `master` truth，而不是当前分支 `2b9d74f650c3d9c694f662cd307a526c98cedf80` 已拿到的 same-head proof：revised contract 已真实执行到 `Resolve closed production billing window` 与 prod baseline artifact 下载路径；其中 `24005242689` 证明即使 target account 已拿到新增 billing-view scope，`/billing/profile` 的成功响应仍可能不带 `next_bill_date`，`24005804747` 则进一步证明当时的 `master` head 已能在 same-head remote truth 下退回到唯一的 official subscriptions `current_period_end`，把 `billing-window.json` 解析成 `from_date = 2026-03-01`、`to_date = 2026-03-29`，并在 raw artifact + immutable R2 bundle 保留后，于 `Capture production cost snapshot` 步骤按 fail-closed 语义明确报出 `production cost snapshot requires a closed billing window that starts after prod install date 2026-04-05`。因此 production-side blocker 现阶段已经可以诚实改写成“historical remote billing-cycle closure 已打通，但 current account 仍未到 fixed prod topology 的第一个 truthful post-install closed billing period”；不再需要把它表述成“prod monthly snapshot 仍缺 same-head remote closure”。但 `08.05` 现在依然不能被概括成“只剩 cost blocker”，因为 `08.05A` 的 reopen 形态已经从 earlier baseline/provenance 争议前移成了 current `master` head `73cb166f911ea6d192f7d7b8ad5ab3e64fc78775` 上更晚的 workflow blocker：same-head `Operational Prod Refresh` run `24042461600` 与 `Promote Prod` run `24042505083` 都已成功走过 `Resolve production baseline input`，却仍分别在 raw artifact 保留后的 current-state disposition contract 与 candidate commit sha fail-closed enforcement contract 上停止；该 same-head remote truth 还没有证明更后面的 candidate checkout / manifest-match gate。也就是说，除了 cost-only 的 release-evidence blocker 仍然存在外，production automation prerequisite 还额外欠一条新的 honesty closure：必须由覆盖这组 repair 的 same-head `promote-prod` / `operational-prod-refresh` rerun，真实证明上述更晚阶段的 fail-closed contract 已收口，或成功产出新的 production record。在这条 prerequisite 与 `OQ-0006` / `OQ-0002` 中任一未收口前，`EVID-COST-001` 仍会继续失败，而 `EVID-MEDIA-001` / `EVID-DER-001` / `EVID-SEC-001` / `EVID-OPS-001` 也都会继续因为 shared pre-release attestation 缺失而 fail-closed，因此仍不能诚实声称达到 `L1 Local-Core`。
