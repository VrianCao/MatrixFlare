...
- [ ] 为 `staging` / `pre-release` 建立可审计的 Access-protected management ingress 与 automation contract（account-owned `workers.dev`/Preview URL 或 active-zone custom hostname），作为 `TEST-DER-001` rebuild consistency 和 `TEST-OPS-001` positive non-local gate 的前置条件。
  ...
  截至本轮修复提交前的 pushed head `e12fc2bfbd73a5edf68fd947fe434ab82219edfe`，最新真实 non-local rerun 是 GitHub Actions run `23916407173`。该 run 不是“Access automation 没接上”，而是在 `staging` / `pre-release` 的 `ops_rebuild_start` live positive path 上失败：两环境都先通过了 Access-protected `/_ops/v1/healthz`，但随后对 `POST /_ops/v1/rebuilds` 的正向请求返回了 Cloudflare HTML `500`，说明请求已经穿过 Access ingress，却未能在该代码状态上维持 `ops-worker` typed error / success contract。基于该 run 的 raw bundle，本轮未提交修复 worktree 已补上两类 repo-side fail-closed 修复：其一，`packages/control-plane/src/ops-handler.mjs` 现在会 `await startControlPlaneJob(...)`，避免 async rejection 逃逸成平台默认 HTML `500`；其二，`packages/control-plane/src/services.mjs` 现在会在 job start failure、failed-state persistence/dedupe finalization 局部写入失败，以及同一 `Idempotency-Key` replay 命中 stale accepted envelope、或命中 `failed` job truth 但已丢失 `last_error` payload 时保持 deterministic JSON error / stored failure replay，而不是把失败重放成错误的 `202 accepted`。对应回归已由 `tests/local/control-plane/control-plane.test.mjs` 锁定，并通过 `node --test tests/local/control-plane/control-plane.test.mjs`、`node --test tests/local/runtime-foundations/testing-harness.test.mjs`、`npm run test:all` 与 fail-closed `npm run evidence:l1 -- --timestamp 20260402T193746Z` 重新验证；但在新的 current-head GitHub Actions attested rerun 证明 `/_ops/v1/rebuilds` 已经不再暴露 HTML `500` 之前，`08.04B` 仍只能被诚实表述为“repo-side repair 已落地，待 live Access path 重跑证明”。
...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  pushed-head 事实还必须再加上一层本轮 live failure 收敛：run `23916407173`（code state `e12fc2bfbd73a5edf68fd947fe434ab82219edfe`）已经证明 `staging` / `pre-release` 的 `/_ops` Access healthz 正向入口可达，但 `TEST-DER-001` 所依赖的 `ops_rebuild_start` 仍在真实 Cloudflare 环境中打到了 HTML `500`，因此 `08.04B` 和 `EVID-DER-001` 目前不能被写成“只差 attestation artifact 下载”或“Access 前置条件已经闭环”。本轮未提交修复 worktree 通过前述 `ops-handler` / `services` fail-closed 修复，把这次 live 失败进一步收敛成一个待 rerun 验证的 repo-side bugfix，而不是继续保留“可能是平台、也可能是 harness、也可能是 typed error 泄漏”的模糊状态；但在新的 current-head attested rerun 出来之前，`08.04` 仍未完成，`L1` 也仍不能宣称闭环。
