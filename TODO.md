...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  当前状态:
  `tests/integration/l1-mandatory.test.mjs`、`tests/staging/l1-mandatory.test.mjs`、`tests/pre-release/l1-mandatory.test.mjs` 已改成 dedicated remote harness，直接驱动部署后的 HTTP surface，并通过 `packages/testing/src/evidence.mjs` 的 parser 复核为 environment-backed；`packages/testing/src/nonlocal.mjs` 与 `.github/workflows/nonlocal-phase08.yml` 也已补上 Cloudflare 资源 ensure、环境 deploy、R2 artifact upload 与 GitHub Actions non-local 入口，因此 `08.04` 已不再停留在“薄 local shim”阶段。最新验证 run `23869349481`（code state `ef3665d77a98b6cc7398d3a6fa5df51a44dbc4cb`）已再次证明 `ci-integration`、`staging`、`pre-release` 三环境都能成功部署、通过 bounded readiness probe、执行 environment-backed suite、上传 raw bundle 到 R2，并分别生成有效 `EnvironmentRunAttestation`；旧的“等待 staging/pre-release 成功 attestation” blocker，以及把 `23862913809` 那次 `pre-release` 失败继续写成 latest verified code state blocker 的做法，都已失效，必须回退。与此同时，parser 仍会对 template-literal dynamic import 等隐藏依赖 fail-closed，non-local deploy/run/upload/attest primitives 也会要求 GitHub-signed Actions OIDC job identity，并按 dedicated environment 全局串行 workflow / fresh deployment identity / 当前 account workers.dev subdomain / 当前 active deployment-version identity 回读复核，避免仅靠本地伪造 `GITHUB_ACTIONS=true`、跨 ref 争用环境、或篡改 deployment JSON 就假装产出 non-local gate 结果。  
  本轮已把一批 dedicated canonical non-local suites 真正落到对应环境目录，而不是 wrapper / rename / shared shim：`tests/integration/test-cs-001.test.mjs`,`tests/integration/test-cs-004.test.mjs`,`tests/integration/test-room-001.test.mjs`,`tests/integration/test-room-002.test.mjs`,`tests/staging/test-cs-001.test.mjs`,`tests/staging/test-cs-003.test.mjs`,`tests/staging/test-cs-004.test.mjs`,`tests/staging/test-media-001.test.mjs`,`tests/staging/test-room-001.test.mjs`,`tests/staging/test-room-002.test.mjs`,`tests/pre-release/test-media-001.test.mjs`，并分别在 `tests/integration/support.mjs`,`tests/staging/support.mjs`,`tests/pre-release/support.mjs` 中保留同目录 dependency closure。`packages/testing/src/evidence.mjs` 现已为 `TEST-CS-001`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001` 接上 strict non-local mapping；本地重新执行 fail-closed `evidence:l1` 时，对应 `EVID-CS-001`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001` 的 `mapping_error` 均为 `null`，说明这些项已不再停留在“只有 generic smoke 入口”的状态。
  但本项仍未完成，且当前 blocker 现已收敛为六类：其一，`TEST-CS-002` 不再是“缺 staging dedicated file name”的问题，而是远程真实语义尚未落位：`packages/runtime-core/src/durable-objects.mjs` 的 `UserDO.collectSince()` 仍固定返回 `limited: false`，`RoomDO.buildCommittedFanoutDeltas()` 仍固定写入 `notification_count: 0` / `highlight_count: 0`，而 push-rule / unread / notification 真值也尚未贯通到 public non-local 路径，因此不能诚实补出 `TEST-CS-002` 的 staging canonical suite / mapping。其二，`TEST-CS-001`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001` 虽然已有 dedicated suites 与 strict mapping，但当前分支代码状态还没有对应的 GitHub Actions attested run；在新的 non-local run 证明这些 suites 真实通过之前，它们只能算“已接线的 canonical coverage”，不能被写成 Phase 08 已完成。其三，`TEST-DER-001` 仍缺 staging/pre-release dedicated canonical suites，也还没有把 search / user_directory / publicRooms rebuild consistency 的 remote observable path 迁入当前 harness。其四，`TEST-SEC-001` 仍缺 staging/pre-release dedicated canonical suites；token revocation、UIA route binding、secret boundary、always-on baseline abuse guard 与适用的 federation auth failure 仍没有诚实进入 non-local canonical proof。其五，`TEST-OPS-001` 仍缺 pre-release dedicated skew orchestration；现有 workflow 只有单 deployment smoke + provenance，不能替代 new Worker -> old DO / old Worker -> new DO compatibility gate。其六，`TEST-COST-001` 仍缺 pre-release cost/guardrail suite，并继续被真实 production monthly snapshot 缺失卡住，受 `spec/open-questions/OQ-0002.md` 约束。换言之，`08.04` 已从“只有 attested smoke”推进到“部分 L1 surfaces 已有 dedicated non-local canonical suites + strict mapping，剩余项则暴露为真实 runtime/platform gap”，但距离诚实完成仍有明显差距。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  目前仓库已补上 `.github/workflows/nonlocal-phase08.yml` 与对应 non-local tooling，可在 GitHub Actions 中部署 `ci-integration` / `staging` / `pre-release`、上传 raw bundle 到 R2、并按 `DEC-0002` 生成 provenance-ready attestation；workflow 也会在 evidence 阶段消费 `prod-cost-attestation`（若存在），否则继续对 `EVID-COST-001` fail-closed。最新验证 run `23869349481`（timestamp `20260401T202940Z`）已成功下载三份环境 attestation，并在 `evidence-l1` job 中生成新的 evidence bundles；旧的 `_test-runs` 原始 report 型 summary 已被 supersede，不再可作为 release-gate 依据。该 run 的三环境 non-local job 全部成功，`evidence-l1` 则在下载 attestation 并生成 bundle 后继续按预期 fail-closed：`EVID-GOV-001` 是唯一 `pass`，其余 `EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001` 全部为 `fail`。这轮失败说明的是 dedicated non-local canonical coverage 与 production cost evidence 仍未闭合，而不是 attestation/provenance contract 缺失；其中旧的“`EVID-CS-003` 仍缺 staging canonical mapping”与“环境目录里仍只有 generic smoke 入口”叙述已经失效，必须回退。
  本轮本地再次执行 fail-closed `evidence:l1` 时，虽然没有注入任何 non-local attestation，因此整体结果仍然必须失败，但仓库 truth 已发生可审计变化：`EVID-CS-001`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001` 的 `mapping_error` 现均为 `null`，当前剩余失败原因已经收敛为“缺同轮 non-local attestation”，而不再是“canonical file / mapping 缺失”。与此同时，`EVID-CS-002` 继续保持 `mapping_error`，且该缺口是刻意 fail-closed 的真实反映：由于 `TEST-CS-002` 对应的 `/sync limited`、push-rule notification、unread counter 语义尚未贯通到远程 public path，当前不能诚实补 mapping；`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001` 也仍继续因 dedicated non-local suites / mapping 缺失而 fail。`EVID-COST-001` 则同时受两层真实 blocker 约束：一是缺 `TEST-COST-001` 的 pre-release canonical non-local coverage，二是缺真实 `prod-cost-attestation`；后一项仍受 `spec/open-questions/OQ-0002.md` 约束。
  因此，`08.05` 当前已从“attestation contract 未定义”推进到“部分 `EVID-*` 已清除 mapping_error、等待最新 GitHub Actions attested run 消费”，但离诚实声称 `L1 Local-Core` 仍有明确距离：在当前分支重新跑出 attestation-backed `EVID-CS-001`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001` 之前，这些项都不能被写成 `pass`；`EVID-CS-002`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001` 则继续受 runtime gap、missing canonical coverage 或 `OQ-0002` 的真实 blocker 约束。本项保持未完成。
