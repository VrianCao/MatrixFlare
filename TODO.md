...
- [x] 通过 `DEC-0005` / `DEC-0006` 把 production automation contract 固化到 Spec/TODO，再落 GitHub Actions + CLI/tooling 的 `prod-install` / `release-candidate` / `promote-prod` / `operational-prod-refresh` / `rollback-prod` / `prod-cost-monthly` 基础设施；same-head remote 验证必须证明执行入口真实可用，且 fail-closed 时仍保留 raw blocker artifact。
  ...
  当前状态:
  截至 pushed head `ec1b647e55b7fbb571145cc4a3c5acd43beb99b6`，这项前置条件的上一版 contract 确实已经被 same-head remote truth 收口，`OQ-0007` 也已关闭。repository default branch `master` 现已真实包含 `.github/workflows/prod-install.yml`、`.github/workflows/release-candidate.yml`、`.github/workflows/promote-prod.yml`、`.github/workflows/operational-prod-refresh.yml`、`.github/workflows/rollback-prod.yml`、`.github/workflows/prod-cost-monthly.yml`，而对应 production automation 路径也已真实拿到 same-head remote runs：`prod-install` success `24000789563`、`release-candidate` fail-closed `24001043537`、`promote-prod` fail-closed `24001065315`、`rollback-prod` fail-closed `24001077197`、`prod-cost-monthly` fail-closed `24000831481`，以及 `operational-prod-refresh` success `24012543786`,`24014469260`,`24019281851`。
  ...
  `2026-04-06` 另一个必须并存记录的现实是：`operational-prod-refresh` 虽然已经作为同一 prerequisite 的一部分被 earlier same-head runs 真实证明存在并可执行，但 latest live prod/browser truth 仍显示当前 prod 停在旧 gateway head，而 current `master` 上两次 rerun `24023572886`（head `b29ab1f3381d17cbff12997f4ab5b04e9a9414c9`）与 `24024832790`（head `2c5101b4296bec9cadcffabc54298fcd0ac2523b`）都在 baseline/current production identity drift 上 fail-closed。当前 hotfix worktree 正在修的是这些 later drift failures 的 raw blocker retention 精度，而不是否认 `08.05A` 作为 automation prerequisite 曾经达成；在新的 same-head rerun 重新证明 tightened blocker-artifact contract 之前，这条 repair 只能诚实记为 current hotfix，而不能口头冒充“latest master 已重新闭环”。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  同期 standalone `prod-cost-monthly` runs `24003512022`,`24005242689`,`24005804747` 也把 `OQ-0002` 下的 production blocker 进一步前移成了更精确的 current-head truth：revised contract 已真实执行到 `Resolve closed production billing window` 与当前 prod baseline artifact 下载路径；其中 `24005242689` 证明即使 target account 已拿到新增 billing-view scope，`/billing/profile` 的成功响应仍可能不带 `next_bill_date`，`24005804747` 则进一步证明 current head 已能在 same-head remote truth 下退回到唯一的 official subscriptions `current_period_end`，把 `billing-window.json` 解析成 `from_date = 2026-03-01`、`to_date = 2026-03-29`，并在 raw artifact + immutable R2 bundle 保留后，于 `Capture production cost snapshot` 步骤按 fail-closed 语义明确报出 `production cost snapshot requires a closed billing window that starts after prod install date 2026-04-05`。因此 production-side blocker 现阶段已经可以诚实改写成“same-head remote billing-cycle closure 已打通，但 current account 仍未到 fixed prod topology 的第一个 truthful post-install closed billing period”；不再需要把它表述成“prod monthly snapshot 仍缺 same-head remote closure”。与此同时，同轮 `pre-release` raw artifact 仍明确记录 `TEST-DER-001`,`TEST-MEDIA-001`,`TEST-OPS-001`,`TEST-SEC-001` 全部 `pass`，但 `TEST-COST-001` 继续按 `OQ-0006` fail-closed，`pre_release_cost_observation` 仍为 `null`，pre-release attestation 被整体跳过。换言之，`08.05` 当前剩余 blocker 仍只剩两条而且都与成本相关：一是 `OQ-0006` 下 official billing-grade pre-release cost proof 仍不存在，继续挡住 shared `pre_release_run_report` attestation；二是 `OQ-0002` 下 production monthly snapshot 还必须等待 first truthful post-install closed billing period。只要这两类 blocker 中任一未收口，`EVID-COST-001` 仍会继续失败，而 `EVID-MEDIA-001` / `EVID-DER-001` / `EVID-SEC-001` / `EVID-OPS-001` 也都会继续因为 shared pre-release attestation 缺失而 fail-closed，因此仍不能诚实声称达到 `L1 Local-Core`。
