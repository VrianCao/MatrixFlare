...
- [x] 完成 `IF-CS-010`~`IF-CS-014`。
  ...
  当前状态:
  `/_matrix/client/{r0,v1,v3}/login`、`/_matrix/client/{r0,v1,v3}/register` 与 `/_matrix/client/{r0,v1,v3}/register/available` 现已按同一 password / dummy truth 收口；`TEST-CS-001` / `EVID-CS-001` 也已同步抬高为逐别名锁定 `/.well-known/matrix/client`、`/_matrix/client/versions`、discovery / availability truth、`Cache-Control` 真值、browser `Origin` + `OPTIONS` preflight、anonymous public-entry alias-matrix bounded-window live `429 M_LIMIT_EXCEEDED` + `200/429` envelope truth、初始 UIA challenge、成功 registration/login、错误模型，以及 `access_token` / `refresh_token` issuance 与 `device_id` 副作用 parity 的 gate；同时 `TEST-GOV-001` 本地治理检查也显式钉住 `IF-CS-005`,`IF-CS-009`,`IF-CS-067`,`IF-CS-010`,`IF-CS-011` 的 route-set，避免 compatibility alias drift 被便利性绿灯漏掉。
...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  `2026-04-06` latest same-head rerun 现已前推到 pushed head `63e594a68f0a0a23c21b62abc298fc2e5ec6fe78` 的 GitHub Actions run `24051819997`。这轮 rerun 先把 earlier branch-local reopen 又收窄了一层：`tests/staging/test-cs-002.test.mjs` 的 import defect 与 `tests/staging/test-der-001.test.mjs` / `tests/pre-release/test-der-001.test.mjs` 的 rebuild wait defect 已不再是 active blocker，raw `staging.log` 真实记录 `TEST-CS-002` 与 `TEST-DER-001` 已 `pass`，raw `pre-release.log` 也继续记录 `TEST-DER-001`,`TEST-MEDIA-001`,`TEST-OPS-001`,`TEST-SEC-001` 全部 `pass`，而 `TEST-COST-001` 仍按 `OQ-0006` truthful fail-closed。与此同时，same-head `ci-integration` 与 `staging` 又一起把 `TEST-CS-001` 的 current contract 缺陷暴露成了比 `24050404771` 更精确的 truth：`ci-integration.log` 记录 30-attempt public-entry alias probe 仍全为 `200`，`staging.log` 则记录同一 probe 已出现多条 `429`，但仍只有 alias 子集命中，导致“每个 alias individually 都要命中 live 429”的 gate 继续失败。随后对这两个 same-head deployed `workers.dev` 环境做的 direct live probe（80 attempts / 720 public-entry GETs）又把这条差距进一步坐实：`ci-integration` 与 `staging` 都直到 attempts `60/61` 左右才首次观察到 `429`，且整个窗口里仍只有部分 alias ever 命中 `429`。结合 `CF-WKR-027` 与 Cloudflare 官方 Workers rate-limiting docs 对 local-to-location / eventually consistent / permissive 行为的约束，当前 `43` / `44` / canonical suite 把 anonymous public-entry gate 写成“逐 alias 都要 non-local 命中 `429`”已被 same-head truth 证伪，不能继续保留为 honest release contract。当前 local worktree 因而不再是“已修回、只差 rerun”，而是进入一条更精确的 repair：`TEST-CS-001` / `EVID-CS-001` 与 `tests/integration/test-cs-001.test.mjs` / `tests/staging/test-cs-001.test.mjs` 必须改成 shared alias-matrix bounded-window live `429` proof，同时把 alias-to-policy wiring 继续交给 local runtime controls fail-closed 锁定。直到这条 repaired head 拿到新的 same-head non-local attested rerun 前，`08.04` 在本分支上仍不能写成 cost-only；当前 active blocker 是 `TEST-CS-001` contract/suite repair 加上 pre-release `OQ-0006` cost gate。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  `2026-04-06` latest same-head evidence truth 同样已经前推到 pushed head `63e594a68f0a0a23c21b62abc298fc2e5ec6fe78` 的 run `24051819997`，因此这里不能再保留“未推送 repair worktree / multi-blocker pending rerun”的含混叙述。该 run 中，`ci-integration` / `staging` / `pre-release` 都只上传了 raw artifact，没有任何 attestation artifact；`evidence-l1` 随后也真实下载到 `0` 份 `phase08-*-attestation-20260406T212028Z-24051819997-1` 文件，并因此再次只让 `EVID-GOV-001` 为 `pass`。但 active blocker 集合已经与 `24050404771` 不同：`TEST-CS-002` 与 `TEST-DER-001` 的 same-head raw regressions已被移除，pre-release 继续只是 `OQ-0006` truthful fail-closed，而当前 branch 之所以还不是 cost-only，是因为 `TEST-CS-001` 的 anonymous public-entry non-local contract/suite 仍被 same-head `ci-integration` / `staging` 原始日志与 direct live probe 一起证伪。换言之，`08.05` 在本分支上的最新 truth 已不再是泛化的“多项代码回归待 rerun”，而是更精确的“两类 blocker 共存”：其一，`TEST-CS-001` / `EVID-CS-001` 仍需按 `CF-WKR-027` 修正 contract、suite 与 rerun；其二，pre-release shared attestation 仍继续受 `OQ-0006` truthful fail-closed 约束。在这两类 blocker 至少还剩其一未收口时，当前 branch 仍不能诚实声称 `08.05` 或 `L1 Local-Core` 已完成。
