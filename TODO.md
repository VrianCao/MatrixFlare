...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  但本项仍未完成：`2026-04-01` 的真实 run `23856450468` 已证明 `ci-integration` 与 `pre-release` 能成功部署、执行 suite、上传 raw bundle 到 R2，并生成有效 attestation；同一 run 中 `staging` 仍失败，当前可审计 workflow 工件只证明它在 deploy 后立刻进入 suite 时于 `register` 关键写路径遭遇远端 `500`，还不足以单独排除其他 request-path 缺陷或把根因定性为“已确认的 readiness 窗口”。当前变更集已在 non-local tooling 中补上 bounded post-deploy readiness probe，用来验证“fresh deployment identity 已可见但请求路径尚未 ready”的假设，并要求在 suite 前用真实公开只读/写路径证明环境稳定，否则 fail-closed；但在新的 GitHub Actions run 真实证明 `staging` attestation 产出之前，本项仍不得宣称完成。与此同时，当前 remote harness 依然只覆盖 smoke 级路径，尚未把 `TEST-CS-001`~`TEST-COST-001` 的完整 L1 语义迁入 dedicated non-local suites；`packages/testing/src/evidence.mjs` 的 canonical `TEST-ID -> implementation files` 映射也仍只有 `local` 入口，尚不能诚实宣称全部 mandatory non-local gate 项都已有 dedicated environment-backed proof。因此即使 workflow 已存在，本项仍保持未完成。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  目前仓库已补上 `.github/workflows/nonlocal-phase08.yml` 与对应 non-local tooling，可在 GitHub Actions 中部署 `ci-integration` / `staging` / `pre-release`、上传 raw bundle 到 R2、并按 `DEC-0002` 生成 provenance-ready attestation；workflow 也会在 evidence 阶段消费 `prod-cost-attestation`（若存在），否则继续对 `EVID-COST-001` fail-closed。`2026-04-01` 的真实 runs `23855295188` 与 `23856450468` 已证明 `pre-release`、`ci-integration` 能成功部署、执行 suite、上传 raw bundle 到 R2，并生成有效 attestation；但当前可审计 GH 工件对 `staging` 只证明“deploy 后立刻进入 suite 时仍会在远端关键路径失败”，尚不足以把 blocker 诚实定性为单一 readiness window 或宣布旧问题已被完全排除。当前变更集已补上 bounded post-deploy readiness probe，用来验证该 readiness 假设；在新的 GitHub Actions run 真实证明 `staging` attestation 产出前，`08.04` 仍未闭合。与此同时，`20260331T130238Z` 那批基于 `_test-runs` 原始 report 的旧 L1 evidence summary 已被 supersede，不再可作为 release-gate 依据；而且 non-local `TEST-ID -> implementation files` 映射仍缺 `ci-integration` / `staging` / `pre-release` 入口，当前 evidence 仍会对 `TEST-CS-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001` 报 `mapping_error`。更关键的是，`EVID-COST-001` 仍要求真实 production monthly cost snapshot，而 Cloudflare Billing Usage/Paygo API 的自动化适用边界仍记录在 `spec/open-questions/OQ-0002.md`，因此当前仍不能诚实宣称 `L1 Local-Core` 已达成，本项保持未完成。
