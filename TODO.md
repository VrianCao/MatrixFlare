...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  但本项仍未完成：`2026-04-01` 的真实 run `23860170272` 已证明 `ci-integration`、`staging`、`pre-release` 都能成功部署、通过 bounded readiness probe、执行 environment-backed suite、上传 raw bundle 到 R2，并生成有效 attestation；旧的“等待 staging 成功 attestation” blocker 已失效，相关完成声明不得再沿用。当前 `08.04` 的真实剩余问题已收敛为两类：其一，`tests/integration/l1-mandatory.test.mjs`、`tests/staging/l1-mandatory.test.mjs`、`tests/pre-release/l1-mandatory.test.mjs` 仍只覆盖 smoke 级 HTTP path，还没有把 `TEST-CS-001`~`TEST-COST-001` 的完整 L1 语义迁入 dedicated non-local suites；其二，`packages/testing/src/evidence.mjs` 对 non-local canonical `TEST-ID -> implementation files` 继续 fail-closed，且现在不仅拒绝 `l1-mandatory.test.mjs`、`bootstrap.test.mjs` 与 `tests/shared/*` 这类 generic/shared proof 路径、拒绝 basename 未以对应 `TEST-ID` 小写形式起始的同目录改名 shim，还要求 canonical file 必须真实存在于 repo、且其 repo-owned transitive dependency closure 也留在同一环境目录内，因此当前仍不能诚实补上映射或宣称全部 mandatory non-local gate 项都已有 dedicated environment-backed proof。换言之，non-local harness 已从“薄 shim”推进到“真实三环境 attested smoke + readiness closure”，但距离 `08.04` 完成仍差完整的 `TEST-ID` 级 non-local suites。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  目前仓库已补上 `.github/workflows/nonlocal-phase08.yml` 与对应 non-local tooling，可在 GitHub Actions 中部署 `ci-integration` / `staging` / `pre-release`、上传 raw bundle 到 R2、并按 `DEC-0002` 生成 provenance-ready attestation；workflow 也会在 evidence 阶段消费 `prod-cost-attestation`（若存在），否则继续对 `EVID-COST-001` fail-closed。`2026-04-01` 的真实 run `23860170272` 已证明三环境都能成功产出有效 attestation，并在 `evidence-l1` job 中生成 `20260401T164911Z` evidence bundles；旧的 `_test-runs` 原始 report 型 summary 已被 supersede，不再可作为 release-gate 依据。当前失败也因此被进一步收敛为真实 blocker：所有需要 non-local coverage 的 `EVID-*` 现在都因缺少对应环境的 canonical `TEST-ID -> implementation files` mapping 而 fail-closed，`EVID-COST-001` 还额外因缺少真实 `prod-cost-attestation` 而 fail。由于当前 `tests/integration|staging|pre-release` 下现有文件仍只有 `bootstrap.test.mjs` 与 `l1-mandatory.test.mjs` 这类 generic wiring/smoke 入口，`packages/testing/src/evidence.mjs` 现已明确拒绝把它们、`tests/shared/*` 支撑模块、basename 未以对应 `TEST-ID` 小写形式起始的同目录改名 shim、canonical path 指向不存在 repo 文件的伪映射，或经由 wrapper 间接回指这些 generic/shared 路径的 repo-owned dependency closure 计入 canonical non-local `TEST-ID` coverage proof；因此现在既不能靠补上映射把 evidence 伪装为通过，也不能把三环境 attested smoke run 误报成 `L1 Local-Core`。更关键的是，`EVID-COST-001` 仍要求真实 production monthly cost snapshot，而 Cloudflare Billing Usage/Paygo API 的自动化适用边界仍记录在 `spec/open-questions/OQ-0002.md`，因此当前仍不能诚实宣称 `L1 Local-Core` 已达成，本项保持未完成。
