...
- [ ] 先把 `TEST-OPS-001` / `EVID-OPS-001` 所需的 dual-version orchestration、Worker/DO pairing observability 与 attested proof contract 写成可执行 Spec，再实现 non-local pre-release skew gate。
  Spec refs: `13` `CF-WKR-012`,`CF-WKR-014`,`CF-DO-005`,`CF-DO-021`, `42` `REQ-OPS-010`,`REQ-OPS-011`,`REQ-OPS-012`, `43` `TEST-OPS-001`, `44` `EVID-OPS-001`, `OQ-0005`
  产出:
  ...
  最新 pushed-head rerun `23935350182`（head `10ab90373674e9686bbbb03823f02c87c2cf2501`）进一步证明前一轮 header-echo repair 已经真实生效，但 `08.04C` 仍未闭环：`ci-integration` 与 `staging` 均成功上传 attestation，`pre-release` 也成功完成 dual-version start、suite execution、restore、raw bundle upload、R2 upload、provenance write 与 pre-attestation environment gate；真正失败点已从 seed-route header echo 收窄为 probe-owned identity uniqueness。复核 raw `pre-release.log` / `pre-release.json` 后可确认：`tests/pre-release/test-ops-001.test.mjs` 现在失败于 `rollout skew probe returned 409: {"code":"precondition_failed","message":"Probe login failed for baseline-user-1-probe-gha-pre-re: \"error code: 1042\"" ...}`，随后 `rollout_skew_probe` sidecar 缺失、attestation 被跳过、`Enforce environment gate` fail-closed。结合 current worktree 代码复核后可确认：上一轮 repair 仍把 `seed_prefix + probe_run_id` 先裁到 `256` 再提取 uniqueness tail，因此当 `seed_prefix` 足够长时，`probe_run_id` 的熵仍可能被裁掉，远端 rerun 还能复用同一 probe-owned username / room alias。当前未推送 repair worktree 已把 `buildProbeScopedToken(...)` 改为从完整 normalized scope 提取 uniqueness tail，不再提前截断，并把 `tests/local/control-plane/phase-08-ops.test.mjs` 的 rerun regression 扩到 `seed_prefix > 256` 的边界；重新执行 `npm run governance:check`、`node --test tests/local/control-plane/phase-08-ops.test.mjs`、`node --test tests/local/runtime-foundations/nonlocal-tooling.test.mjs`、`node --test tests/local/runtime-foundations/testing-harness.test.mjs`、`npm run test:all`、`date -u +%Y%m%dT%H%M%SZ`（得到 `20260403T055906Z`）以及 fail-closed `npm run evidence:l1 -- --timestamp 20260403T055906Z` 后，结果仍只让 `EVID-GOV-001` 为 `pass`。在这轮 repair 被推送并拿到新的 attested rerun 之前，`08.04C` 仍不得被写成完成。
  更近一轮 latest pushed-head rerun `23936154908`（head `192224890f65ed0d5b80826a6e0c502ecbba080a`）又把 remote truth 改写了一次：`ci-integration` / `staging` 继续成功上传 attestation，`pre-release` 也成功完成 dual-version start、suite execution、restore、raw bundle upload、R2 upload 与 provenance write，但 `tests/pre-release/test-ops-001.test.mjs` 仍在真实 probe 请求上返回 `409`，raw `pre-release.log` / `pre-release.json` 记录为 `Probe login failed for baseline-user-1-61549081: "error code: 1042"`，随后 `rollout_skew_probe` sidecar 缺失、attestation 被跳过、`Enforce environment gate` fail-closed。由于 current head 已经包含上一轮 long-seed truncation repair，这轮失败不再支持“仍是旧的 uniqueness tail 被截断”这一解释；复核 `packages/control-plane/src/ops-handler.mjs` 后可确认，更贴近远端真相的 remaining code gap 是：`seedProbeUserForVersion()` / `seedProbeRoomForVersion()` 的 bounded sampling 目前只会对“成功创建 identity 但命中错误 authority version”的情况继续采样，一旦 `ensureProbeUser()` / `ensureProbeRoom()` 在 register/login/create/join 上遇到单次 setup failure，就会整轮 abort，而不会前进到下一个 deterministic seed attempt。
  当前未推送 repair worktree 已据 `23936154908` 再补一轮 probe hardening，而不是继续把 blocker 错归因给旧的 truncation bug：`packages/control-plane/src/ops-handler.mjs` 现在会把 register/login/createRoom/join 这类 setup failure 标记为 attempt-local retryable，并让 `seedProbeUserForVersion()` / `seedProbeRoomForVersion()` 在同一 bounded sampling 窗口内继续前进到下一个 deterministic seed label；`tests/local/control-plane/phase-08-ops.test.mjs` 也新增了直接复现 remote failure shape 的回归，覆盖“第一次 register 冲突后 login 返回纯文本 `error code: 1042`，probe 仍必须继续到下一个 seed attempt 并最终完成 skew proof”。对应本地重新执行 `node --test tests/local/control-plane/phase-08-ops.test.mjs` 与 `node --test tests/local/runtime-foundations/nonlocal-tooling.test.mjs` 后均已通过；但在这轮 repair 被推送并拿到新的 attested rerun 之前，`08.04C` 仍不得被写成完成。
...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  `2026-04-03` 最新 pushed-head rerun 现已更新为 `23935350182`（head `10ab90373674e9686bbbb03823f02c87c2cf2501`）。这轮远端真相说明 `08.04` 的非 COST blocker 仍只剩 `TEST-OPS-001`：`ci-integration` / `staging` 已成功上传 attestation，`pre-release` 的 dual-version rollout / suite / restore / provenance 主链路也已闭合，但 rollout probe 仍因长 `seed_prefix` 下 probe-owned identity collision 触发 login fallback 而 fail-closed，进而缺失 `rollout_skew_probe` sidecar 与 attestation。当前未推送 repair worktree 已把这条 long-seed truncation collision 修正到与现有 rollout contract 一致；但在新的 current-head attested rerun 产出前，`08.04` 的未完成根因仍然是 `TEST-OPS-001` / `EVID-OPS-001` 尚未对 repaired head 收口，以及 `TEST-COST-001` / `EVID-COST-001` 仍未闭环。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  更近一轮 latest pushed-head rerun `23935350182`（head `10ab90373674e9686bbbb03823f02c87c2cf2501`）继续证明 `08.05` 仍不能诚实前推：前一轮 seed-route header-echo repair 已真实把 blocker 再向前推了一步，但 raw `pre-release` artifact 又暴露出新的 current-head OPS truth，即 probe-owned usernames / room aliases 在长 `seed_prefix` 下仍可能发生 truncation collision，进而触发 login fallback、缺失 `rollout_skew_probe` sidecar，并继续把 pre-release attestation fail-closed 挡住。当前未推送 repair worktree 已据此把 `packages/control-plane/src/ops-handler.mjs` 改为从完整 `seed_prefix + probe_run_id` scope 提取 uniqueness tail，不再在 `256` 字符边界前截断 probe-run entropy，并把 `tests/local/control-plane/phase-08-ops.test.mjs` 的 rerun regression 扩到 `seed_prefix > 256` 的边界；重新执行 `npm run governance:check`、`node --test tests/local/control-plane/phase-08-ops.test.mjs`、`node --test tests/local/runtime-foundations/nonlocal-tooling.test.mjs`、`node --test tests/local/runtime-foundations/testing-harness.test.mjs`、`npm run test:all`、`date -u +%Y%m%dT%H%M%SZ`（得到 `20260403T055906Z`）及 fail-closed `npm run evidence:l1 -- --timestamp 20260403T055906Z` 后，结果仍然只让 `EVID-GOV-001` 为 `pass`。因此，`08.05` 当前的真实闭环条件仍未变化：必须先让 repaired head 拿到新的 pre-release attestation，再观察 `EVID-DER-001` / `EVID-OPS-001` 是否收口；`EVID-COST-001` 则继续同时受 `OQ-0006` 与 `OQ-0002` 约束。
  `2026-04-04` current worktree 还把 `08.05` 的 active blocker 面写得更清楚了：当前 `pre-release` 证据不再只是被 rollout-skew probe 挡住。因为 dedicated canonical `tests/pre-release/test-cost-001.test.mjs` 现已自动进入同一 `pre-release` suite，而 `/_ops/v1/cost/observation` 又按 `OQ-0006` 继续 fail-closed，所以在当前单一 `pre_release_run_report` topology 下，未解决的 `TEST-COST-001` 会和 `TEST-OPS-001` 共享同一个 pre-release attestation gate。换言之，`08.05` 的 current-head 闭环条件已经从“先恢复 pre-release attestation，再观察 `EVID-DER-001` / `EVID-OPS-001` 是否收口”改写成了更严格也更真实的版本：必须先让 repaired head 解决 active pre-release gate 中的至少 `OPS` 与 `COST` 两类问题，才能重新得到可消费的 same-head pre-release attestation；即使 `OPS` repair 真实生效，`EVID-COST-001` 仍继续同时受 `OQ-0006` 与 `OQ-0002` 约束，且在 cost gate 继续 fail-closed 时，`EVID-DER-001` / `EVID-OPS-001` 也会一起缺少可消费的 same-head pre-release attestation。
