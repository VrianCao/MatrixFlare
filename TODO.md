...
- [ ] 通过 `DEC-0005` 把 production automation contract 固化到 Spec/TODO，再落 GitHub Actions + CLI/tooling 的 `prod-install` / `release-candidate` / `promote-prod` / `rollback-prod` / `prod-cost-monthly` 基础设施；若 remote 验证尚未执行，或 fail-closed 时仍留不下 raw blocker artifact，不得把本项写成已完成。
  ...
  当前状态:
  该项在当前 worktree 中已不再是“完全未落地”的状态：`prod-install` / `release-candidate` / `promote-prod` / `rollback-prod` / `prod-cost-monthly` workflow 以及对应 `CLI/tooling/local tests` 现已真实存在，并继续补 current-repo/current-head 绑定、active-topology/current-state fail-closed guard、migration-safe promote path 与 raw failure-artifact retention。但这仍不构成完成依据，因为 current-head remote proof 现在已经被更具体的 GitHub Actions 平台事实卡住，而不再只是“还没来得及跑”：`2026-04-05` pushed head `2da7daccee16a65c3fc08e8dda95541763c43029` 上，仓库默认分支仍是 `master`，对 `phase08-nonlocal-closure` 执行 `gh workflow run prod-install.yml --ref phase08-nonlocal-closure -f release_ref=2da7daccee16a65c3fc08e8dda95541763c43029` 立即收到 GitHub API `404 workflow prod-install.yml not found on the default branch`。结合官方 `workflow_dispatch` 文档，这说明 newly-added prod workflows 还只存在于 feature branch 时，same-head dispatch 本身就不可执行；对应 blocker 已转入 `OQ-0007`。与此同时，earlier pushed-head run 已继续证明 `prod-cost-monthly` 的 Billing API access 与 raw bundle R2 upload 可用，但 `prod-install` / `release-candidate` / `promote-prod` / `rollback-prod` 仍因 default-branch constraint 缺 current-head remote proof，target account 中 `matrix-*-prod` 固定命名 topology 也尚未被当前 head 真实建立。在这些 workflow definitions 进入 default branch 或通过新 `DEC-ID` 固定 default-branch dispatcher path，并重新拿到 same-head GitHub Actions truth 之前，本项继续保持未完成。
