...
- [ ] 先把 `TEST-OPS-001` / `EVID-OPS-001` 所需的 dual-version orchestration、Worker/DO pairing observability 与 attested proof contract 写成可执行 Spec，再实现 non-local pre-release skew gate。
  ...
  当前未推送 repair worktree 已据此再补一轮修复：`runEnvironmentBackedSuite()` 现在会在 pre-release rollout 场景下把 `before_readiness` 与 `before_suite` 两次 Cloudflare deployment revalidation 都绑定到 `rolloutState.dual_version_deployment_id`，并继续固定 node runner 为 `--test-concurrency=1`；`tests/staging/test-der-001.test.mjs` 与 `tests/pre-release/test-der-001.test.mjs` 也把 derived search eventual window 扩到 `120 * 500ms` 并补上缺失 event id 的诊断信息。修复后重新执行 `npm run governance:check`、`node --test tests/local/runtime-foundations/nonlocal-tooling.test.mjs`、`node --test tests/local/runtime-foundations/testing-harness.test.mjs`、`npm run test:all`、`date -u +%Y%m%dT%H%M%SZ`（得到 `20260403T032833Z`）以及 fail-closed `npm run evidence:l1 -- --timestamp 20260403T032833Z`；结果仍只让 `EVID-GOV-001` 为 `pass`，说明当前本地 evidence gate 没有因这轮 repair 而出现 fail-open。因此，`08.04C` 现在仍不受 “缺 canonical suite/mapping” 或 “低于 SR2” 阻塞，但也还没被新的 attested rerun 收口；在新的 current-head GitHub Actions run 真实产出 pre-release attestation 前，`TEST-OPS-001` / `EVID-OPS-001` 继续保持未完成。
...
- [ ] 先把 `TEST-COST-001` / `EVID-COST-001` 的 pre-release actual-cost proof contract 写成可执行 Spec，再讨论对应 canonical non-local suite。
  ...
  当前状态:
  `2026-04-02` 本轮已把 pre-release half 的 contract 收敛到 `PreReleaseCostObservation`：必须对 bounded workload 的同一 pre-release environment/window 查询 official Cloudflare metrics/billing surfaces，并把 normalized `cost_surfaces` + `model_comparison` attestation 化到 pre-release report 中；`OQ-0006` 也已相应更新，不再把本项描述成“没有 contract”。但当前 repo 仍未证明 workflow/tooling/secret scope 可以稳定拿到这些 official metrics surfaces，且 dedicated suite/mapping 仍缺，因此本项虽已达到 `SR2`，仍不能宣称完成；`OQ-0002` 继续只约束 production monthly snapshot / pricing semantics，不得与本项混写。
...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  但本项仍未完成，当前真相已进一步收敛为四点。其一，`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-SEC-001` 已在 pushed-head attested runs 上拿到 `pass`，不再受“缺同轮 attestation”阻塞。其二，run `23923285726` 曾真实证明 `TEST-DER-001` 可以由 dedicated staging / pre-release canonical suites 闭环；但更近一轮 latest pushed-head rerun `23932130160`（head `21b1e59baf78c7de7410b97baa76940d42e038c0`）又在 `staging` 上把 `TEST-DER-001` 打回 fail-closed，因此 `DER` 目前不能被诚实写成“current head 已闭环”，只能写成“已有历史 attested pass，但当前 head 仍待 rerun 重新证明”。其三，`TEST-OPS-001` 已不再低于 `SR2`：current worktree 已补上 dedicated canonical suite、workflow orchestration、probe route 与 strict evidence mapping，并已通过本地 fail-closed 验证；但 `23932130160` 仍证明 current head 的 pre-release attestation 没拿到，所以 `08.04C` 继续未完成。其四，`TEST-COST-001` 仍继续受 `OQ-0006` 与 `OQ-0002` 约束，既缺 official metrics/billing access 闭环，也缺 production monthly snapshot。换言之，`08.04` 现在未完成的原因不再是 non-local Access topology，但仍同时受 current-head DER attested closure、current-head OPS attested closure 与真实 cost proof 收口约束；在这些 blocker 收口前，仍不能把 `08.04` 标成完成。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  `2026-04-03` 当前未推送 repair worktree 先据 `23931328404` 补上第三轮修复：`packages/testing/src/nonlocal.mjs` 现会在 pre-release rollout 场景下按 `rolloutState.dual_version_deployment_id` 做 suite 前 Cloudflare revalidation，并把 node runner 固定为 `--test-concurrency=1`；`tests/local/runtime-foundations/nonlocal-tooling.test.mjs` 也新增对这两个行为的回归覆盖。更近一轮 latest pushed-head rerun `23932130160`（head `21b1e59baf78c7de7410b97baa76940d42e038c0`）随后又把 current-head truth 进一步改写为：`ci-integration` 成功并上传 attestation，但 `staging` 在 `TEST-DER-001` fail-closed 失败，`pre-release` 仍因 rollout-scoped deployment revalidation 只修到了 `before_suite`、尚未修到 `before_readiness` 而失败，`evidence-l1` 因只下载到 `ci-integration` attestation 而再次只剩 `EVID-GOV-001` 为 `pass`。因此，先前任何“current head 的 DER 已闭环”表述现在都不再成立，必须回退成真实未完成状态。
  当前未推送 repair worktree 已再补第四轮修复：`runEnvironmentBackedSuite()` 现会在 pre-release rollout 场景下把 `before_readiness` 与 `before_suite` 都绑定到 `rolloutState.dual_version_deployment_id`，`tests/staging/test-der-001.test.mjs` / `tests/pre-release/test-der-001.test.mjs` 则把 derived search eventual window 扩到 `120 * 500ms`。修复后重新执行 `npm run governance:check`、`node --test tests/local/runtime-foundations/nonlocal-tooling.test.mjs`、`node --test tests/local/runtime-foundations/testing-harness.test.mjs`、`npm run test:all`、`date -u +%Y%m%dT%H%M%SZ`（得到 `20260403T032833Z`）以及 fail-closed `npm run evidence:l1 -- --timestamp 20260403T032833Z`；结果仍只让 `EVID-GOV-001` 为 `pass`，说明当前本地 evidence gate 没有因这轮 repair 而出现 fail-open。换言之，`08.05` 现在的未完成原因已收敛为三类：一是必须把当前 DER repair 推到新的 attested rerun 上，真实恢复 `staging` / `pre-release` 的 `EVID-DER-001` 输入；二是必须把当前 OPS repair 推到新的 attested rerun 上，真实观察 `EVID-OPS-001` 是否随 `TEST-OPS-001` implementation 收口；三是 `EVID-COST-001` 仍继续受 `OQ-0006` / `OQ-0002` 约束，既缺 pre-release official metrics proof，也缺真实 production monthly cost snapshot。在这三项收口前，仍不能诚实声称达到 `L1 Local-Core`。
