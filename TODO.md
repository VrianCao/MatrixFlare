...
- [ ] 先把 `TEST-OPS-001` / `EVID-OPS-001` 所需的 dual-version orchestration、Worker/DO pairing observability 与 attested proof contract 写成可执行 Spec，再实现 non-local pre-release skew gate。
  ...
  `2026-04-04` 当前未推送 repair worktree 已把这条 transport diagnosis 推进成可执行配置与可审计运行时 contract，而不是只停留在文档推断：`apps/ops-worker/wrangler.jsonc` 与 `packages/runtime-core/src/runtime-manifest.mjs` 现在对 `ops-worker` 显式钉住 `global_fetch_strictly_public`；`packages/runtime-core/src/deployment-records.mjs`、`packages/control-plane/src/ops-handler.mjs`、`packages/control-plane/src/schemas.mjs` 与 `spec/framework/26-wire-schema-catalog.md` 也把 `compatibility_flags` 纳入 deployment record 与 `/_ops/v1/healthz`，使该 worker-specific rollout invariant 可被真实审计，而不是继续藏在 manifest/wrangler 之外。对应本地重新执行 `npm run governance:check`、`node --test tests/local/control-plane/phase-08-ops.test.mjs`、`node --test tests/local/runtime-foundations/config.test.mjs`、`node --test tests/local/runtime-foundations/phase-08-runtime-controls.test.mjs`、`node --test tests/local/runtime-foundations/runtime-skeleton.test.mjs`、`node --test tests/local/runtime-foundations/nonlocal-tooling.test.mjs`、`node --test tests/local/runtime-foundations/testing-harness.test.mjs`、`npm run test:all`、`date -u +%Y%m%dT%H%M%SZ`（得到 `20260404T143813Z`）以及 fail-closed `npm run evidence:l1 -- --timestamp 20260404T143813Z` 后，结果仍只让 `EVID-GOV-001` 为 `pass`。换言之，这轮 repair 已经补齐 local config/audit gap，但仍不能替代新的 pushed-head attested rerun。
...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  `2026-04-04` latest pushed-head rerun `23980399903` 进一步把 `08.04` 的 current-head truth 写实到单轮 artifact 上：`ci-integration` / `staging` 已成功上传 attestation，`pre-release` raw report 里 dedicated `tests/pre-release/test-ops-001.test.mjs` 与 `tests/pre-release/test-cost-001.test.mjs` 都已真实执行，但 `TEST-OPS-001` 以与 same-zone public fetch 约束一致的 Cloudflare error `1042` failure shape 失败，`TEST-COST-001` 则因 `OQ-0006` 继续得到诚实的 fail-closed `500/internal`。因此，`08.04` 当前未完成的根因已不再是 Access topology、canonical mapping、或 uniqueness/retry 假设，而是两个 active pre-release blockers：`OPS` 仍缺 Cloudflare-official same-zone transport closure 的 attested验证，`COST` 仍缺 official billing-grade environment-scoped proof surface。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  最新 pushed-head rerun `23980399903`（head `5b9ddbd6b2105c170ba807d5e93efdd2df62f85b`）继续证明 `08.05` 仍不能诚实前推，但当前 evidence 真相已比 earlier runs 更具体：同轮 `evidence-l1` 中，`EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002` 已真实转为 `pass`；`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001` 仍为 `fail`。其中前四项当前都共享同一个 same-head pre-release attestation 缺口，而 `EVID-COST-001` 还继续额外缺真实 `ProdCostSnapshotAttestation`。
  当前 `08.05` 的真实闭环条件也因此进一步收敛：必须先让 repaired head 解决 active pre-release gate 中的至少 `OPS` 与 `COST` 两类问题，才能重新得到可消费的 same-head pre-release attestation；而 `OPS` 的 current-head blocker 最强支撑解释已从 earlier probe-owned identity 猜测转向 official-doc-backed same-zone fetch/platform-topology gap，`COST` 则继续按 `OQ-0006` / `OQ-0002` fail-closed。只要 cost gate 继续 fail-closed，`EVID-DER-001` / `EVID-MEDIA-001` / `EVID-SEC-001` / `EVID-OPS-001` 也会一起缺少可消费的 same-head pre-release attestation，因此仍不能诚实声称达到 `L1 Local-Core`。
  `2026-04-04` latest pushed-head rerun `23980399903` 也把 evidence truth 改写得更精确了：`evidence-l1` 已不再回退到“只有 `EVID-GOV-001` pass”的旧状态，而是恢复为 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002` 为 `pass`；`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001` 仍为 `fail`。其中当前 `OPS` 失败原因也不再是 long-seed collision / retry 假设，而是 current-head pre-release suite缺 attested `rollout_skew_probe`，其更底层 blocker 目前由同轮 raw artifact 与 Cloudflare 官方 docs 强烈指向 same-zone public fetch contract 未闭合；`COST` 则继续同时受 `OQ-0006` 与 `OQ-0002` 约束。因此，`08.05` 现在的真实闭环条件是：先让 current head 在 pre-release 上拿到 Cloudflare-official transport closure后的 `rollout_skew_probe`，再面对仍然 fail-closed 的 `TEST-COST-001` / `prod-cost-attestation` 双重 gate；在这些 blocker 中任一仍未收口时，都不能诚实声称达到 `L1 Local-Core`。
  `2026-04-04` 当前未推送 repair worktree 也已把这轮 OPS transport repair 的本地 fail-closed 验证补齐：除了前述 config/runtime/control-plane/local test 更新外，`npm run evidence:l1 -- --timestamp 20260404T143813Z` 仍旧只让 `EVID-GOV-001` 为 `pass`，随后本地生成的 `evidence/common/_test-runs/20260404T143813Z` 与对应 `EVID-*` 目录也已清理。也就是说，这轮 worktree 没有把 `08.05` 做成 fail-open；但在新的 non-local attested rerun 与 production monthly cost snapshot 出现前，它仍不能把任何额外 `EVID-*` 诚实转成 `pass`。
