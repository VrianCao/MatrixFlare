...
- [x] 实现 `IF-PUB-001`,`IF-PUB-002`,`IF-CS-001`,`IF-CS-002`,`IF-CS-005`,`IF-CS-009`,`IF-CS-066`,`IF-CS-067`。
  ...
  当前状态:
  `gateway-worker` 现在会对 `/.well-known/matrix/client`、`/_matrix/client/versions` 与 `/_matrix/client/*` 的 browser `Origin` 请求统一附加 CORS 响应头，并在 `OPTIONS` preflight 时直接返回 `204`，不再把标准浏览器预检请求误落到 `M_UNRECOGNIZED`。本轮针对真实 Element Web homeserver validation 又补上了先前遗漏的 `/versions` stable ladder contract：当前响应已改为枚举 `v1.1`~`v1.17`，而不是只回 bare latest `v1.17`，避免 current browser SDK 因 exact-match version gating 把 prod 误判成“过旧/不支持”。
...
- [x] 完成 `IF-CS-010`~`IF-CS-014`。
  ...
  当前状态:
  `/_matrix/client/{r0,v1,v3}/login`、`/_matrix/client/{r0,v1,v3}/register` 与 `/_matrix/client/{r0,v1,v3}/register/available` 现已按同一 password / dummy truth 收口；`TEST-CS-001` / `EVID-CS-001` 也已同步抬高为逐别名锁定 discovery、availability、初始 UIA challenge、成功 registration/login、错误模型，以及 session issuance / `device_id` 副作用 parity 的 machine-auditable gate，避免 compatibility alias drift 被便利性绿灯漏掉。
...
- [ ] 实现 `IF-CS-040`,`IF-CS-042`,`IF-CS-043`,`IF-CS-044`,`IF-CS-048`,`IF-CS-045`,`IF-CS-046`,`IF-CS-047`,`IF-INT-USER-004`,`DATA-USER-002`,`DATA-USER-003`,`DATA-USER-004`,`DATA-USER-005`,`DATA-USER-010`,`DATA-USER-011`,`DATA-R2-006`。
  ...
  产出:
  device CRUD、keys upload/query/claim/changes、cross-signing upload/signatures、room key backup metadata / object handling、对应 gateway handler 与 `UserDO` 裁决路径。
  完成标准:
  `TEST-CS-003` 可在本地与 non-local 路径上被诚实实现，并验证 one-time key at-most-once、device surfaces、`GET /_matrix/client/v3/keys/changes`、cross-signing surfaces、`/sync` device truth 与 backup metadata。
  当前状态:
  官方 Matrix `v1.17` 核对后新增的 cross-signing route truth（`POST /_matrix/client/v3/keys/device_signing/upload`,`POST /_matrix/client/v3/keys/signatures/upload`）与 `GET /_matrix/client/v3/keys/changes` 已同步补入 `12/23/30/43/44/TODO`，`apps/gateway-worker/src/index.mjs` 与 `UserDO` 也已真实落位 `IF-CS-040`,`IF-CS-042`~`IF-CS-048` 对应公共入口与裁决路径，不再存在“只剩 sendToDevice、devices/keys/room_keys 还没接路由”的旧 blocker。当前本地实现前置由 `tests/local/client-identity/phase-05.test.mjs` 与 `tests/local/client-identity/phase-05a.test.mjs` 共同收口：前者覆盖 to-device transport，后者覆盖 device CRUD/delete UIA replay、keys upload/query/claim/changes、cross-signing upload/signature mismatch、`/sync` device truth 与 room key backup metadata/object handling。
  本轮还新增了 dedicated staging canonical suite `tests/staging/test-cs-003.test.mjs`，并把 `packages/testing/src/evidence.mjs` 的 `TEST-CS-003 -> staging` mapping 接入 strict parser；本地重新执行 fail-closed `evidence:l1` 时，`EVID-CS-003` 的 `mapping_error` 已清零，说明当前 blocker 已不再是“缺 staging canonical suite / mapping”，`/keys/changes` 所需的 missing/cross-device/inverted/future token fail-closed 与 encrypted-room `left` semantics 也已进入 canonical suite 覆盖范围。但本项完成标准仍要求当前代码状态拿到真实 same-head non-local attested pass；在 GitHub Actions 重新执行 `staging` 环境并让 `EVID-CS-003` 由 attestation-backed run 诚实转为 `pass` 前，`05.04A` 继续保持未完成，并由 `08.04` / `08.05` 收口其 non-local 证明链。
...
当前状态:
  stored/inline filter 解析、带完整性保护的 opaque `next_batch`、`collectSince()`、room/account-data/ephemeral 分类收集，以及 `RoomDO` durable fanout -> `DATA-USER-010` append 路径已验证。本轮在真实 Cinny browser session 暴露出 hidden gap 后，又把 token/filter contract 收紧为更贴近官方 `/sync` 语义的 truth：`next_batch` 虽仍可携带 `filter_hash` 作为 opaque integrity metadata，但后续增量 `/sync` 已不再因为调用方从 inline JSON filter 切到 stored `filter_id`，或切到另一个合法 filter，而被错误拒绝为 `M_INVALID_PARAM filter mismatch`。current worktree 的 dedicated staging canonical suite 也已进一步锁定真实 Cinny 会发出的“initial inline filter 带 `room.timeline.limit`，后续 stored filter 只保留 `lazy_load_members`”切换路径；same-head non-local attestation 仍需后续 workflow rerun 诚实收口。
...
- [x] 实现 `IF-CS-051`,`FLOW-CS-MEDIA-DOWNLOAD`,`DATA-R2-001`,`DATA-R2-002`,`DATA-R2-003`。
  ...
  当前状态:
  current `/_matrix/client/v1/media/*` 与 legacy `/_matrix/media/*` 现在不仅共享同一对象/冻结真相，也已对 browser `Origin` 请求共享可读 CORS/preflight 语义；这修复了真实 browser client 在 `GET /_matrix/media/v3/config` 上因缺失 `Access-Control-Allow-Origin` 而被预检拦截的 prod 可用性问题。
...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  `2026-04-06` current pushed head `f1fe81569614af3258d1b14b3254ab69ccad30c7` 的 same-head rerun `24030384435` 已把此前这批 client/media strengthening gates 重新推进到 remote truth：`ci-integration` 与 `staging` 都重新成功上传 attestation，证明 `TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002` 以及 strengthened `TEST-MEDIA-001` 至少已在 current head 上拿到所需的 `ci-integration` / `staging` attested support；同一 run 的 raw `pre-release` artifact 也再次真实记录 `TEST-DER-001`,`TEST-MEDIA-001`,`TEST-OPS-001`,`TEST-SEC-001` 全部 `pass`，且 `rollout_skew_probe.json` 真实存在。该 run 中 `pre-release` 唯一失败项继续只剩 `TEST-COST-001`：raw `pre-release.log` 明确记录它仍按 `OQ-0006` 以 `500/internal` fail-closed，而 `pre-release.json` 也继续以 `pre_release_cost_observation sidecar is required when TEST-COST-001 is covered` 结束，导致 pre-release attestation 被整体跳过。因此，`08.04` 现在的 active blocker 已可再次诚实收敛为 official billing-grade pre-release cost proof 本身，而不是 DER/OPS/client/media dedicated non-local coverage 回退。

### 08.05 完成 L1 证据

### 08.05A 建立 production topology install/promote/rollback automation prerequisite

- [x] 通过 `DEC-0005` 把 production automation contract 固化到 Spec/TODO，再落 GitHub Actions + CLI/tooling 的 `prod-install` / `release-candidate` / `promote-prod` / `rollback-prod` / `prod-cost-monthly` 基础设施；same-head remote 验证必须证明执行入口真实可用，且 fail-closed 时仍保留 raw blocker artifact。
  ...
  完成标准:
  仓库中真实存在 `prod-install` / `release-candidate` / `promote-prod` / `rollback-prod` / `prod-cost-monthly` workflow 与对应 CLI/tooling；其中 `promote-prod` 只能消费 `ReleaseCandidateManifest`，`rollback-prod` 只能消费 `ProdPromotionRecord` 中的 rollback handle，`prod-install` 不得在 active prod topology 已存在时继续充当 redeploy bypass，migration-safe promote 不得再复用 bootstrap gateway 路径；并且至少有 same-head GitHub Actions 证明 target account 中的 `matrix-*-prod` topology 已被这些入口真实建立，`release-candidate` / `prod-install` / `promote-prod` / `rollback-prod` / `prod-cost-monthly` fail-closed 时仍留下 raw blocker artifact，或把 remote blocker 诚实写回正确 artifact。
  当前状态:
  截至 pushed head `ec1b647e55b7fbb571145cc4a3c5acd43beb99b6`，这项前置条件的上一版 contract 确实已经被 same-head remote truth 收口，`OQ-0007` 也已关闭。repository default branch `master` 现已真实包含 `.github/workflows/prod-install.yml`、`.github/workflows/release-candidate.yml`、`.github/workflows/promote-prod.yml`、`.github/workflows/rollback-prod.yml`、`.github/workflows/prod-cost-monthly.yml`，而该 pushed head 已拿到对应 same-head remote runs：`prod-install` success `24000789563`、`release-candidate` fail-closed `24001043537`、`promote-prod` fail-closed `24001065315`、`rollback-prod` fail-closed `24001077197`、`prod-cost-monthly` fail-closed `24000831481`。
  ...
  latest reviewed pushed head 现已前推到 `a706c04127a5cd7978b850061c81b3c646220a4e`：same-head standalone `prod-cost-monthly` run `24005804747` 已真实证明 current worktree 新增的 subscriptions fallback 不只是本地 closure，而是 remote truth。该 run 中 `Resolve closed production billing window` 已从 billing profile 缺字段的情况退回到唯一的 account subscriptions `current_period_end`，并把 `billing-window.json` 真实写成 `resolution_method = cloudflare-account-subscriptions-current-period-end`、`next_bill_date = 2026-04-29T00:00:00Z`、`from_date = 2026-03-01`、`to_date = 2026-03-29`；同时 `Resolve latest successful prod-install baseline run` / `Download prod install baseline record` 继续成功，raw artifact 与 immutable R2 bundle 也都已保留。随后 `Capture production cost snapshot` 在同一 run 中继续按 fail-closed 语义停止，明确报出 `production cost snapshot requires a closed billing window that starts after prod install date 2026-04-05`。这说明 `08.05A` 现仍可诚实维持完成，而且 production-side blocker 已不再是“缺可信 billing-cycle anchor”，而是“当前账号还没到 fixed prod topology 的第一个 truthful post-install closed billing period”；active gap 现已完全转移到 `08.05` 的成本证据闭环，而不是 automation entry contract 本身。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  当前 `08.05` 的真实闭环条件现已进一步收窄为 cost-only gates，而不再要求先单独解决 `OPS`：latest reviewed pushed-head run `23986289866` 的 same-head raw artifact 已证明 `TEST-OPS-001` 为 `pass` 且 `rollout_skew_probe.json` 真实存在，因此 `EVID-OPS-001` 当前仍为 `fail` 的原因已经不是 rollout-skew proof 缺失，而是 shared `pre-release` attestation 仍被 `TEST-COST-001` 的 `pre_release_cost_observation` 缺失挡住。故当前剩余 blocker 只剩两条都与成本相关：`OQ-0006` 下 official billing-grade pre-release cost proof 仍未存在，以及 `OQ-0002` 下真实 `ProdCostSnapshotAttestation` 仍缺失。只要这两类 blocker 中任一未收口，`EVID-COST-001` 就仍会继续失败，而 `EVID-MEDIA-001` / `EVID-DER-001` / `EVID-SEC-001` / `EVID-OPS-001` 也都会继续因为 shared pre-release attestation 缺失而 fail-closed，因此仍不能诚实声称达到 `L1 Local-Core`。
  ...
  同一 run 的 raw `pre-release` artifact 也把 remaining blocker 精确改写成了 cost-only shared gate，而不是 `OPS + COST` 并列：`TEST-DER-001`,`TEST-MEDIA-001`,`TEST-OPS-001`,`TEST-SEC-001` 在 pre-release raw suite 中都已 `pass`，`rollout_skew_probe.json` 也已真实存在；但 `TEST-COST-001` 继续按 `OQ-0006` 以 `500/internal` fail-closed，导致 `pre_release_cost_observation` sidecar 缺失，`pre-release` attestation 被整体跳过。因此，当前 `08.05` 的 remaining blockers 已收敛为两类而且都与 cost 相关：一是 `OQ-0006` 下的 official billing-grade pre-release cost proof仍未存在，继续挡住 shared `pre_release_run_report` attestation；二是 `OQ-0002` 下真实 `prod_cost_snapshot` 仍缺失。只要这两类 blocker 中任一未收口，`EVID-COST-001` 仍会继续失败，而 `EVID-MEDIA-001` / `EVID-DER-001` / `EVID-SEC-001` / `EVID-OPS-001` 也都会继续因为 shared pre-release attestation 缺失而 fail-closed，故仍不能诚实声称达到 `L1 Local-Core`。
  `2026-04-05` reviewed pushed-head runs `24001049145`（head `ec1b647e55b7fbb571145cc4a3c5acd43beb99b6`）与 `24003512030`（head `700b8600264b78eacc07733a7fd09248d9c68e8a`）都曾真实收敛到 “historical pushed heads 上的 remaining blocker 只剩 cost” 这一个结论：`ci-integration` 与 `staging` attestation 已存在，`pre-release` raw artifact 里 `TEST-DER-001`,`TEST-MEDIA-001`,`TEST-OPS-001`,`TEST-SEC-001` 都已 `pass`，而 shared `pre-release` attestation 只被 `TEST-COST-001` / `pre_release_cost_observation` 缺失挡住；production 侧也进一步证明 current account 还没到 fixed prod topology 的第一个 truthful post-install closed billing period。
  `2026-04-06` current pushed head `f1fe81569614af3258d1b14b3254ab69ccad30c7` 的 same-head rerun `24030384435` 已把上述 “current worktree 仍需重新拿回 strengthened client/media attestation” 这一条件转成最新远端真相：同一 run 中，`ci-integration` 与 `staging` attestation 都真实存在，`evidence-l1` 也成功下载并消费了这两份 attestation。对应 `phase08-evidence-l1-20260406T113924Z-24030384435-1` artifact 可确认 current head 的 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002` 已重新为 `pass`；这说明 strengthened `/versions` ladder、真实 Web client `/sync` filter 切换、以及对应 client-core surfaces 现在已经不再是 current head 的 active evidence blocker。
  但这轮 same-head evidence 仍不能写成 `08.05` 完成，因为 shared pre-release attestation 依旧被 cost gate 卡住：`evidence-l1` 中，`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001` 都继续为 `fail`。其中前四项现在的失败原因已不再是各自 suite 回归，而是 current head 仍缺 valid `pre-release` attestation；而这个 attestation 缺口又已被同轮 raw `pre-release` artifact 精确收敛为 `TEST-COST-001` / `pre_release_cost_observation` 缺失。与此同时，`prod-cost-attestation` 仍未存在，`OQ-0002` 下 production monthly snapshot 也仍必须等待 first truthful post-install closed billing period。因此，current head 的真实 remaining blocker 现在已经再次回到 pure cost 口径：`OQ-0006` 下 official billing-grade pre-release cost proof 仍不存在，继续挡住 shared `pre_release_run_report` attestation；`OQ-0002` 下真实 production monthly snapshot 也仍缺失。只要这两类成本 blocker 中任一未收口，`EVID-COST-001` 仍会继续失败，而 `EVID-MEDIA-001` / `EVID-DER-001` / `EVID-SEC-001` / `EVID-OPS-001` 也都会继续因 shared pre-release attestation 缺失而 fail-closed，故仍不能诚实声称达到 `L1 Local-Core`。
