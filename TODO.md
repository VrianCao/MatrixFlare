...
- [ ] 实现 `IF-CS-040`,`IF-CS-042`,`IF-CS-043`,`IF-CS-044`,`IF-CS-045`,`IF-INT-USER-004`,`DATA-USER-002`,`DATA-USER-003`,`DATA-USER-004`,`DATA-USER-005`,`DATA-USER-011`,`DATA-R2-006`。
  Spec refs: `30` 5.5, 6; `24` `DATA-USER-002`,`DATA-USER-003`,`DATA-USER-004`,`DATA-USER-005`,`DATA-USER-011`,`DATA-R2-006`; `43` `TEST-CS-003`; `44` `EVID-CS-003`
  产出:
  device CRUD、keys upload/query/claim、room key backup metadata / object handling、对应 gateway handler 与 `UserDO` 裁决路径。
  完成标准:
  `TEST-CS-003` 可在本地与 non-local 路径上被诚实实现，并验证 one-time key at-most-once、device surfaces 与 backup metadata。
  当前状态:
  `IF-CS-041` 的 `sendToDevice` 已在 `05.04` 落地，但仓库当前仍缺 `IF-CS-040`,`IF-CS-042`~`IF-CS-045` 对应的 gateway route / handler 落位；`apps/gateway-worker/src/index.mjs` 目前只能看到 `sendToDevice` 路由分发，看不到 `devices`、`keys/*`、`room_keys/*` 公共入口。因此 `TEST-CS-003` / `EVID-CS-003` 目前不是“只差 dedicated non-local suite”，而是缺更早的实现前置；该 prerequisite 必须先补回依赖顺序，再继续 Phase 08 收口。
...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  当前状态:
  `tests/integration/l1-mandatory.test.mjs`、`tests/staging/l1-mandatory.test.mjs`、`tests/pre-release/l1-mandatory.test.mjs` 已改成 dedicated remote harness，直接驱动部署后的 HTTP surface，并通过 `packages/testing/src/evidence.mjs` 的 parser 复核为 environment-backed；`packages/testing/src/nonlocal.mjs` 与 `.github/workflows/nonlocal-phase08.yml` 也已补上 Cloudflare 资源 ensure、环境 deploy、R2 artifact upload 与 GitHub Actions non-local 入口，因此 `08.04` 已不再停留在“薄 local shim”阶段。最新验证 run `23863557465`（code state `f400b26d36fbe2c962b75fd06456a69405cafd3b`）已再次证明 `ci-integration`、`staging`、`pre-release` 三环境都能成功部署、通过 bounded readiness probe、执行 environment-backed suite、上传 raw bundle 到 R2，并分别生成有效 `EnvironmentRunAttestation`；旧的“等待 staging/pre-release 成功 attestation” blocker，以及把 `23862913809` 那次 `pre-release` 失败继续写成 latest verified code state blocker 的做法，都已失效，必须回退。与此同时，parser 仍会对 template-literal dynamic import 等隐藏依赖 fail-closed，non-local deploy/run/upload/attest primitives 也会要求 GitHub-signed Actions OIDC job identity，并按 dedicated environment 全局串行 workflow / fresh deployment identity / 当前 account workers.dev subdomain / 当前 active deployment-version identity 回读复核，避免仅靠本地伪造 `GITHUB_ACTIONS=true`、跨 ref 争用环境、或篡改 deployment JSON 就假装产出 non-local gate 结果。  
  但本项仍未完成，且当前 blocker 已经收敛为四类：其一，`TEST-CS-003` 对应的 `IF-CS-040`,`IF-CS-042`~`IF-CS-045` 尚未在代码与更早的 `TODO` 依赖顺序中补齐，现已回退为新的前置项 `05.04A`；因此 `TEST-CS-003` 当前甚至还没进入“只差 non-local suite”的阶段。其二，除该前置缺口外，`tests/integration/l1-mandatory.test.mjs`、`tests/staging/l1-mandatory.test.mjs`、`tests/pre-release/l1-mandatory.test.mjs` 仍只覆盖 smoke 级 HTTP path，还没有把其余 `TEST-CS-001`,`TEST-CS-002`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001` 的完整 L1 语义迁入 dedicated non-local suites。其三，`packages/testing/src/evidence.mjs` 对 non-local canonical `TEST-ID -> implementation files` 继续 fail-closed，且现在不仅拒绝 `l1-mandatory.test.mjs`、`bootstrap.test.mjs` 与 `tests/shared/*` 这类 generic/shared proof 路径、拒绝 basename 未以对应 `TEST-ID` 小写形式起始的同目录改名 shim，还要求 canonical file 必须真实存在于 repo、且其 repo-owned transitive dependency closure 也留在同一环境目录内，因此当前仍不能诚实补上映射或宣称全部 mandatory non-local gate 项都已有 dedicated environment-backed proof。其四，`TEST-OPS-001` 与 `TEST-COST-001` 即使补出 dedicated file name，也仍分别需要真实 version-skew orchestration 与可审计的 pre-release cost/guardrail 可观测面；现有 smoke/provenance workflow 还不能把这两项自动抬成已验证通过。换言之，non-local harness 已从“薄 shim”推进到“latest verified code state 已具备三环境 attested smoke 与 provenance-ready workflow，但仍同时受前置功能缺口、dedicated suite 缺失、strict mapping，以及部分 test-id 额外可观测性要求约束”的状态，距离 `08.04` 完成仍有明显差距。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  目前仓库已补上 `.github/workflows/nonlocal-phase08.yml` 与对应 non-local tooling，可在 GitHub Actions 中部署 `ci-integration` / `staging` / `pre-release`、上传 raw bundle 到 R2、并按 `DEC-0002` 生成 provenance-ready attestation；workflow 也会在 evidence 阶段消费 `prod-cost-attestation`（若存在），否则继续对 `EVID-COST-001` fail-closed。最新验证 run `23863557465`（timestamp `20260401T180928Z`）已成功下载三份环境 attestation，并在 `evidence-l1` job 中生成新的 evidence bundles；旧的 `_test-runs` 原始 report 型 summary 已被 supersede，不再可作为 release-gate 依据。该次 evidence gate 仍 exit `1`，但其表层报错并不等于全部根因都已解决：第一，`EVID-CS-003` 对应的 `TEST-CS-003` 目前不仅缺 staging canonical mapping，更缺更早的实现前置 `05.04A`，因为 `devices`、`keys/*`、`room_keys/*` surfaces 还没有在 gateway 路由中真实落位。第二，除 `TEST-CS-003` 外，其余需要 non-local coverage 的 `EVID-*` 也继续因缺少对应环境的 canonical `TEST-ID -> implementation files` mapping 而 fail-closed，当前 summary 已明确显示例如 `EVID-CS-001` 缺 `ci-integration/staging` 的 `TEST-CS-001` mapping、`EVID-ROOM-001` 缺 `TEST-ROOM-001` mapping、`EVID-OPS-001` 缺 `pre-release` 的 `TEST-OPS-001` mapping、`EVID-COST-001` 缺 `pre-release` 的 `TEST-COST-001` mapping。第三，`EVID-OPS-001` 与 `EVID-COST-001` 即使未来补上 dedicated file path，也仍分别受真实 version-skew 证明路径与 pre-release cost/guardrail 可观测面的额外约束，不能拿 generic smoke 或纯命名映射充作通过。第四，`EVID-COST-001` 还额外因缺少真实 `prod-cost-attestation` 而 fail；该 production monthly cost snapshot 口径仍受 `spec/open-questions/OQ-0002.md` 约束。由于当前 `tests/integration|staging|pre-release` 下现有文件仍只有 `bootstrap.test.mjs` 与 `l1-mandatory.test.mjs` 这类 generic wiring/smoke 入口，`packages/testing/src/evidence.mjs` 现已明确拒绝把它们、`tests/shared/*` 支撑模块、basename 未以对应 `TEST-ID` 小写形式起始的同目录改名 shim、canonical path 指向不存在 repo 文件的伪映射，或经由 wrapper / symlink 间接逃出 repo-owned dedicated 环境目录的 dependency closure 计入 canonical non-local `TEST-ID` coverage proof；因此现在既不能靠补上映射把 evidence 伪装为通过，也不能把 `23863557465` 这次三环境 attested smoke run 误报成 `L1 Local-Core` 已达成。本项保持未完成。
