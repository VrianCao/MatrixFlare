...
- [x] 通过 `DEC-0005` 把 production automation contract 固化到 Spec/TODO，再落 GitHub Actions + CLI/tooling 的 `prod-install` / `release-candidate` / `promote-prod` / `rollback-prod` / `prod-cost-monthly` 基础设施；same-head remote 验证必须证明执行入口真实可用，且 fail-closed 时仍保留 raw blocker artifact。
  ...
  latest reviewed pushed head `700b8600264b78eacc07733a7fd09248d9c68e8a` 已对 `prod-cost-monthly` / phase08 embedded `prod-cost` contract 做了 same-head rerun 验证：standalone `prod-cost-monthly` run `24003512022` 与 embedded `prod-cost` job in `24003512030` 都先执行到 revised entry contract 中的 `Resolve closed production billing window` 与 `Download prod install baseline record` 路径；其中 billing-window step 在缺失可信 `next_bill_date` 时按 fail-closed 语义写出 blocker，而 baseline download 继续成功。standalone workflow 继续在 fail-closed 前保留并上传 raw blocker bundle，embedded `prod-cost` job 则还额外证明 workflow 能在 gate 前进入 `Write prod-cost provenance` step。最新 blocker 也因此从旧的 `/billing/usage/paygo ... 404` 进一步前移为“官方 billing profile 响应未提供可信 `next_bill_date`”，而不是 workflow/tooling 没接通。换言之，`08.05A` 当前已拿到 revised prod-cost contract 的 same-head remote proof，production automation prerequisite 现可诚实勾回；剩余未完成项已转移到 `08.05` 的成本证据闭环，而不再属于 automation entry contract 本身。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  同期 standalone `prod-cost-monthly` run `24003512022` 与 embedded `prod-cost` job in `24003512030` 也把 `OQ-0002` 下的 production blocker 进一步前移成了比 earlier 404 更精确的 current-head truth：revised contract 已真实执行到 `Resolve closed production billing window` 与当前 prod baseline artifact 下载路径；其中 billing-window step 在缺失可信 `next_bill_date` 时按 fail-closed 语义写出 blocker，standalone workflow 继续保留且上传 raw blocker bundle，embedded `prod-cost` job 还额外证明 workflow 能在 gate 前进入 `Write prod-cost provenance` step。但官方 billing profile 响应的 `result` 仍未包含有效 `next_bill_date`，因此 `billing-window.json` 被诚实写成 `resolution_method = cloudflare-account-billing-profile-next-bill-date` + `error = "Cloudflare billing profile must include a valid next_bill_date"`，workflow 在进入 usage query 之前就 fail-closed 停止。与此同时，同轮 `pre-release` raw artifact 仍明确记录 `TEST-DER-001`,`TEST-MEDIA-001`,`TEST-OPS-001`,`TEST-SEC-001` 全部 `pass`，但 `TEST-COST-001` 继续按 `OQ-0006` fail-closed，`pre_release_cost_observation` 仍为 `null`，pre-release attestation 被整体跳过。换言之，`08.05` 当前剩余 blocker 仍只剩两条而且都与成本相关：一是 `OQ-0006` 下 official billing-grade pre-release cost proof 仍不存在，继续挡住 shared `pre_release_run_report` attestation；二是 `OQ-0002` 下当前账号返回的官方 billing profile 仍未给出可信 closed-period anchor，且也尚未到达 fixed prod topology 的第一个 truthful post-install closed billing period。只要这两类 blocker 中任一未收口，`EVID-COST-001` 仍会继续失败，而 `EVID-MEDIA-001` / `EVID-DER-001` / `EVID-SEC-001` / `EVID-OPS-001` 也都会继续因为 shared pre-release attestation 缺失而 fail-closed，因此仍不能诚实声称达到 `L1 Local-Core`。
