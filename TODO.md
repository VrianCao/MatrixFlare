...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  但本项仍未完成：当前 remote harness 只覆盖 smoke 级路径，尚未把 `TEST-CS-001`~`TEST-COST-001` 的完整 L1 语义迁入 dedicated non-local suites；`packages/testing/src/evidence.mjs` 的 canonical `TEST-ID -> implementation files` 映射也仍只有 `local` 入口，尚不能诚实宣称全部 mandatory non-local gate 项都已有 dedicated environment-backed proof。因此即使 workflow 已存在，本项仍保持未完成。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  目前仓库已补上 `.github/workflows/nonlocal-phase08.yml` 与对应 non-local tooling，可在 GitHub Actions 中部署 `ci-integration` / `staging` / `pre-release`、上传 raw bundle 到 R2、并按 `DEC-0002` 生成 provenance-ready attestation；workflow 也会在 evidence 阶段消费 `prod-cost-attestation`（若存在），否则继续对 `EVID-COST-001` fail-closed。`2026-04-01` 的真实 GitHub Actions run `23855295188` 已证明 `pre-release` 能成功部署、执行 suite、上传 raw bundle 到 R2，并生成有效 attestation，但同轮 `ci-integration` 与 `staging` 仍因 `/_matrix/client/v3/publicRooms` 在 Cloudflare 远端触发 `operator_authz_policies` control-plane D1 schema bootstrap 的 multiline `exec()` 报错而失败，因此 `08.04` 的完整 non-local L1 语义覆盖仍未闭合；当前修复正是为消除此 blocker。与此同时，`20260331T130238Z` 那批基于 `_test-runs` 原始 report 的旧 L1 evidence summary 已被 supersede，不再可作为 release-gate 依据。更关键的是，`EVID-COST-001` 仍要求真实 production monthly cost snapshot，而 Cloudflare Billing Usage/Paygo API 的自动化适用边界仍记录在 `spec/open-questions/OQ-0002.md`，因此当前仍不能诚实宣称 `L1 Local-Core` 已达成，本项保持未完成。
