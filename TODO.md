...
- [ ] 为 `staging` / `pre-release` 建立可审计的 Access-protected management ingress 与 automation contract（account-owned `workers.dev`/Preview URL 或 active-zone custom hostname），作为 `TEST-DER-001` rebuild consistency 和 `TEST-OPS-001` positive non-local gate 的前置条件。
  ...
  但本项仍不能诚实打勾。在本轮修复提交前的未提交 worktree 中，`tests/local/runtime-foundations/nonlocal-tooling.test.mjs`、`tests/local/control-plane/control-plane.test.mjs` 与 dedicated `TEST-DER-001` suites 只能证明 contract 在 repo 内自洽；它仍缺少一轮来自对应代码状态的 GitHub Actions attested run，去证明真实 Cloudflare 账号上确实能拿到 team domain / AUD / service token material、通过 Access 命中 `/_ops/v1/healthz` / `/_ops/v1/rebuilds`，并让 `ops-worker` 在 live staging / pre-release 流量上收到可验证 `Cf-Access-Jwt-Assertion`。在这轮 current-head non-local proof 出来前，`OQ-0004` 继续保持 open，`08.04B` 继续保持未完成。
  截至 `2026-04-02T19:47:44Z`，最新真实 current-head non-local rerun 已更新为 GitHub Actions run `23918727505`（created `2026-04-02T19:43:52Z`，updated `2026-04-02T19:47:44Z`，code state `7bfb527e28dba368e7b9622b9078e43469182000`）。该 run 进一步证明当前问题已不再是 “Access automation 没接上”：`staging` / `pre-release` 的 `Prepare non-local ops Access session` 都成功，正向 `/_ops/v1/healthz` 也已通过，失败收敛到了 live `ops_rebuild_start`。两环境 raw bundle 现在一致返回 `503` + `D1_EXEC_ERROR: ... BEGIN IMMEDIATE; ... use state.storage.transaction()`，说明请求已经真实穿过 Access ingress 并进入 `/_ops/v1/rebuilds`，但当前代码状态仍在 control-plane D1 写路径上使用了 Cloudflare managed D1 不接受的 raw transaction SQL。基于该 run 的 raw bundle，本轮未提交修复 worktree 又进一步把 `packages/control-plane/src/persistence.mjs` 的 control-plane D1 事务改为 D1 `batch()` 聚合 prepared statements，并补上 `tests/local/runtime-foundations/data-plane-storage.test.mjs` 的 managed-D1/rollback 回归与 `tests/local/control-plane/control-plane.test.mjs` 的 rebuild managed-D1 回归；这些本地门禁已经重新通过，但在新的 current-head GitHub Actions attested rerun 证明 `/_ops/v1/rebuilds` 不再被 raw transaction SQL 卡住之前，`08.04B` 仍只能被诚实表述为“workers.dev Access ingress 已被 live run 证明可用，managed-D1 rebuild 兼容性修复已落地，待 rerun 证明”。

### 08.04C 收敛 `TEST-OPS-001` rollout-skew contract prerequisite

- [ ] 先把 `TEST-OPS-001` / `EVID-OPS-001` 所需的 dual-version orchestration、Worker/DO pairing observability 与 attested proof contract 写成可执行 Spec，再实现 non-local pre-release skew gate。
  Spec refs: `13` `CF-WKR-012`,`CF-WKR-014`,`CF-DO-005`, `42` `REQ-OPS-010`,`REQ-OPS-011`,`REQ-OPS-012`, `43` `TEST-OPS-001`, `44` `EVID-OPS-001`, `OQ-0005`
  产出:
  gradual deployment contract、version targeting / override strategy、Worker/DO version-pair observability contract、对应 workflow / harness / evidence 接线或 blocker。
  完成标准:
  `TEST-OPS-001` 对当前 repo 至少达到 `SR2`，不再只能以 `mapping_error` / placeholder suite 形式 fail-closed。
  当前状态:
  `spec/open-questions/OQ-0005.md` 已把本项单独写成真实前置条件：Cloudflare 平台虽支持 versions / deployments / gradual deployment / version override，但当前 repo 仍缺双版本 orchestration 与可 attestation 的 skew 观测 contract。换言之，`TEST-OPS-001` 目前并不是“忘了补 suite”，而是尚未达到可实施状态；在该 contract 落地前，不得把 `/_ops` healthz、单 deployment smoke、或 active deployment identity revalidation 误写成 rollout-skew proof。
...
- [ ] 先把 `TEST-COST-001` / `EVID-COST-001` 的 pre-release actual-cost proof contract 写成可执行 Spec，再讨论对应 canonical non-local suite。
  ...
  完成标准:
  `TEST-COST-001` 的 pre-release half 对当前 repo 至少达到 `SR2`，不再只能以 `mapping_error` 或模糊 “缺 prod snapshot” 叙述 fail-closed。
  当前状态:
  `spec/open-questions/OQ-0006.md` 已把本项与 `OQ-0002` 分离：`OQ-0006` 记录 pre-release cost proof contract 仍不存在，`OQ-0002` 则继续记录 production monthly snapshot / pricing semantics blocker。当前 repo 因此前后两半都未闭合；在 pre-release contract 写清之前，不得新增 placeholder `test-cost-001.test.mjs` 假装 coverage 已具备。
...
- [ ] 落地 `TEST-GOV-001`,`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-DER-001`,`TEST-SEC-001`,`TEST-OPS-001`,`TEST-COST-001`。
  ...
  但本项仍未完成，且当前真相已进一步收敛为四点。其一，`TEST-CS-001`,`TEST-CS-002`,`TEST-CS-003`,`TEST-CS-004`,`TEST-ROOM-001`,`TEST-ROOM-002`,`TEST-MEDIA-001`,`TEST-SEC-001` 已在 pushed-head attested run `23900855087` 上拿到 `pass`：下载并复核该 run 的 evidence artifact 后，可确认 `EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-SEC-001` 已全部转为 `pass`，因此这些项当前不再受“缺同轮 attestation”阻塞。其二，`TEST-DER-001` 已不再缺 canonical suite / mapping：`tests/staging/test-der-001.test.mjs` 与 `tests/pre-release/test-der-001.test.mjs` 现已作为 dedicated canonical suites 落地，`packages/testing/src/evidence.mjs` 也已接入 `TEST-DER-001 -> staging/pre-release` strict mapping；本地重新执行 `npm run evidence:l1 -- --timestamp 20260402T154112Z` 时，`EVID-DER-001` 的 `mapping_error` 已清零，当前失败理由已收敛为 “Missing valid staging/pre-release attestation”，说明 repo 内的 canonical coverage/mapping 半边已闭合。其三，`TEST-OPS-001` 不能再被表述成“只差 canonical file”：根据 `spec/open-questions/OQ-0005.md`，Cloudflare 平台虽然存在 gradual deployment + version targeting 能力，但当前仓库仍缺 dual-version orchestration、Worker/DO pairing observability 与 attested rollout-skew contract，因此本项对当前 repo 仍低于 `SR2`。其四，`TEST-COST-001` 也不再只是“缺 prod snapshot”：`spec/open-questions/OQ-0006.md` 已把 pre-release cost proof contract gap 单独写明，而 `spec/open-questions/OQ-0002.md` 继续记录 production monthly snapshot / pricing semantics blocker；因此本项对当前 repo 同样低于 `SR2`。换言之，`08.04` 现已真实推进到“除 `OPS` / `COST` 仍低于 SR2、`DER` 仍待 current-head non-local attested proof 外，其余 L1 dedicated canonical non-local coverage 已被 pushed-head attested run 证明”的状态；但在这些 blocker 收口前，仍不能把 `08.04` 标成完成。
  pushed-head 事实还必须再加上一层新的 live failure 收敛：run `23918727505`（created `2026-04-02T19:43:52Z`，updated `2026-04-02T19:47:44Z`，code state `7bfb527e28dba368e7b9622b9078e43469182000`）已经证明 `staging` / `pre-release` 的 `/_ops` Access healthz 正向入口可达，且 `ops_rebuild_start` 不再暴露 HTML `500`，而是稳定收敛为 Cloudflare managed D1 对 raw `BEGIN IMMEDIATE` 的 `503`/`D1_EXEC_ERROR`。这说明 `08.04B` 当前不再是 “Access 可能没接上” 或 “typed error 仍泄漏成平台 HTML”，而是更具体的 control-plane D1 transaction compatibility 问题。本轮未提交修复 worktree 又继续把这条 live 失败收敛为 repo-side `batch()` 事务修复，并通过新增 local regression 证明不再发 raw transaction SQL、且 grouped writes 仍保持原子回滚；但在新的 current-head attested rerun 出来之前，`08.04` 仍未完成，`L1` 也仍不能宣称闭环。
...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  在 `20260402T154112Z` 那轮本地验证中，evidence gate 继续保持 fail-closed，且失败理由已比 `23900855087` 时更精确：`npm run evidence:l1 -- --timestamp 20260402T154112Z` 仍只让 `EVID-GOV-001` 为 `pass`，因为当时 worktree 尚未导入新的非本地 attestation；但 `EVID-DER-001` 的本地失败已不再包含 canonical `mapping_error`，summary 现明确指向 `tests/staging/test-der-001.test.mjs` 与 `tests/pre-release/test-der-001.test.mjs` 这两个 canonical files，并把失败原因收敛为 `Missing valid staging/pre-release attestation for EVID-DER-001`。这说明 `DER` 的 repo-side dedicated suite + mapping 已经真实落地，剩余闭环现在是 current-head non-local attested proof，而不再是命名或目录层面的假缺口。相比之下，`EVID-OPS-001` 仍同时暴露两层真实 blocker：本地 evidence 因缺 pre-release attestation 而 fail，coverage summary 也继续记录 `mapping_error: Missing L1 test implementation mapping for "TEST-OPS-001" in environment "pre-release"`；该 mapping 缺口当前是 `OQ-0005` 记录的 honest 结果，而不是忘记补 suite。`EVID-COST-001` 也仍同时暴露两层真实 blocker：coverage summary 继续记录 `TEST-COST-001` 在 `pre-release` 的 mapping 缺失，manual artifacts 继续记录 `prod_cost_snapshot: missing`；前者受 `OQ-0006` 约束，后者继续受 `spec/open-questions/OQ-0002.md` 约束。因此，`08.05` 依旧保持未完成；即使 `EVID-SEC-001` 与 `EVID-CS-002` 已在 pushed head `23900855087` 上真实收口，在新的 non-local attested rerun 或新的 blocker artifact 出来之前，仍不能诚实声称达到 `L1 Local-Core`。
  在较早一轮未提交 dirty worktree（当时 `HEAD 5e48d033b8627ebaaf2140f4252fb8b484d4e002`，其上叠加了未提交的 Access fail-closed 与 derived-queue producer 修复）中，又追加执行了 `node --test tests/local/control-plane/control-plane.test.mjs`、`node --test tests/local/runtime-foundations/nonlocal-tooling.test.mjs`、`node --test tests/local/runtime-foundations/testing-harness.test.mjs`、`npm run governance:check`、`npm run test:all`、`date -u +%Y%m%dT%H%M%SZ`（得到 `20260402T161527Z`）以及 fail-closed `npm run evidence:l1 -- --timestamp 20260402T161527Z`。结果继续保持诚实：`EVID-GOV-001` 仍是唯一 `pass`；`EVID-DER-001` 继续只缺 staging / pre-release attestation，说明 repo-side canonical suite + mapping 仍成立；`EVID-OPS-001` 则继续同时暴露缺 pre-release attestation 与 `TEST-OPS-001` mapping gap，受 `OQ-0005` 约束；`EVID-COST-001` 继续同时暴露 `TEST-COST-001` pre-release mapping gap 与 `prod_cost_snapshot: missing`，分别受 `OQ-0006` 与 `OQ-0002` 约束。与此同时，`apps/jobs-worker/wrangler.jsonc` 与 `packages/runtime-core/src/runtime-manifest.mjs` 现已补回 `SEARCH_INDEX_QUEUE` / `MEDIA_THUMBNAIL_QUEUE` producer bindings，并由 `tests/local/runtime-foundations/nonlocal-tooling.test.mjs` 锁定；这说明该 worktree 已把此前 live non-local DER/search 路径暴露出的一个真实 repo-side 缺口收敛为“待 attested rerun 证明”，而不是继续留在未接线状态。
  在本轮提交前的未提交修复 worktree 中，又继续执行 `npm run governance:check`、`node --test tests/local/control-plane/control-plane.test.mjs`、`node --test tests/local/runtime-foundations/testing-harness.test.mjs`、`npm run test:all`、`date -u +%Y%m%dT%H%M%SZ`（得到 `20260402T193746Z`）以及 fail-closed `npm run evidence:l1 -- --timestamp 20260402T193746Z`。结果显示真相仍保持收敛且未被误报放宽：`EVID-GOV-001` 仍是唯一 `pass`；`EVID-DER-001` 继续只缺 valid `staging` / `pre-release` attestation；`EVID-OPS-001` 继续同时暴露缺 pre-release attestation 与 `TEST-OPS-001` mapping gap；`EVID-COST-001` 继续同时暴露缺 pre-release attestation、`TEST-COST-001` mapping gap 与 `prod_cost_snapshot: missing`。换言之，这轮追加的 stale-replay fail-closed 修复并没有把 `08.05` 人为跑绿，只是进一步排除了一个会把失败误重放成 `202 accepted` 的 repo-side 漏洞；在新的 attested run 或新的 blocker artifact 出来之前，`08.05` 仍然未完成，`L1 Local-Core` 仍不能诚实宣称达成。
  在当前未提交 worktree 继续把 control-plane D1 事务改为 `batch()`，并进一步补上“同一 persistence 实例上的 nested / concurrent `transaction()` 必须 fail-closed 拒绝、普通写也不得在活动事务期间静默并入该 batch”的回归之后，又重新执行了 `node --test tests/local/runtime-foundations/data-plane-storage.test.mjs`、`node --test tests/local/control-plane/control-plane.test.mjs`、`node --test tests/local/runtime-foundations/testing-harness.test.mjs`、`npm run governance:check`、`npm run test:all`、`date -u +%Y%m%dT%H%M%SZ`（得到 `20260402T204357Z`）以及 fail-closed `npm run evidence:l1 -- --timestamp 20260402T204357Z`。结果继续保持诚实：`data-plane-storage` 18/18 通过，`control-plane` 59/59 通过，`testing-harness` 183/183 通过，`test:all` 现在达到 401/401 本地通过；但 `evidence:l1` 仍然只让 `EVID-GOV-001` 为 `pass`。新的 summary 再次确认当前失败结构没有被误报放宽：`EVID-DER-001` 继续只缺 valid `staging` / `pre-release` attestation；`EVID-OPS-001` 继续同时暴露缺 pre-release attestation 与 `TEST-OPS-001` mapping gap；`EVID-COST-001` 继续同时暴露缺 pre-release attestation、`TEST-COST-001` mapping gap 与 `prod_cost_snapshot: missing`。换言之，这轮 managed-D1 transaction + shared-instance fail-closed 修复同样没有把 `08.05` 人为跑绿；在新的 attested run 或新的 blocker artifact 出来之前，`08.05` 仍未完成，`L1 Local-Core` 仍不能诚实宣称达成。
