...
- [ ] 生成 `EVID-GOV-001`,`EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001`。
  ...
  目前仓库已补上 `.github/workflows/nonlocal-phase08.yml` 与对应 non-local tooling，可在 GitHub Actions 中部署 `ci-integration` / `staging` / `pre-release`、上传 raw bundle 到 R2、并按 `DEC-0002` 生成 provenance-ready attestation；workflow 也会在 evidence 阶段消费 `prod-cost-attestation`（若存在），否则继续对 `EVID-COST-001` fail-closed。run `23869349481`（timestamp `20260401T202940Z`）仍是最近一轮成功拿到三份环境 attestation 的证据基线；该 run 的 `evidence-l1` job 在下载三份 attestation 后继续按预期 fail-closed：`EVID-GOV-001` 是唯一 `pass`，其余 `EVID-CS-001`,`EVID-CS-002`,`EVID-CS-003`,`EVID-CS-004`,`EVID-ROOM-001`,`EVID-ROOM-002`,`EVID-MEDIA-001`,`EVID-DER-001`,`EVID-SEC-001`,`EVID-OPS-001`,`EVID-COST-001` 全部为 `fail`。这轮失败说明的是 dedicated non-local canonical coverage 与 production cost evidence 仍未闭合，而不是 attestation/provenance contract 缺失；其中旧的“`EVID-CS-003` 仍缺 staging canonical mapping”与“环境目录里仍只有 generic smoke 入口”叙述已经失效，必须回退。  
  但它同样已不是当前代码状态的最新 non-local 尝试。最新 run `23879497392`（timestamp `20260402T014014Z`）里，`staging` 与 `pre-release` 因 `TEST-MEDIA-001` 失败而未能生成有效 attestation，`evidence-l1` 也因此只能在 attestation 不完整的条件下继续失败。换言之，当前分支既不能把 `23869349481` 写成“latest verified code state”，也不能把 `23879497392` 写成“新的 evidence-only gap”；它真实反映的是媒体 remote path 在前一代码状态上仍有回归，而当前工作树修复尚未通过新的 attested run 重新收敛。
