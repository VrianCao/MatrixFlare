...
test('TEST-ROOM-001 staging covers room creation, membership, send, redaction, receipts, and typing', async (context) => {
  ...
  const alice = await registerUser(harness, {
    usernamePrefix: 'room1-staging-alice',
    deviceId: 'ROOM1STGALICE',
  });
  const bob = await registerUser(harness, {
    usernamePrefix: 'room1-staging-bob',
    deviceId: 'ROOM1STGBOB',
  });

  const createdRoom = await createRoom(harness, alice.access_token, {
    invite: [bob.user_id],
    name: 'Phase 08 Room 001 Staging',
  });
  await joinRoom(harness, bob.access_token, createdRoom.room_id);

  const messageTxnId = makeTxnId('room1-stg-send');
  ...
  const conflictingRetry = await putAuthenticated(
    ...
    {
      msgtype: 'm.text',
      body: 'room one staging conflict',
    },
  );
  ...
  assert.equal(eventBeforeRedaction.payload?.content?.body, 'room one staging message');

  const redact = await putAuthenticated(
    harness,
    alice.access_token,
    roomPath(createdRoom.room_id, `/redact/${encodeURIComponent(firstMessage.payload.event_id)}/${makeTxnId('room1-stg-redact')}`),
    {
      reason: 'cleanup',
    },
  );
