...
test('TEST-CS-001 staging covers discovery, session lifecycle, and capability truth', async (context) => {
  ...
  for (const [index, registerPath] of REGISTER_ALIAS_PATHS.entries()) {
    const password = `phase08-cs1-staging-password-compat-${index}`;
    const compatibilityUser = await registerUser(harness, {
      usernamePrefix: `cs1-staging-compat-${index}`,
      password,
      deviceId: `CS1STGCOMPAT${index}`,
      registerPath,
    });
    assert.equal(compatibilityUser.user_id, `@${compatibilityUser.username}:${harness.serverName}`);
    assert.equal(compatibilityUser.device_id, `CS1STGCOMPAT${index}`);
  ...
  for (const [userIndex, compatibilityUser] of compatibilityUsers.entries()) {
    for (const [loginIndex, loginPath] of LOGIN_ALIAS_PATHS.entries()) {
      const deviceId = `CS1STGCOMPAT${userIndex}${loginIndex}`;
  ...
  const alice = await registerUser(harness, {
    usernamePrefix: 'cs1-staging-alice',
    password: 'phase08-cs1-staging-password-1',
    deviceId: 'CS1STGALICE',
  });
  ...
  assert.equal(whoAmI.payload?.device_id, 'CS1STGALICE');
...
test('TEST-CS-001 staging covers password-change UIA branches and deactivation', async (context) => {
  ...
  const carol = await registerUser(harness, {
    usernamePrefix: 'cs1-staging-carol',
    password: 'phase08-cs1-staging-password-2',
    deviceId: 'CS1STGCAROL',
  });
  ...
  const passwordChallenge = await request(harness, '/_matrix/client/v3/account/password', {
    ...
    json: {
      new_password: 'phase08-cs1-staging-password-3',
    },
  });
  ...
  const newPasswordLogin = await loginWithPassword(harness, {
    user: carol.username,
    password: 'phase08-cs1-staging-password-3',
    deviceId: 'CS1STGCAROL2',
  });
  ...
  const tokenlessPasswordChallenge = await request(harness, '/_matrix/client/v3/account/password', {
    method: 'POST',
    json: {
      new_password: 'phase08-cs1-staging-password-4',
      auth: {
        type: 'm.login.password',
        user: carol.username,
      },
  ...
  const latestPasswordLogin = await loginWithPassword(harness, {
    user: carol.username,
    password: 'phase08-cs1-staging-password-4',
    deviceId: 'CS1STGCAROL3',
  });
...
test('TEST-CS-001 staging covers profile full reads, keyName GET/PUT/DELETE, and profile propagation', async (context) => {
  const harness = requireRemoteHarnessContext(context, 'staging');
  if (harness == null) {
    return;
  }
  ...
  const alice = await registerUser(harness, {
    usernamePrefix: 'cs1-staging-profile-alice',
    deviceId: 'CS1STGPROFILEA',
  });
  const bob = await registerUser(harness, {
    usernamePrefix: 'cs1-staging-profile-bob',
    deviceId: 'CS1STGPROFILEB',
  });
  ...
  const setAvatar = await request(
    ...
    `/_matrix/client/v3/profile/${encodeURIComponent(alice.user_id)}/avatar_url`,
    {
      ...
      headers: authHeaders(alice.access_token),
      json: {
        avatar_url: 'mxc://matrix.example.test/cs1-staging-avatar',
      },
    },
  );
  ...
  const room = await createRoom(harness, alice.access_token, {
    invite: [bob.user_id],
    name: 'CS 001 Profile Propagation Staging',
  });
