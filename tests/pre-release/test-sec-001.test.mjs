...
test('TEST-SEC-001 pre-release enforces baseline abuse guards on always-on login, media, search, and publicRooms surfaces', async (context) => {
  const harness = requireRemoteHarnessContext(context, 'pre-release');
  if (harness == null) {
    return;
  }
  ...
  await assertPermissiveSearchAndPublicRoomsLimiter(harness, searchUser.access_token, 'phase08-sec-prerelease');
