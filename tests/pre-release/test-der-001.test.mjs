...
test('TEST-DER-001 pre-release covers derived query semantics and Access-authenticated rebuild consistency', async (context) => {
  const harness = requireRemoteHarnessContext(context, 'pre-release');
  if (harness == null) {
    return;
  }

  const token = makeTxnId('derpre');
  const aliasLocalpart = buildUniqueAliasLocalpart(`der-pre-${token}`);
  const publicRoomName = `Derived pre-release public ${token}`;
  const privateRoomName = `Derived pre-release private ${token}`;
  const spaceRoomName = `Derived pre-release space ${token}`;
  ...
  const alice = await registerUser(harness, {
    usernamePrefix: `der-pre-alice-${token}`,
    deviceId: 'DERPREALICE',
  });
  const bob = await registerUser(harness, {
    usernamePrefix: `der-pre-bob-${token}`,
    deviceId: 'DERPREBOB',
  });
  ...
  const userDirectoryRebuild = await requestOpsAuthorized(harness, '/_ops/v1/rebuilds', {
    method: 'POST',
    headers: {
      'Idempotency-Key': `test-der-001-pre-release-user-${token}`,
    },
    json: {
      ...
      reason: `TEST-DER-001 pre-release user-directory rebuild ${token}`,
      ticket_id: `OPS-DER-PRE-USER-${token}`,
  ...
  const roomDerivedRebuild = await requestOpsAuthorized(harness, '/_ops/v1/rebuilds', {
    method: 'POST',
    headers: {
      'Idempotency-Key': `test-der-001-pre-release-room-${token}`,
    },
    json: {
      ...
      reason: `TEST-DER-001 pre-release room-derived rebuild ${token}`,
      ticket_id: `OPS-DER-PRE-ROOM-${token}`,
      force_full_scan: false,
    },
  });
  assert.equal(
