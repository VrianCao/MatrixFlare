...
* After a valid initial request, Access can mint a JWT scoped to the application as `CF_Authorization`, but this only works as a continuing credential when the Access application has at least one `Allow` policy.
